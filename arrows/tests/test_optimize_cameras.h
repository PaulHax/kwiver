...
TEST ( optimize_cameras, no_noise )
{
  ...
  camera_map::map_camera_t original_cams =
    kwiver::testing::camera_seq()->cameras();
  ...
  feature_track_set_sptr tracks = projected_tracks(
    landmarks,
    working_cam_map );
  ...
  for( auto const& p : working_cam_map->cameras() )
  {
    ...
    EXPECT_MATRIX_NEAR(
      orig_cam_ptr->intrinsics()->as_matrix(),
