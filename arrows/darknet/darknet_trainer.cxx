...
public:
  ...
  // Helper functions
  std::vector< std::string > format_images(
    std::string folder,
    std::string prefix,
...
::get_configuration() const
{
  ...
  config->set_value(
    "net_config", d->m_net_config,
    "Name of network config file." );
  config->set_value(
    "seed_weights", d->m_seed_weights,
    "Optional input seed weights file." );
  config->set_value(
    "train_directory", d->m_train_directory,
    "Temp directory for all files used in training." );
  config->set_value(
    "skip_format", d->m_skip_format,
  ...
  config->set_value(
    "resize_option", d->m_resize_option,
...
::set_configuration( vital::config_block_sptr config_in )
{
  ...
  this->d->m_train_directory =
    config->get_value< std::string >( "train_directory" );
  this->d->m_skip_format = config->get_value< bool >( "skip_format" );
  this->d->m_gpu_index   = config->get_value< int >( "gpu_index" );
  this->d->m_resize_option =
    config->get_value< std::string >( "resize_option" );
  ...
  this->d->m_overlap_required =
    config->get_value< double >( "overlap_required" );
  this->d->m_random_int_shift =
    config->get_value< double >( "random_int_shift" );
...
::check_configuration( vital::config_block_sptr config ) const
{
  ...
  if( net_config.empty() || !kwiversys::SystemTools::FileExists( net_config ) )
  {
    LOG_ERROR(
      logger(),
      "net config file \"" << net_config << "\" not found." );
    return false;
  }
  ...
  std::vector< kwiver::vital::detected_object_set_sptr > test_groundtruth )
{
  ...
  if( !d->m_skip_format )
  {
    ...
    train_list = d->format_images(
      d->m_train_directory, "train",
      train_image_names, train_groundtruth, object_labels );
    test_list = d->format_images(
      d->m_train_directory, "test",
    ...
    std::string import_cmd =
      "import kwiver.arrows.darknet.generate_headers as dth;";
    ...
    std::string full_cmd = python_cmd + import_cmd + header_cmd + header_args +
                           header_end;
  ...
  std::string darknet_args = "-i " + std::to_string( d->m_gpu_index ) +
                             " detector train " + d->m_train_directory +
                             "/yolo_v2.data " +
...
::format_images(
  std::string folder, std::string prefix,
  std::vector< std::string > image_names,
  std::vector< kwiver::vital::detected_object_set_sptr > groundtruth,
  vital::category_hierarchy_sptr object_labels )
{
  ...
  for( unsigned fid = 0; fid < image_names.size(); ++fid )
  {
    ...
    kwiver::vital::detected_object_set_sptr scaled_detections_ptr =
      groundtruth[ fid ]->clone();
    ...
    if( m_resize_option != "chip" && m_resize_option != "chip_and_original" )
    {
      ...
      kwiver::vital::bounding_box_d roi_box( 0, 0, resized_image.cols,
        resized_image.rows );
      if( print_detections(
        gt_file, scaled_detections_ptr, roi_box,
        object_labels ) )
      {
    ...
    else
    {
      ...
      for( int i = 0; i < resized_image.cols - m_resize_i + m_chip_step;
           i += m_chip_step )
      {
        int cw = i + m_resize_i;
        ...
        for( int j = 0; j < resized_image.rows - m_resize_j + m_chip_step;
             j += m_chip_step )
        {
          int ch = j + m_resize_j;
          ...
          kwiver::vital::bounding_box_d roi_box( i, j, i + m_resize_i,
            j + m_resize_j );
          if( print_detections(
            gt_file, scaled_detections_ptr, roi_box,
            object_labels ) )
      ...
      if( m_resize_option == "chip_and_original" )
      {
        ...
        double scaled_original_scale = scale_image_maintaining_ar(
          original_image,
          scaled_original, m_resize_i, m_resize_j );

        kwiver::vital::detected_object_set_sptr scaled_original_dets_ptr =
          groundtruth[ fid ]->clone();
        ...
        if( print_detections(
          gt_file, scaled_original_dets_ptr, roi_box,
          object_labels ) )
        {
  ...
  vital::category_hierarchy_sptr object_labels )
{
  ...
  for( auto detection = all_detections->cbegin(); detection != ie; ++detection )
  {
    ...
    kwiver::vital::bounding_box_d overlap = kwiver::vital::intersection(
      region,
      det_box );
        ...
        overlap.area() / det_box.area() >= m_overlap_required )
    {
      ...
      {
        std::cout << "Warning: Ignoring unlisted class " << category <<
          std::endl;
        continue;
      }
