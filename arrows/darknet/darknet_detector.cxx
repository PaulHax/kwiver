...
::get_configuration() const
{
  ...
  config->set_value(
    "weight_file", d->m_weight_file,
    "Name of optional weight file." );
  config->set_value(
    "class_names", d->m_class_names,
    "Name of file that contains the class names." );
  config->set_value(
    "thresh", d->m_thresh,
    "Threshold value." );
  config->set_value(
    "hier_thresh", d->m_hier_thresh,
...
::set_configuration( vital::config_block_sptr config_in )
{
  ...
  d->m_net =
    parse_network_cfg( const_cast< char* >( d->m_net_config.c_str() ) );
...
::check_configuration( vital::config_block_sptr config ) const
{
  ...
  if( net_config.empty() )
  {
    ...
    fmt.print( str );
    LOG_ERROR(
      logger(), "Required net config file not specified. "
  ...
  if( class_file.empty() )
  {
    ...
    fmt.print( str );
    LOG_ERROR(
      logger(), "Required class name list file not specified, "
                "Configuration is as follows:\n" << str.str() );
    success = false;
  }
  else if( !kwiversys::SystemTools::FileExists( class_file ) )
  {
    LOG_ERROR(
      logger(),
      "class names file \"" << class_file << "\" not found." );
    success = false;
  }

  if( d->m_resize_option != "disabled" &&
      ( d->m_resize_i != 0 || d->m_resize_j != 0 ||
        d->m_resize_option == "scale" ) )
  {
...
::detect( vital::image_container_sptr image_data ) const
{
  ...
  if( d->m_resize_option != "disabled" )
  {
    scale_factor = format_image(
      cv_image, cv_resized_image,
  ...
  else
  {
    ...
    for( int li =
           0; li < cv_resized_image.cols - d->m_resize_i + d->m_chip_step;
         li += d->m_chip_step )
    {
           ...
           lj += d->m_chip_step )
      {
        ...
        cv::Mat cropped_image = cv_resized_image(
          cv::Rect(
            li, lj, ti - li,
            tj - lj ) );
        cv::Mat scaled_crop;
        ...
        vital::detected_object_set_sptr new_dets =
          d->process_image( scaled_crop );
    ...
    if( d->m_resize_option == "chip_and_original" )
    {
      ...
      vital::detected_object_set_sptr new_dets =
        d->process_image( scaled_original );
...
::process_image( const cv::Mat& cv_image )
{
  ...
  m_probs = ( float** ) calloc( l_size, sizeof( float* ) ); // allocate vector
                                                            // of pointers
  ...
  for( size_t i = 0; i < l_size; ++i )
  {
    ...
    // Iterate over all classes and collect all names over the threshold, and
    // max score
    double conf = 0.0;
