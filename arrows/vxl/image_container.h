...
public:
  ...
  virtual size_t
  width() const { return data_->ni(); }

  /// The height of the image in pixels
  virtual size_t
  height() const { return data_->nj(); }

  /// The depth (or number of channels) of the image
  virtual size_t
  depth() const { return data_->nplanes(); }

  /// Get an in-memory image class to access the data
  virtual vital::image
  get_image() const { return vxl_to_vital( *data_ ); }
  using vital::image_container::get_image;

  /// Get image data in this container.
  vil_image_view_base_sptr
  get_vil_image_view() const { return data_; }
