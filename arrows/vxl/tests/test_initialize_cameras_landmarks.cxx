...
TEST ( initialize_cameras_landmarks, create )
{
  EXPECT_NE(
    nullptr,
    algo::initialize_cameras_landmarks::create( "mvg-basic" ) );
}
...
static void
configure_algo(
  initialize_cameras_landmarks_basic& algo,
...
evaluate_initialization(
  const kwiver::vital::camera_map_sptr true_cams,
  ...
  double tol )
{
  ...
  vital::similarity_d global_sim = est_sim->estimate_transform(
    est_cams,
    true_cams );
  ...
  for( auto const& p : orig_cams )
  {
    ...
    vital::rotation_d dR = new_cam_t->rotation().inverse() *
                           orig_cam->rotation();
  ...
  {
    vital::landmark_sptr new_lm_tr = transform(
      new_lms[ p.first ],
      global_sim );
...
TEST ( initialize_cameras_landmarks, subset_init )
{
  ...
  vital::landmark_map_sptr new_landmarks( new vital::simple_landmark_map(
    lms_to_init ) );
  ...
  new_cameras =
    vital::camera_map_sptr( new vital::simple_camera_map( cams_to_init ) );
  ...
  new_landmarks =
    vital::landmark_map_sptr( new simple_landmark_map( lms_to_init ) );
