...
public:
  ...
  SetUp()
  {
    points = { { -0.01717344,  0.63277306,  1.16735385 },
    ...
    test_points = { { -0.33473656,  1.52684247, -0.86753264 },
      { -0.55981981, -1.09909536,  0.59573499 },
      { 0.46238199,  0.46761188,  0.07077688 },
      { 1.55802851, -0.65494246,  2.50201591 } };

    expected_nearest_indices = { { 9, 8, 2 }, { 10, 7, 1 }, { 4, 0, 6 },
      { 5, 11, 0 } };

    expected_nearest_distances = { { 0.44681713, 0.88033834, 1.41468588 },
      { 0.90424467, 1.20810056, 1.28346573 },
      { 1.11075157, 1.20819398, 1.21565536 },
      { 1.94259906, 2.41259374, 2.43326843 } };

    expected_radius_indices = { { 9, 8 }, { 7, 10, 1 }, { 10, 11, 4, 0, 6, 14 },
      {} };
  }
...
TEST_F ( kd_tree_test, find_nearest )
{
  ...
  search_tree.find_nearest_point(
    test_points[ 0 ], K, nearest_single_indices,
  ...
  search_tree.find_nearest_points(
    test_points, K, nearest_indices,
    nearest_distances );
...
TEST_F ( kd_tree_test, find_within_radius )
{
  ...
  {
    search_tree.find_within_radius(
      test_points[ i ], radius,
      nearest_indices );
