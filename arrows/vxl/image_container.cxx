  ...
  case F:                                                          \
    {                                                              \
      ...
      vital::image_memory_sptr memory =                            \
        vxl::vxl_to_vital( img_t.memory_chunk() );                 \
      return vital::image_of< pix_t >(                             \
    memory, img_t.top_left_ptr(),                                  \
...
#undef CONVERT_CASE
    default:
      VITAL_THROW(
        ...
        "kwiver::arrows::vxl::image_container::vxl_to_vital(const vil_image_view_base&)" );
  }
  return vital::image();
}
...
make_vil_image_view( const vital::image& img )
{
  ...
  return new vil_image_view< T >(
    chunk, reinterpret_cast< const T* >( img.first_pixel() ),
...
::vital_to_vxl( const vital::image& img )
{
  ...
  VITAL_THROW(
    vital::image_type_mismatch_exception,
    "kwiver::arrows::vxl::image_container::vital_to_vxl(const image&)" );
