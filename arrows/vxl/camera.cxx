...
template < typename T >
void
vital_to_vpgl_camera(
  const camera_perspective& mcam,
...
template < typename T >
void
vpgl_calibration_to_vital(
  const vpgl_calibration_matrix< T >& vcal,
  simple_camera_intrinsics& mcal )
{
  vgl_point_2d< T > vpp = vcal.principal_point();
  mcal = simple_camera_intrinsics(
    vcal.focal_length() * vcal.x_scale(),
...
template < typename T >
void
vital_to_vpgl_calibration(
  const camera_intrinsics& mcal,
  vpgl_calibration_matrix< T >& vcal )
{
  const vector_2d& mpp = mcal.principal_point();
  vcal = vpgl_calibration_matrix< T >(
    static_cast< T >( mcal.focal_length() ),
    vgl_point_2d< T >(
      static_cast< T >( mpp.x() ),
      static_cast< T >( mpp.y() ) ),
...
template KWIVER_ALGO_VXL_EXPORT camera_perspective_sptr vpgl_camera_to_vital( \
  const vpgl_perspective_camera< T >& vcam );                                 \
template KWIVER_ALGO_VXL_EXPORT void vpgl_camera_to_vital(                    \
  const vpgl_perspective_camera< T >&, simple_camera_perspective& );          \
template KWIVER_ALGO_VXL_EXPORT void vital_to_vpgl_camera(                    \
  const camera_perspective&, vpgl_perspective_camera< T >& );                 \
template KWIVER_ALGO_VXL_EXPORT void vpgl_calibration_to_vital(               \
  const vpgl_calibration_matrix< T >& vcal,                                   \
  simple_camera_intrinsics & mcal );                                          \
template KWIVER_ALGO_VXL_EXPORT void vital_to_vpgl_calibration(               \
  const camera_intrinsics& mcal,                                              \
  vpgl_calibration_matrix< T >& vcal )
