...
opt_orient_pos(
  vpgl_perspective_camera< double > const& camera,
  ...
  std::vector< vgl_point_2d< double > > const& image_points )
{
  ...
  return vpgl_perspective_camera< double >(
    K, c_min,
    vgl_rotation_3d< double >( w_min ) );
}
  ...
  kwiver::vital::sfm_constraints_sptr constraints ) const
{
  ...
  {
    LOG_WARN(
      vital::get_logger( "arrows.vxl.optimize_cameras" ),
  ...
  camera =
    std::dynamic_pointer_cast< vital::camera_perspective >( mcamera.clone() );
}
