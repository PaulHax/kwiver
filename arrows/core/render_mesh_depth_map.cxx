  ...
  vital::camera_perspective_sptr camera )
{
  ...
  vital::image_of< double > zbuffer( camera->image_width(),
  ...
  if( mesh->faces().regularity() == 3 )
  {
    ...
    transform_image(
      zbuffer, [](double d){
        return std::isinf( d ) ? d : ( -1.0 / d );
      } );
  }
  else
  {
    LOG_ERROR(
      vital::get_logger( "arrows.core.render_mesh_depth_map" ),
      "The mesh has to be triangular." );
  }
...
vital::image_container_sptr
render_mesh_height_map( vital::mesh_sptr mesh, vital::camera_sptr camera )
{
  vital::image_of< double > height_map( camera->image_width(),
    camera->image_height(), 1 );
  // fill each pixel with infinity
  transform_image(
    height_map, [](double){
  ...
  if( mesh->faces().regularity() == 3 )
  {
    vital::camera_perspective_sptr perspective_camera =
      std::dynamic_pointer_cast< vital::camera_perspective >( camera );
    if( perspective_camera )
    {
      vital::image_container_sptr depth_map_container =
        render_mesh_depth_map( mesh, perspective_camera );
...
/// This function converts a depth map into a height map obtained with a
/// perspective camera
void
depth_map_to_height_map(
  vital::camera_perspective_sptr const& camera,
...
vital::vector_3d
triangle_attribute_vector(
  vital::vector_2d const& v1,
...
void
render_triangle(
  const vital::vector_2d& v1, const vital::vector_2d& v2,
  const vital::vector_2d& v3,
  ...
  vital::image_of< double >& depth_img )
{
  ...
  auto Vd = triangle_attribute_vector(
    v1, v2, v3, depth_v1, depth_v2,
    depth_v3 );
  ...
  for( tsi.reset(); tsi.next(); )
  {
    ...
    int max_x = std::min(
      static_cast< int >( depth_img.width() ) - 1,
      tsi.end_x() );
