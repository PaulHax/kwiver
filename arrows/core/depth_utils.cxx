...
{
  std::vector< vector_3d > visible_landmarks = filter_visible_landmarks(
    cam,
    roi, landmarks );
  ...
  vector_3d const& world_normal )
{
  ...
  compute_offset_range(
    visible_landmarks, world_normal, height_min,
    height_max );
}
...
points_of_box(
  kwiver::vital::vector_3d const& minpt,
...
void
compute_offset_range(
  std::vector< vector_3d > const& landmarks,
  ...
  const double safety_margin_factor )
{
  ...
  const double safety_margin = safety_margin_factor *
