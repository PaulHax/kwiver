...
stretch_to_byte_image( kwiver::vital::image_of< T > const& in_image )
{
  ...
  kwiver::vital::foreach_pixel(
    in_image, [ &min_v, &max_v ](T p){
      if( std::isfinite( p ) )
      {
  ...
  // map the range of finite values to [1, 255] and use 0 for all non-finite
  // values
  T scale = 254.0 / ( max_v - min_v );
  T offset = 1.0 - min_v * scale;
  kwiver::vital::image_of< uint8_t > byte_image( in_image.width(),
    in_image.height() );
  kwiver::vital::transform_image(
    in_image, byte_image,
...
::add_command_options()
{
  ...
  ( "output-config", "Dump configuration for tool",
...
::run()
{
  ...
  if( !kwiver::vital::algo::image_io::check_nested_algo_configuration(
    "image_io", config ) )
  {
  ...
  std::dynamic_pointer_cast< kwiver::vital::simple_camera_perspective >(
