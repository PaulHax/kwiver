...
//      m_delim( "," )
  ...
  m_delim() { return parent.get_delimiter(); }
...
// ----------------------------------------------------------------------------
void
set_configuration_internal() override
{
...
::new_stream()
{
  d->m_first = true;
  d->m_stream_reader =
    std::make_shared< kwiver::vital::data_stream_reader >( stream() );
}
...
::get_input()
{
  ...
  if( m_input_buffer.size() < 7 )
  {
    ...
    str << "Too few field in input at line " <<
      m_stream_reader->line_number() << std::endl
        << "\"" << line << "\"";
  ...
  {
    std::stringstream str;
    str << "Invalid format in input at line " <<
      m_stream_reader->line_number() << std::endl
        << "\"" << line << "\"";
    VITAL_THROW( kwiver::vital::invalid_data, str.str() );
  }
