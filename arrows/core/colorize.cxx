...
/// \file
/// \brief Implementation of kwiver::arrows::core::colorize functions to
/// extract/compute colors
  ...
  vital::feature_track_set const& tracks )
{
  ...
  for( auto const& track : tracks.tracks() )
  {
    ...
    if( lmi != no_such_landmark )
    {
      ...
      {
        auto fts =
          std::dynamic_pointer_cast< vital::feature_track_state >( ts );
        if( !fts )
        {
          continue;
        }
      ...
      if( k )
      {
        ...
        auto lm =
          std::make_shared< kwiver::vital::landmark_d >( *( lmi->second ) );
        lm->set_color( { r, g, b } );
        lmi->second = lm;
      }
    }
