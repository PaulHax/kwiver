...
double
check_neighbouring_pixels(
  const kwiver::vital::image& image, double x,
  double y )
{
  ...
  if( x1 >= 0 && x1 < static_cast< int >( image.width() ) && y1 >= 0 &&
...
generate_mesh()
{
  std::unique_ptr< mesh_vertex_array_base > verts( new mesh_vertex_array< 3 >(
    vertices ) );
  std::unique_ptr< mesh_regular_face_array< 3 > > faces(
    new mesh_regular_face_array< 3 >() );
  faces->push_back( face1 );
  faces->push_back( face2 );

  mesh_sptr mesh( new kwiver::vital::mesh(
    std::move( verts ),
    std::move( faces ) ) );
  return mesh;
}
...
TEST ( render_mesh_depth_map, perspective_camera )
{
  ...
  // Perspective camera
  camera_intrinsics_sptr camera_intrinsic( new simple_camera_intrinsics(
    780, { 500, 500 },
    1.0, 0.0, {},
    1000, 1000 ) );
  ...
  camera_perspective_sptr camera( new simple_camera_perspective(
    cam_position,
    cam_orientation.inverse(),
    camera_intrinsic ) );

  image_container_sptr depth_map =
    kwiver::arrows::core::render_mesh_depth_map( mesh, camera );
  ...
  double measured_depth_A = check_neighbouring_pixels(
    depth_map->get_image(),
    a( 0 ), a( 1 ) );
  double measured_depth_B = check_neighbouring_pixels(
    depth_map->get_image(),
    b( 0 ), b( 1 ) );
  double measured_depth_C = check_neighbouring_pixels(
    depth_map->get_image(),
    c( 0 ), c( 1 ) );
  double measured_depth_bary =
    check_neighbouring_pixels( depth_map->get_image(), bary( 0 ), bary( 1 ) );
...
TEST ( render_mesh_height_map, perspective_camera )
{
  ...
  image_container_sptr height_map =
    kwiver::arrows::core::render_mesh_height_map( mesh, camera );
  ...
  double measured_height_A = check_neighbouring_pixels(
    height_map->get_image(),
    a( 0 ), a( 1 ) );
  double measured_height_B = check_neighbouring_pixels(
    height_map->get_image(),
    b( 0 ), b( 1 ) );
  double measured_height_C = check_neighbouring_pixels(
    height_map->get_image(),
    c( 0 ), c( 1 ) );
  double measured_height_bary =
    check_neighbouring_pixels( height_map->get_image(), bary( 0 ), bary( 1 ) );
