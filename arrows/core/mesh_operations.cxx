...
// Helper function to find intersection of edge between two vertices and a
// plane.
  ...
  vector_4d const& plane )
{
  ...
  {
    LOG_ERROR(
      vital::get_logger( "arrows.core.clip_mesh" ),
  ...
  for( auto const& tri : triangles )
  {
    ...
    unsigned new_vert_ind1 = edge_plane_intersection(
      ind[ 0 ], ind[ 1 ], vertices,
      dist, new_vert_map );
    unsigned new_vert_ind2 = edge_plane_intersection(
      ind[ 0 ], ind[ 2 ], vertices,
      dist, new_vert_map );
    ...
    if( dist[ ind[ 0 ] ] > 0.0 )
    {
      ...
      new_vert_ind1, ind[ 1 ], ind[ 2 ], new_vert_ind2,
      vertices ) )
    {
  ...
  double near, double far, double margin )
{
  ...
  vital::vector_4d right_plane = ( camera.image_width() + margin ) *
                                 P.row( 2 ) - P.row( 0 );
  vital::vector_4d top_plane = P.row( 1 ) + margin * P.row( 2 );
  vital::vector_4d bottom_plane = ( camera.image_height() + margin ) *
                                  P.row( 2 ) - P.row( 1 );
