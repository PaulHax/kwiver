...
::get_configuration() const
{
  ...
  config->set_value(
    "min_required_inlier_count", d_->min_required_inlier_count,
    "The minimum required inlier point count. If there are less "
    "than this many inliers, no matches will be output." );
  config->set_value(
    "min_required_inlier_percent", d_->min_required_inlier_percent,
  ...
  vital::algo::estimate_homography::get_nested_algo_configuration(
  ...
  vital::algo::match_features::get_nested_algo_configuration(
    ...
    matcher2_ );
  vital::algo::filter_features::get_nested_algo_configuration(
...
::set_configuration( vital::config_block_sptr in_config )
{
  ...
  vital::algo::estimate_homography::set_nested_algo_configuration(
    "homography_estimator",
    config, h_estimator_ );
  ...
  vital::algo::match_features::set_nested_algo_configuration(
    "feature_matcher2", config,
    matcher2_ );
  vital::algo::filter_features::set_nested_algo_configuration(
    "filter_features", config,
    feature_filter_ );

  // Other parameters
  d_->inlier_scale = config->get_value< double >( "inlier_scale" );
  d_->min_required_inlier_count =
    config->get_value< int >( "min_required_inlier_count" );
  d_->min_required_inlier_percent =
    config->get_value< double >( "min_required_inlier_percent" );
}
...
::check_configuration( vital::config_block_sptr config ) const
{
  ...
  if( config->has_value( "filter_features" ) &&
      ...
      !vital::algo::filter_features::check_nested_algo_configuration(
        "filter_features", config ) )
  {
    config_valid = false;
  }
  ...
  return (
    vital::algo::estimate_homography::check_nested_algo_configuration(
  ...
  feature_set_sptr feat2, descriptor_set_sptr desc2 ) const
{
  ...
  match_set_sptr init_matches = matcher1_->match(
    src_feat, src_desc,
    dst_feat, dst_desc );

  // estimate a homography from the initial matches
  std::vector< bool > inliers;
  homography_sptr H = h_estimator_->estimate(
