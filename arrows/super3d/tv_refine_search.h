...
/// \file
/// \brief Header file for tv_refine_search, minimizes a cost volume using total
/// variation and
...
public:
  ...
  depth_refinement_monitor(
    std::function< bool( update_data ) > callback,
    int interval )
    : callback_( callback ),
      interval_( interval )  {}

private:
  friend void refine_depth(
    vil_image_view< double >& cost_volume,
    const vil_image_view< double >& g,
    vil_image_view< double >& d,
    unsigned int iterations,
