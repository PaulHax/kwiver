  ...
  depth_refinement_monitor* drm )
{
  ...
  vil_image_view< double > sqrt_cost_range( cost_volume.ni(), cost_volume.nj(),
    1 );
...
#pragma omp parallel for
  for( int64_t j = 0; j < d.nj(); j++ )
  {
    for( unsigned int i = 0; i < d.ni(); i++ )
    {
      ...
      double& dij = d( i, j );
      dij = ( dij + step * ( g( i, j ) * ( divx + divy ) + theta_inv * a(
        i,
        j ) ) ) / denom;
    }
  }
  ...
  for( int64_t j = 0; j < d.nj(); j++ )
  {
    ...
    for( unsigned int i = 0; i < d.ni(); i++, col_c += istep_c )
    {
      ...
      const int r = std::min(
        last_plane,
        std::max(
          0, static_cast< int >( S * range_coeff *
                                 sqrt_range ) ) );
      const double dij = d( i, j ) * S - 0.5;
      const int init_k = std::min(
        last_plane,
        std::max( 0, static_cast< int >( dij ) ) );
      ...
      if( bestk > 0 && bestk < S - 1 )
      {
        ...
        const double diff2 = 2 * coeff *
                             ( dij - static_cast< double >( bestk ) );
