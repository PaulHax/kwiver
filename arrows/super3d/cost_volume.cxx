  ...
  const std::vector< vil_image_view< bool > >& masks )
{
  ...
  std::vector< vpgl_perspective_camera< double > > warp_cams =
    ws->warp_cams( cameras, ref_frame );

  double s_step = 1.0 / static_cast< double >( S );

  LOG_DEBUG(
    logger, "Computing cost volume of size ("
      << cost_volume.ni() << ", "
      << cost_volume.nj() << ", "
      << cost_volume.nplanes() << ")" );
  ...
  for( unsigned int k = 0; k < S; k++ )
  {
    ...
    ws->warp_image_to_depth(
      ref, warp_ref, warp_cams[ ref_frame ],
      s, ref_frame, -1.0 );
    if( !masks.empty() )
    {
      ws->warp_image_to_depth(
        masks[ ref_frame ], warp_ref_mask,
    ...
    for( unsigned int f = 0; f < frames.size(); f++ )
    {
      ...
      {
        ws->warp_image_to_depth(
          masks[ f ], warp_mask, warp_cams[ f ], s, f,
          false );
      }
...
#pragma omp parallel for
      for( int64_t j = 0; j < warp_ref.nj(); j++ )
      {
        ...
        {
          if( warp( i, j ) == -1 ||
              ( !masks.empty() && ( warp_ref_mask( i, j ) || warp_mask(
                i,
                j ) ) ) )
          {
            continue;
...
double
compute_depth_sampling(
  world_space const& ws,
  std::vector< vpgl_perspective_camera< double > > const& cameras )
{
  ...
  for( unsigned i = 0; i < 3; ++i )
  {
    ...
    {
      auto near = ws.point_at_depth_on_axis(
        ( ws.ni() * i ) / 2,
        ( ws.nj() * j ) / 2, 0.0 );
      auto far = ws.point_at_depth_on_axis(
        ( ws.ni() * i ) / 2,
        ( ws.nj() * j ) / 2, 1.0 );
