...
::descriptors() const
{
  ...
  {
    vital::descriptor_fixed< int, 4 >* d = new vital::descriptor_fixed< int,
      4 >;
...
descriptors_to_viscl( const vital::descriptor_set& desc_set )
{
  ...
  for( unsigned int i = 0; i < descriptors.size(); i++ )
  {
    ...
    if( const vital::descriptor_fixed< int, 4 >* dfixed =
          dynamic_cast< const vital::descriptor_fixed< int,
            4 >* >( descriptors[ i ].get() ) )
    {
      cl_int4 d;
  ...
  {
    viscl::buffer buf =
      viscl::manager::inst()->create_buffer< cl_int4 >(
        CL_MEM_READ_WRITE,
        viscl_descr.size() );
    viscl::cl_queue_t queue = viscl::manager::inst()->create_queue();
    queue->enqueueWriteBuffer(
      *buf().get(), CL_TRUE, 0, buf.mem_size(),
      &viscl_descr[ 0 ] );
    queue->finish();
    return buf;
  }
