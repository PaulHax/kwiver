...
features_to_viscl( const vital::feature_set& features )
{
  ...
  fs.features_ =
    viscl::manager::inst()->create_buffer< cl_int2 >(
      CL_MEM_READ_WRITE,
      numfeat[ 0 ] );
  ...
  queue->enqueueWriteBuffer(
    *fs.numfeat_().get(), CL_TRUE, 0,
    fs.numfeat_.mem_size(), numfeat );
...
::size() const
{
  ...
  q->enqueueReadBuffer(
    *data_.numfeat_().get(), CL_TRUE, 0,
    data_.numfeat_.mem_size(), buf );
  return buf[ 0 ];
}
