...
public:
  ...
  void
  set_tracked_feature_location_mask(
    ...
    image_container_sptr image_data )
  {
    ...
    int log_tracked_feat_mask_downsample_fact =
      int( floor( log2f( float( exclude_rad_pixels ) / 2.0f ) ) );

    tracked_feat_mask_downsample_fact = 1 <<
                                        log_tracked_feat_mask_downsample_fact;

    int tfh = int( ceil(
      float( image_data->height() ) /
      float( tracked_feat_mask_downsample_fact ) ) );
    int tfw = int( ceil(
      float( image_data->width() ) /
      float( tracked_feat_mask_downsample_fact ) ) );
  ...
  void
  set_exclude_mask( cv::Point2f const& pt )
  {
  ...
  bool
  exclude_mask_is_set( vital::vector_2d const& pt ) const
  {
    return tracked_feature_location_mask.at< unsigned char >(
      pt.y() /
      tracked_feat_mask_downsample_fact,
  ...
  check_configuration( vital::config_block_sptr config ) const
  {
    ...
    {
      LOG_ERROR(
        m_logger, "redetect_frac_lost_threshold ("
    ...
    {
      LOG_ERROR(
        m_logger, "grid_rows (" << test_grid_rows <<
    ...
    {
      LOG_ERROR(
        m_logger, "grid_cols (" << test_grid_cols <<
          ") must be greater than 0" );
      success = false;
    }
    ...
    {
      LOG_ERROR(
        m_logger, "new_feat_exclusionary_radius_image_fraction ("
    ...
    {
      LOG_ERROR(
        m_logger, "win_size (" << test_win_size <<
  ...
  public:
    ...
    void
    set_grid_size( int _rows, int _cols )
    {
    ...
    void
    get_grid_size( int& _rows, int& _cols ) const
    {
      _rows = rows;
      _cols = cols;
    }
    ...
    bool
    should_redetect(
      const feature_distribution_image& lastDetectDist,
      ...
      image_container_sptr image_data )
    {
      ...
      for( auto v = points.begin(); v != points.end(); ++v )
      {
        ...
        int dist_bin_x =
          std::min< int >(
            std::max< int >( tp.x / dist_bin_x_len, 0 ),
            cols - 1 );
        int dist_bin_y =
          std::min< int >(
            std::max< int >( tp.y / dist_bin_y_len, 0 ),
            rows - 1 );
    ...
    uint16_t&
    get_bin_feat_count( const cv::Point2f& pt, image_container_sptr image_data )
    {
      const int dist_bin_x_len = int( image_data->width() / cols );
      const int dist_bin_y_len = int( image_data->height() / rows );
      int dist_bin_x =
        std::min< int >(
          std::max< int >( pt.x / dist_bin_x_len, 0 ),
          cols - 1 );
      int dist_bin_y =
        std::min< int >(
          std::max< int >( pt.y / dist_bin_y_len, 0 ),
          rows - 1 );
...
bool
feat_stren_less( feature_sptr a, feature_sptr b )
{
  ...
  image_container_sptr mask ) const
{
  ...
  cv::Mat cv_img = ocv::image_container::vital_to_ocv(
  ...
  if( mask && mask->size() > 0 )
  {
    ...
    cv_mask = ocv::image_container::vital_to_ocv(
      i,
      ocv::image_container::OTHER_COLOR );
  }
  ...
  if( !active_tracks.empty() )
  {
    ...
    // limit the stored detection pyramids to ones in which the active tracks
    // started
         ...
         ++kf_feat_i )
    {
      ...
      // first we check if the active track has a descriptor.  If it does, it's
      // not a klt track so we skip over it.
      track_sptr t = prev_klt_tracks[ kf_feat_i ];

      // info from feature detector (location, scale etc.)
      auto last_fts =
        std::dynamic_pointer_cast< feature_track_state >( t->back() );
  ...
  if( detect_new_features )
  {
    ...
    for( feat_itr fit = vf.crbegin(); fit != vf.crend(); ++fit )
    {
      ...
      cv::Point2f new_pt = cv::Point2f(
        ( *fit )->loc().x(),
        ( *fit )->loc().y() );

      // check if there are too many features already in this bin, either
      // tracked from
      // the previous frame or detected in this frame.
      uint16_t& bin_count = d_->dist_image.get_bin_feat_count(
        new_pt,
        image_data );
      ...
      d_->det_data_map[ next_track_id ] = priv::detection_data(
        frame_number,
        ( *fit )->loc() );
      ...
      d_->set_exclude_mask( new_pt );      // this makes the points earlier in
                                           // the
    ...
    d_->last_detect_distImage.set_from_feature_vector(
      next_points,
      image_data );
  }
