...
public:
  ...
  create() const
  {
    ...
    return cv::Ptr< cv_FREAK_t >(
      new cv_FREAK_t(
        orientation_normalized, scale_normalized, pattern_scale,
        n_octaves )
    );
#else
    return cv_FREAK_t::create(
      orientation_normalized, scale_normalized,
  ...
  void
  update( cv::Ptr< cv_FREAK_t > freak ) const
  {
  ...
  void
  set_config( vital::config_block_sptr const& config )
  {
    orientation_normalized =
      config->get_value< bool >( "orientation_normalized" );
