...
TEST ( image, create )
{
  ...
  ASSERT_NE(
    nullptr,
    img_io =
      kwiver::vital::create_algorithm< kwiver::vital::algo::image_io >(
        "ocv" ) );
...
run_ocv_conversion_tests( cv::Mat const& img )
{
  ...
  image const& vimg = ocv::image_container::ocv_to_vital(
    img,
  ...
  cv::Mat img2 = ocv::image_container::vital_to_ocv(
    vimg,
    ocv::image_container::RGB_COLOR );
...
template < typename T >
void
run_vital_conversion_tests(
  kwiver::vital::image_of< T > const& img,
  bool requires_copy = false )
{
  // convert to a cv::Mat and verify that the properties are correct
  cv::Mat ocv_img = ocv::image_container::vital_to_ocv(
    img,
    ocv::image_container::RGB_COLOR );
  ...
  image img2 = ocv::image_container::ocv_to_vital(
    ocv_img,
    ocv::image_container::RGB_COLOR );
...
using conversion_types =
  ::testing::Types< uint8_t, int8_t, uint16_t, int16_t, int32_t, float,
    double >;
    ...
    for( int j = 0; j < ocv_img.rows; ++j )
    {
      for( int i = 0; i < ocv_img.cols; ++i )
      {
        for( int c = 0; c < 3; ++c )
        {
          ASSERT_EQ(
            img( i, j, c ),
            ocv_img.ptr< TypeParam >( j )[ num_c * i + ( 2 - c ) ] )
            << "Pixels differ at " << i << ", " << j << ", (" << c << "," <<
            ( 2 - c ) << ")";
        }
      }
    ...
    ocv::image_container::BGR_COLOR );
  {
    ...
    for( int j = 0; j < ocv_img.rows; ++j )
    {
      for( int i = 0; i < ocv_img.cols; ++i )
      {
        ...
        ASSERT_EQ(
          img( i, j, 3 ),
          ocv_img.ptr< TypeParam >( j )[ num_c * i + ( 3 ) ] )
...
TYPED_TEST ( get_image, crop )
{
  ...
  image_container_sptr img_cont =
    std::make_shared< ocv::image_container >(
      img,
      ocv::image_container::RGB_COLOR );
