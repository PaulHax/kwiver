...
::set_schema( schema_style new_schema )
{
  ...
  {
    LOG_ERROR(
      main_logger, "KPF yaml writer: can't change schemas mid-stream; was "
...
operator<<( record_yaml_writer& w, const packet_t& p )
{
  ...
  switch( p.header.style )
  {
    ...
    case packet_style::ID:     w << writer< canonical::id_t >( p.id, d );
      break;
    case packet_style::TS:     w <<
        writer< canonical::timestamp_t >( p.timestamp, d );
      break;
    case packet_style::TSR:    w <<
        writer< canonical::timestamp_range_t >( p.timestamp_range, d );
      break;
