...
struct kpf_io_adapter : public kpf_canonical_io_adapter_base
{
  ...
  kpf_io_adapter(
    USER_TYPE( *k2u )( const KPF_TYPE& ),
    KPF_TYPE( *u2k )( const USER_TYPE& ) )
    : kpf2user_function( k2u ),
      kpf2user_inplace( nullptr ),
      user2kpf_function( u2k )
  {}

  kpf_io_adapter(
    void( *k2u )( const KPF_TYPE&, USER_TYPE& ),
    KPF_TYPE( *u2k )( const USER_TYPE& ) )
    : kpf2user_function( nullptr ),
      kpf2user_inplace( k2u ),
      user2kpf_function( u2k )
  {}

  // these two methods both convert a KPF type into a user type.
  USER_TYPE
  operator()( const KPF_TYPE& k ) { return ( user2kpf_function ) ( k ); }
  void
  operator()( const KPF_TYPE& k, USER_TYPE& u )
  {
    ( kpf2user_inplace ) ( k, u );
  }
...
{
  kpf_box_adapter(
    USER_TYPE( *k2u )( const canonical::bbox_t& ),
  ...
  kpf_box_adapter(
    void( *k2u )( const canonical::bbox_t&, USER_TYPE& ),
    canonical::bbox_t( *u2k )( const USER_TYPE& ) )
    : kpf_io_adapter< USER_TYPE, canonical::bbox_t >( k2u, u2k )
  {
    this->packet_bounce.init( packet_header_t( packet_style::GEOM ) );
  ...
  void
  get( kpf_reader_t& parser, USER_TYPE& u )
  {
...
{
  kpf_poly_adapter(
    USER_TYPE( *k2u )( const canonical::poly_t& ),
  ...
  kpf_poly_adapter(
    void( *k2u )( const canonical::poly_t&, USER_TYPE& ),
    canonical::poly_t( *u2k )( const USER_TYPE& ) )
    : kpf_io_adapter< USER_TYPE, canonical::poly_t >( k2u, u2k )
  {
    this->packet_bounce.init( packet_header_t( packet_style::POLY ) );
...
template < typename USER_TYPE >
struct kpf_act_adapter : public kpf_io_adapter< USER_TYPE,
                           canonical::activity_t >
{
  kpf_act_adapter(
    USER_TYPE( *k2u )( const canonical::activity_t& ),
  ...
  kpf_act_adapter(
    void( *k2u )( const canonical::activity_t&, USER_TYPE& ),
    canonical::activity_t( *u2k )( const USER_TYPE& ) )
    : kpf_io_adapter< USER_TYPE, canonical::activity_t >( k2u, u2k )
  {
    this->packet_bounce.init( packet_header_t( packet_style::ACT ) );
