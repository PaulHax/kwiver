...
{
  writer( const canonical::bbox_t& b, int d ) : box( b ),
                                                domain( d ) {}
...
{
  writer( const canonical::poly_t& p, int d ) : poly( p ),
                                                domain( d ) {}
...
{
  writer( const canonical::activity_t& a, int d ) : activity( a ),
                                                    domain( d ) {}
...
{
  writer( const canonical::cset_t& c, int d ) : cset( c ),
                                                domain( d ) {}
...
{
  reader( kpf_canonical_io_adapter_base& b, int d ) : box_adapter( b ),
                                                      domain( d ) {}
...
{
  reader( kpf_canonical_io_adapter_base& b, int d ) : poly_adapter( b ),
                                                      domain( d ) {}
...
{
  reader( kpf_canonical_io_adapter_base& b, int d ) : act_adapter( b ),
                                                      domain( d ) {}
...
{
  reader( size_t& id, int d ) : id_ref( id ),
                                domain( d ) {}
  size_t& id_ref;
  int domain;
};
...
public:
  ...
  reader( int& ts, int d ) : which( to_int ),
                             int_ts( ts ),
                             unsigned_ts( u_dummy ),
                             double_ts( d_dummy ),
                             domain( d ) {}
  reader( unsigned& ts, int d ) : which( to_unsigned ),
                                  int_ts( i_dummy ),
                                  unsigned_ts( ts ),
                                  double_ts( d_dummy ),
                                  domain( d ) {}
  reader( double& ts, int d ) : which( to_double ),
                                int_ts( i_dummy ),
                                unsigned_ts( u_dummy ),
                                double_ts( ts ),
                                domain( d ) {}
...
{
  reader( const std::string& k, std::string& v ) : key( k ),
                                                   val( v ) {}
  std::string key;
  std::string& val;
};
...
{
  reader( canonical::cset_t& c, int d ) : cset( c ),
                                          domain( d ) {}
  canonical::cset_t& cset;
  int domain;
};
...
public:
  ...
  reader( std::pair< int, int >& ts, int d ) : which( to_int ),
                                               int_ts( ts ),
                                               unsigned_ts( u_dummy ),
                                               double_ts( d_dummy ),
                                               domain( d ) {}
  reader( std::pair< unsigned, unsigned >& ts, int d ) : which( to_unsigned ),
                                                         int_ts( i_dummy ),
                                                         unsigned_ts( ts ),
                                                         double_ts( d_dummy ),
                                                         domain( d ) {}
  reader( std::pair< double, double >& ts, int d ) : which( to_double ),
                                                     int_ts( i_dummy ),
                                                     unsigned_ts( u_dummy ),
                                                     double_ts( ts ),
                                                     domain( d ) {}
