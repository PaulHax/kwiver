...
public:
  ...
  friend KPF_YAML_EXPORT kpf_reader_t& operator>>(
    kpf_reader_t& t,
    packet_bounce_t& b );
  ...
  const packet_buffer_t&
  get_packet_buffer() const { return this->packet_buffer; }
  ...
  std::pair< bool,
    packet_t > transfer_packet_from_buffer(
    const packet_header_t& h,
    bool set_bad_if_missing = false );

  // like above, but specifically for kv (key/value) packets with a
  // particular key
  std::pair< bool,
    packet_t > transfer_kv_packet_from_buffer(
