...
struct user_complex_detection_t
{
  ...
  {}
  user_complex_detection_t(
    int d, unsigned f, const pair< double, double >& c,
...
bool
operator==(
  const user_complex_detection_t& lhs,
...
{
  return {
    { 100, 4, { 33.3, 33.3 }, 10, 20, "vehicle", 0.3, { 10, 20, 10 },
      { 10, 20, 30 } },
    { 101, 4, { 44.4, 44.4 }, 4, 9,   "person",  0.8, { 10, 20, 20, 10 },
      { 10, 10, 20, 20 } },
    { 102, 5, { 55.5, 55.5 }, 11, 7,  "vehicle", 0.5, { 1, 2, 1 },
      { 1, 2, 3 } } };
}
...
read_detections_from_stream( std::istream& is )
{
  ...
  )
  {
    ...
    // buffers them up until it sees the next non-metadata record (or
    // end-of-file.)
    //
  ...
  const vector< user_complex_detection_t >& dets )
{
  ...
  for( const auto& det : dets )
  {
    ...
    w
      ...
      det.frame_number,
      KPFC::timestamp_t::FRAME_NUMBER )
      << KPF::writer< KPFC::poly_t >(
      poly_adapter( det ),
      KPFC::poly_t::IMAGE_COORDS )
...
IMPLEMENT_TEST( kpf_yaml_complex_io )
{
  ...
  vector< user_complex_detection_t > new_dets =
    read_detections_from_stream( ss );
  {
    ostringstream oss;
    oss << "Wrote " << src_dets.size() << " detections; read " <<
      new_dets.size() << " back";
...
IMPLEMENT_TEST( kpf_packet_extraction )
{
  ...
  TEST_EQUAL(
    "Start of packet extraction: reader is good",
    static_cast< bool >( reader ), true );

  // read a line of packets
  reader.next();
  TEST_EQUAL(
    "Read one line; reader is good", static_cast< bool >( reader ),
    true );

  auto packet_buffer = reader.get_packet_buffer();
  TEST_EQUAL(
    "After one line: non-zero number of packets",
    packet_buffer.empty(), false );

  const auto id_header = KPF::packet_header_t(
    KPF::packet_style::ID,
    KPFC::id_t::DETECTION_ID );
  ...
  TEST_EQUAL(
    "After one line: first transfer of ID succeeded", id_one.first,
    true );
  TEST_EQUAL(
    "After one line: reader still good",
    static_cast< bool >( reader ), true );

  // get the detection ID again (should fail but leave reader good)
  auto id_two = reader.transfer_packet_from_buffer( id_header );
  TEST_EQUAL(
    "After one line: second transfer of ID failed", id_two.first,
    false );
  TEST_EQUAL(
    "After one line: reader still good",
    static_cast< bool >( reader ), true );
