...
struct KPF_YAML_EXPORT validation_data
{
  packet_style style;
  std::string key;
  validation_data() : style( packet_style::INVALID ),
                      key( "" ) {}
  explicit validation_data( YAML::const_iterator it );
  explicit validation_data( packet_style s ) : style( s ),
                                               key( "" ) {}
  validation_data( packet_style s, const std::string& k ) : style( s ),
                                                            key( k ) {}
...
validate_schema(
  schema_style schema,
  const std::vector< validation_data >& vpackets );
