...
struct scoped
{
  ...
  int domain;
  scoped() : t( T() ),
             domain( packet_header_t::NO_DOMAIN ) {}
  explicit scoped( const T& t_in ) : t( t_in ),
                                     domain( packet_header_t::NO_DOMAIN ) {}
  scoped( const T& t_in, int domain_in ) : t( t_in ),
                                           domain( domain_in ) {}
};
...
struct KPF_YAML_EXPORT bbox_t
{
  ...
  bbox_t() : x1( 0 ),
             ...
             x2( 0 ),
             y2( 0 ) {}
  bbox_t( double a, double b, double c, double d ) : x1( a ),
                                                     y1( b ),
                                                     x2( c ),
                                                     y2( d ) {}
};
...
{
  timestamp_range_t( double a, double b ) : start( a ),
                                            stop( b ) {}
  timestamp_range_t() : start( 0 ),
                        stop( 0 ) {}
  double start, stop;
};
