...
required_packets( KPF::schema_style schema )
{
  switch( schema )
  {
    ...
    default:
      LOG_ERROR(
        main_logger, "Unhandled validation schema request for "
...
{
  for( auto style : { schema_style::INVALID, schema_style::META,
                      schema_style::GEOM, schema_style::ACT,
...
validate_schema( schema_style schema, const packet_buffer_t& packets )
{
  ...
  LOG_DEBUG(
    ...
    "Looking for required '" <<
      validation_data::schema_style_to_str( schema ) );

  for( const auto& v : req )
  {
    LOG_DEBUG(
      main_logger,
      "verification check for " << style2str( v.style ) << " / '" << v.key <<
        "'" );
