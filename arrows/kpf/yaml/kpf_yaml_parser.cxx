...
bool
cset_helper_parser(
  const string& context, const YAML::Node& n,
  KPFC::cset_t& cset )
{
  if( !n.IsMap() )
  {
...
parse_poly( KPF::packet_t& p, const YAML::Node& n )
{
  ...
  for( auto pt = n.begin(); pt != n.end(); ++pt )
  {
    ...
    if( npt.size() != 2 )
    {
      LOG_ERROR(
        main_logger,
        "YAML parser: poly: pt has " << npt.size() << " points; expected 2" );
      return false;
    }
    try
    {
      xy.push_back(
        make_pair(
          stod( npt[ 0 ].as< string >() ),
          stod( npt[ 1 ].as< string >() ) ) );
    }
    catch( const std::invalid_argument& e )
    {
      LOG_ERROR(
        main_logger, "YAML parser: geometry: error converting to double: "
          << e.what() );
      return false;
    }
  } // ... for all points
...
parse_geom( KPF::packet_t& p, const string& s )
{
  ...
  {
    LOG_ERROR(
      main_logger, "YAML parser: geometry: Expected four tokens from '"
        << s << "'; got " << tokens.size() );
    return false;
  }
  ...
  catch( const std::invalid_argument& e )
  {
    LOG_ERROR(
      main_logger,
      "YAML parser: geometry: error converting to double " << e.what() );
    for( auto i = 0; i < 4; ++i )
    {
  ...
  try
  {
    switch( p.header.style )
    {
      ...
      default:
      {
        LOG_ERROR(
          main_logger,
          "No implementation for parsing packet of type " << p.header );
        okay = false;
      }
    } // ... switch
  } // ... try
  ...
  vector< KPFC::scoped< KPFC::timestamp_range_t > >& tsvector )
{
  ...
  for( auto seq = n.begin(); seq != n.end(); ++seq )
  {
    ...
    for( auto i = seq->begin(); i != seq->end(); ++i )
    {
      ...
      {
        LOG_ERROR(
          main_logger, "timespan parser: couldn't parse timespan from '"
            << i->first.as< string >() << "'" );
        return false;
      }
      ...
      {
        LOG_ERROR(
          main_logger, "timespan parser: sequence length " << tsr_payload.size()
                                                           << "; expected 2" );
        return false;
      }
      tsvector.push_back(
        KPFC::scoped< KPFC::timestamp_range_t >(
          KPFC::timestamp_range_t(
            tsr_payload[ 0 ].as< double >(),
...
parse_activity( const YAML::Node& n, KPF::packet_buffer_t& local_packet_buffer )
{
  ...
  for( auto i = n.begin(); i != n.end(); ++i )
  {
    ...
    if( h.style == KPF::packet_style::ACT )
    {
      ...
      if( !cset_helper_parser(
        "activity-label cset", i->second,
        act.activity_labels ) )
      {
        return false;
      }
  ...
  p.header = KPF::packet_header_t(
    KPF::packet_style::ACT,
