...
pair< bool, packet_t >
kpf_reader_t
::transfer_packet_from_buffer(
  const packet_header_t& h,
...
{
  auto probe = t.transfer_packet_from_buffer(
    packet_header_t(
      packet_style::ID,
      r.domain ), true );
  if( probe.first )
  ...
  const reader< canonical::meta_t >& r )
{
  auto probe =
    t.transfer_packet_from_buffer(
      packet_header_t( packet_style::META ),
      true );
  if( probe.first )
  {
...
{
  auto probe =
    t.transfer_packet_from_buffer( packet_header_t( packet_style::TSR ), true );
  if( probe.first )
  {
    switch( r.which )
    {
      ...
      case reader< canonical::timestamp_range_t >::to_unsigned:
        r.unsigned_ts = make_pair(
          static_cast< unsigned >( probe.second.timestamp_range.start ),
          static_cast< unsigned >( probe.second.timestamp_range.stop ) );
        break;
      case reader< canonical::timestamp_range_t >::to_double:
        r.double_ts = make_pair(
          probe.second.timestamp_range.start,
