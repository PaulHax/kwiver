...
config_valid = false
  ...
  std::string active_attribute =
    config->get_value< std::string >( "active_attribute" );
...
public:
  ...
  process_command_line( cxxopts::ParseResult& cmd_args )
  {
    ...
    occlusion_threshold_ = config_->get_value(
      "occlusion_threshold",
      occlusion_threshold_ );
    ...
    remove_color_count_less_equal_ =
      config_->get_value(
        "remove_color_count_less_equal",
        remove_color_count_less_equal_ );
...
// --------------------------------------------------------------------------
  ...
  load_mesh( std::string const& input_mesh )
  {
    ...
    char const* output_path )
  {
    ...
    for( vtkIdType i = 0; i < numPts; ++i )
    {
      ...
      {
        vtkIdType idx = 3 * i;
        colors[ i ] = vital::rgb_color(
          rgbArray->GetValue( idx ),
  ...
  run_algorithm()
  {
    ...
    auto cameras = load_camera_map(
      video_reader_, video_source_,
      cameras_dir_ );
    set_cameras( cameras );
    ...
    {
      vtkDataArray* active =
        output->GetPointData()->GetArray( active_attribute_.c_str() );
      if( !active )
      {
