...
config_valid = false
  ...
  config_valid = validate_required_input_file(
    "video_source", *config,
    main_logger ) && config_valid;

  config_valid = validate_required_input_file(
    "input_landmarks_file", *config,
    main_logger ) && config_valid;

  config_valid = validate_optional_input_file(
    "mask_source", *config,
    main_logger ) && config_valid;

  // This functions as checking for a required input directory, by not allowing
  // it to be created
  config_valid = validate_required_output_dir(
    "input_cameras_directory",
    *config, main_logger, false ) && config_valid;
...
public:
  ...
  process_command_line( cxxopts::ParseResult& cmd_args )
  {
    ...
    {
      output_depths_directory =
        cmd_args[ "output-depths-dir" ].as< std::string >();
...
// --------------------------------------------------------------------------
  ...
  default_config()
  {
    ...
    config->set_value(
      "input_cameras_directory", input_cameras_directory,
      "Path to a directory to read cameras from." );

    config->set_value(
      "input_landmarks_file", input_landmarks_file,
...
// --------------------------------------------------------------------------
    ...
    kv::frame_id_t frame )
  {
    ...
    {
      LOG_ERROR(
        main_logger, "The depth data for frame "
          << frame <<
          " was empty" );
      return false;
    }
  ...
  compute_frames_in_range()
  {
    ...
    const kv::frame_id_t start_frame =
      config->get_value< kv::frame_id_t >( "batch_depth:first_frame" );
    const kv::frame_id_t end_frame =
      config->get_value< kv::frame_id_t >( "batch_depth:end_frame" );

    for( auto itr = cameras.begin(); itr != cameras.end(); itr++ )
    {
      if( itr->first >= start_frame &&
          ( end_frame < 0 || itr->first <= end_frame ) )
      {
  ...
  run_single_frame_algorithm(
    const kv::camera_perspective_map& pcm,
    ...
    double height_max )
  {
    ...
    LOG_DEBUG(
      main_logger, "found " << similar_cameras->size()
    ...
    auto cb = nullptr;
    int ref_frame = gather_depth_frames(
      *similar_cameras, video_reader,
    ...
    LOG_DEBUG(
      main_logger, "ref frame at index " << ref_frame
                                         << " out of " << frames_out.size() );

    auto crop = kwiver::arrows::core::project_3d_bounds(
      minpt, maxpt, *cameras_out.at( ref_frame ),
    ...
    auto depth = depth_algo->compute(
      frames_out, cameras_out,
  ...
  run_algorithm()
  {
    ...
    kwiver::arrows::core::height_range_from_3d_bounds(
      minpt, maxpt, height_min,
      height_max );
    ...
    if( config->has_value( "frame_index" ) )
    {
      ...
      // compute depth
      auto depth = run_single_frame_algorithm(
        pcm,
        frame, minpt,
        maxpt, height_min,
        height_max );
    ...
    else
    {
      ...
      const size_t num_depth_maps =
        config->get_value< size_t >( "batch_depth:num_depth" );
...
::add_command_options()
{
  ...
  ( "f,frame",    "The frame number to compute depth for.",
    cxxopts::value< int > () )
  ( "l,input-landmarks-file", "3D sparse features",
    cxxopts::value< std::string > () )
  ( "m,mask-source", "Masks of unusable regions",
    cxxopts::value< std::string > () )

  // positional parameters
  ( "video-source", "Video input file", cxxopts::value< std::string > () )
  ( "input-cameras-dir", "Camera location data",
    cxxopts::value< std::string > () )
  ...
  m_cmd_options->parse_positional(
    { "video-source", "input-cameras-dir",
      "output-depths-dir" } );
}
