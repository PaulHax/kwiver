...
config_valid = false
  ...
  config_valid =
    validate_required_input_dir(
      "input_cameras_directory", *config,
      main_logger ) && config_valid;

  config_valid =
    validate_required_input_dir(
      "input_depths_directory", *config,
      main_logger ) && config_valid;

  config_valid =
    validate_optional_output_file(
      "output_volume_file", *config,
      main_logger ) && config_valid;
...
public:
  ...
  process_command_line( cxxopts::ParseResult& cmd_args )
  {
    ...
    {
      input_depths_directory =
        cmd_args[ "input-depths-dir" ].as< std::string >();
  ...
  default_config()
  {
    ...
    config->set_value(
      "input_depths_directory", input_depths_directory,
  ...
  common_camera_and_depth_basenames()
  {
    ...
    for( unsigned long i = 0; i < num_cam_files; ++i )
    {
      ...
      if( ST::GetFilenameLastExtension( file ) == CAMERA_EXTENSION )
      {
        ...
        camera_basenames.insert(
          ST::GetFilenameWithoutLastExtension(
            filename ) );
      }
    }
    ...
    for( unsigned long i = 0; i < num_depth_files; ++i )
    {
      ...
      {
        std::string filename = ST::GetFilenameName( file );
        depth_basenames.insert(
          ST::GetFilenameWithoutLastExtension(
            filename ) );
      }
    }

    std::vector< std::string > union_basenames( std::min(
      camera_basenames.size(),
      depth_basenames.size() ) );

    auto last_position = std::set_intersection(
      camera_basenames.begin(),
  ...
  write_isosurface()
  {
    if( output_mesh_file != "" )
    {
      ...
      {
        LOG_WARN(
          main_logger, "output_mesh_file " << output_mesh_file
                                           <<
  ...
  void
  compute_ROI( kv::vector_3d& minpt, kv::vector_3d& maxpt )
  {
  ...
  run_algorithm()
  {
    ...
    if( output_mesh_file != "" )
    {
      ...
      contour_filter->SetInputArrayToProcess(
        0, 0, 0,
...
::add_command_options()
{
  ...
  ( "t,isosurface-threshold", "isosurface extraction threshold (default: " +
    std::to_string(d->isosurface_threshold) + ").",
