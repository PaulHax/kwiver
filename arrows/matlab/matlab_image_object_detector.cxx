...
///
///   - get_configuration() - returns the required configuration (format to be
/// determined)
///     May just punt and pass a filename to the algorithm and let it decode the
/// config.
///
///   - set_configuration() - accepts a new configuration into the detector. (?)
///
///   - check_configuration() - returns error if there is a configuration
/// problem
///
///   - detect() - performs detection operation using input variables as input
/// and
///     produces output on output variables.
///
/// Input variables:
///  - in_image - contains the input image. Shape of the array is the size of
/// the image.
///
/// Output variables:
///  - detected_object_set - array containing detections; boxes and confidence
///  - detected_object_classification - array of structs containing the
/// classification
///    labels and scores.
...
public:
  ...
  initialize_once()
  {
    ...
    std::string full_path =
      kwiversys::SystemTools::CollapseFullPath( m_matlab_program );
...
::get_configuration() const
{
  ...
  config->set_value(
    "program_file", d->m_matlab_program,
...
::detect( kwiver::vital::image_container_sptr image_data ) const
{
  ...
  if( col < 5 )
  {
    throw std::runtime_error(
      "Insufficient columns in detections array. Must have 5 columns." );
  }
  ...
  for( size_t i = 0; i < num_det; i++ )
  {
    ...
    if( class_rows ) // there are some classification details
    {
      ...
      for( size_t cc = 0; cc < class_cols; ++cc )
      {
        ...
        cmd << "temp_temp=detected_object_classification(" << ( i + 1 ) <<
          "," << ( cc + 1 ) << ").name;";
        d->eval( cmd.str() );

        MxArraySptr temp = d->engine()->get_variable( "temp_temp" );

        // If the name is empty, then there are no more names for this
        // detection.
        if( temp->size() == 0 )
        {
          continue;
