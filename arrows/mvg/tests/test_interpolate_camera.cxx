...
TEST ( interpolate_camera, interpolation )
{
  ...
  b(
    vector_3d( 3, 3, 3 ),
    rotation_d( -pi / 2, vector_3d( 0, 0, 1 ) ) ),           // rotated around
                                                             // z-axis 90
                                                             // degrees
  c;
  c = kwiver::arrows::mvg::interpolate_camera( a, b, 0.5 );

  cerr << "a.rotation: " << a.rotation().axis() << ' '  <<
    a.rotation().angle() << endl;
  cerr << "b.rotation: " << b.rotation().axis() << ' '  <<
    b.rotation().angle() << endl;
  ...
  cerr << "c.rotation (aa): " << c.rotation().axis() << ' ' <<
    c.rotation().angle() << endl;
...
TEST ( interpolate_camera, multiple_interpolations )
{
  ...
  {
    cerr << "\t" << cam.center() << " :: " << cam.rotation().axis() << " " <<
      cam.rotation().angle() << endl;
  }

  simple_camera_perspective i1 = cams[ 1 ],
    i2 = cams[ 2 ],
    i3 = cams[ 3 ];
  cerr << "i1 .25 c : " << i1.center() << " :: " << i1.rotation().axis() <<
    ' ' << i1.rotation().angle() << endl;
  cerr << "i2 .25 c : " << i2.center() << " :: " << i2.rotation().axis() <<
    ' ' << i2.rotation().angle() << endl;
  cerr << "i3 .25 c : " << i3.center() << " :: " << i3.rotation().axis() <<
    ' ' << i3.rotation().angle() << endl;
