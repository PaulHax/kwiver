...
/// Generate an interpolated camera between \c A and \c B by a given fraction \c
/// f
vital::simple_camera_perspective
interpolate_camera(
  vital::simple_camera_perspective const& A,
  vital::simple_camera_perspective const& B, double f )
{
  ...
  vital::rotation_d R = interpolate_rotation(
    A.get_rotation(),
    B.get_rotation(), f );
  ...
  vital::vector_2d principal_point = f1 * k1->principal_point() + f *
                                     k2->principal_point();
  double aspect_ratio = f1 * k1->aspect_ratio() + f * k2->aspect_ratio();
  double skew = f1 * k1->skew() + f * k2->skew();
  vital::simple_camera_intrinsics k( focal_len, principal_point, aspect_ratio,
    skew );
...
interpolate_camera(
  vital::camera_perspective_sptr A,
  vital::camera_perspective_sptr B, double f )
{
  ...
  return std::dynamic_pointer_cast< vital::camera_perspective >(
    interpolate_camera(
      vital::simple_camera_perspective( *A ),
