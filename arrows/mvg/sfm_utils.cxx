...
crop_camera(
  const kwiver::vital::camera_perspective_sptr& cam,
  ...
  for( auto key1 = key2++; key2 != key_idx.end(); ++key1, ++key2 )
  {
    ...
    while( k1 < k2 )
    {
      while( k1 < k2 )
      {
        ...
        double avg_tracks = ( mm.coeff( k1, k1 + 1 ) + mm.coeff(
          k2 - 1,
          k2 ) ) / 2;
  ...
  vital::simple_camera_perspective_map::frame_to_T_sptr_map const& cams )
{
  ...
  for( auto cam : cams )
  {
    ...
    {
      auto simp_cam =
        std::static_pointer_cast< simple_camera_perspective >( cam.second );
      im_dims dims;
  ...
  for( const track_sptr& t : trks )
  {
    if( lms.find( t->id() ) != lms.end() )
    {
      for( auto ts : *t )
      {
        ...
        int mask_row = int( std::max< float >(
          0.0f,
          std::min< float >(
            mask_h * float( x.y() ) / fd_it->second.h,
            mask_h - 1 ) ) );
        int mask_col = int( std::max< float >(
          0.0f,
          std::min< float >(
            mask_w * float( x.x() ) / fd_it->second.w,
            mask_w - 1 ) ) );
...
detect_critical_tracks(
  camera_components const& cc,
  feature_track_set_sptr tracks )
{
  ...
  double median_distance_multiple )
{
  ...
  for( auto const& lm_it : lms )
  {
    ...
    {
      if( !bundle_angle_is_at_least(
        observing_cams, lm->loc(),
        triang_cos_ang_thresh ) )
      {
  ...
  if( median_distance_multiple > 0 )
  {
    ...
    if( !depths.empty() )
    {
      ...
      for( auto const& lm_it : lms )
      {
        if( landmarks_to_remove.find( lm_it.first ) !=
            landmarks_to_remove.end() )
        {
          // already removed, no need to process further.
          continue;
        }
        ...
        for( auto ts : *t )
        {
          ...
          const auto cam =
            std::static_pointer_cast< simple_camera_perspective >( ci->second );
          auto d = cam->depth( lm->loc() );
          if( d > depth_thresh )
          {
            landmarks_to_remove.insert( lm_it.first );
  ...
  LOG_DEBUG(
    logger, "num landmarks " << lms.size() << " num unconstrained " <<
  ...
  int min_landmark_inliers )
{
  ...
  while( keep_cleaning )
  {
    ...
    std::set< track_id_t > lm_to_remove =
      detect_bad_landmarks(
        cams, det_lms, tracks, triang_cos_ang_thresh,
        error_tol, min_landmark_inliers );
    ...
    if( !lm_to_remove.empty() )
    {
      ...
      {
        LOG_DEBUG(
          logger, "removing " << lm_to_remove.size() <<
...
bool
camera_upright(
  vital::camera_perspective const& camera,
