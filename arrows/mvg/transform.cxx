...
void
transform_inplace(
  vital::simple_camera_perspective_map& cameras,
  const vital::similarity_d& xform )
{
...
void
transform_inplace(
  vital::landmark_map::map_landmark_t& landmarks,
  const vital::similarity_d& xform )
{
  for( vital::landmark_map::map_landmark_t::value_type& p : landmarks )
  {
    if( vital::landmark_d* vlm_d =
          dynamic_cast< vital::landmark_d* >( p.second.get() ) )
    {
      transform_inplace( *vlm_d, xform );
    }
    else if( vital::landmark_f* vlm_f =
               dynamic_cast< vital::landmark_f* >( p.second.get() ) )
    {
...
template < typename T >
vital::covariance_< 3, T >
transform(
  const vital::covariance_< 3, T >& covar,
  const vital::similarity_< T >& xform )
{
...
transform(
  vital::camera_perspective_sptr cam,
  const vital::similarity_d& xform )
{
...
/// construct a transformed map of cameras by applying a similarity
/// transformation
...
transform(
  vital::camera_perspective_map_sptr cameras,
  const vital::similarity_d& xform )
{
  auto cam_map = cameras->T_cameras();
  for( auto& p : cam_map )
  ...
  const vital::similarity_d& xform )
{
  ...
  lm = lm->clone();
  if( vital::landmark_d* vlm_d =
        dynamic_cast< vital::landmark_d* >( lm.get() ) )
  {
    transform_inplace( *vlm_d, xform );
  }
  else if( vital::landmark_f* vlm_f =
             dynamic_cast< vital::landmark_f* >( lm.get() ) )
  {
    transform_inplace( *vlm_f, vital::similarity_f( xform ) );
  }
  else
...
/// construct a transformed map of landmarks by applying a similarity
/// transformation
