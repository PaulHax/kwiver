...
typedef std::map< frame_id_t,
  simple_camera_perspective_sptr >               map_cam_t;
typedef std::map< frame_id_t,
  simple_camera_perspective_sptr >::iterator cam_map_itr_t;
typedef std::map< frame_id_t,
  simple_camera_perspective_sptr >::const_iterator const_cam_map_itr_t;
...
public:
  ...
  double
  score( int min_matches = 30 ) const
  {
    ...
    double ave_angle = angle_sum /
                       ( double( well_conditioned_landmark_count ) );
    double angle_score = std::max(
      1.0 -
      fabs( ( ave_angle / target_angle ) - 1.0 ), 0.0 );

    // having more than 50 features really doesn't improve things
    double count_score = std::min(
      1.0,
      double( well_conditioned_landmark_count / 50.0 ) );
    ...
    double coverage_score = std::min(
      0.2f,
      std::min( coverage_0, coverage_1 ) );
    return count_score * angle_score * angle_score * coverage_score;
  }

  bool
  operator<( const rel_pose& other ) const
  {
    if( f0 < other.f0 )
    {
      return true;
...
public:
  ...
  rel_pose
  calc_rel_pose(
    frame_id_t frame_0, frame_id_t frame_1,
  ...
  double
  c_feature_angle_threshold() const { return parent.c_feature_angle_threshold; }

  int
  c_max_cams_in_keyframe_init() const
  {
  ...
  double
  c_frac_frames_for_init() const { return parent.c_frac_frames_for_init; }

  double
  c_metadata_init_permissive_triang_thresh() const
  {
    return parent.c_metadata_init_permissive_triang_thresh;
  }

  bool
  c_do_final_sfm_cleaning() const { return parent.c_do_final_sfm_cleaning; }
  bool
  c_force_common_intrinsics() const { return parent.c_force_common_intrinsics; }
  ...
  vital::algo::bundle_adjust_sptr
  d_global_bundle_adjuster() const { return parent.c_global_bundle_adjuster; }
  vital::algo::estimate_canonical_transform_sptr
  d_canonical_estimator() const { return parent.c_canonical_estimator; }
  vital::algo::estimate_similarity_transform_sptr
  d_similarity_estimator() const { return parent.c_similarity_estimator; }
  vital::algo::estimate_pnp_sptr
  d_estimate_pnp() const { return parent.c_estimate_pnp; }
  ...
  double inlier_threshold ) const
{
  ...
  double triang_thresh_orig =
    triang_config->get_value< double >( "inlier_threshold_pixels", 2.0 );
  if( inlier_threshold > 0.0 )
  {
    triang_config->set_value< double >(
  ...
  bool triangulate_only_outliers )
{
  ...
  for( auto fid : frame_ids )
  {
    ...
    {
      auto fts =
        std::static_pointer_cast< feature_track_state >( *( t->find( fid ) ) );
...
public:
  ...
  void
  add_entry( vector_2d loc )
  {
    int cx = static_cast< int >( m_mask.cols() *
                                 ( loc.x() / double( m_input_w ) ) );
    int cy = static_cast< int >( m_mask.rows() *
                                 ( loc.y() / double( m_input_h ) ) );
  ...
  bool
  conditionally_remove_entry( vector_2d loc )
  {
    int cx = static_cast< int >( m_mask.cols() *
                                 ( loc.x() / double( m_input_w ) ) );
    int cy = static_cast< int >( m_mask.rows() *
  ...
  std::set< frame_id_t > down_select_these_frames ) const
{
  ...
  for( auto cam : cams->T_cameras() )
  {
    ...
    int image_w = static_cast< int >( 2 *
                                      cam.second->intrinsics()->principal_point()
                                      .x() );
    int image_h = static_cast< int >( 2 *
                                      cam.second->intrinsics()->principal_point()
                                      .y() );

    auto mask = std::make_shared< gridded_mask >(
      image_w, image_h,
  ...
  while( !lm_to_downsample.empty() )
  {
    ...
    for( int ns = 0; ns < 20; ++ns )
    {
      ...
      int t1_effective_len = std::min< int >(
...
#pragma omp critical
    {
      ...
      d_global_bundle_adjuster()->optimize(
        *cam_map, lms, trk_set,
  ...
  rp.well_conditioned_landmark_count =
    static_cast< int >( inlier_lm_ids.size() );
  rp.angle_sum = 0;
...
#pragma omp parallel for schedule(dynamic, 10)
    for( int64_t i = 0; i < static_cast< int64_t >( pairs_to_process.size() );
         ++i )
    {
...
#pragma omp critical
      {
        if( rp.well_conditioned_landmark_count >
            static_cast< int >( c_min_frame_to_frame_matches() ) )
        {
          m_rel_poses.insert( rp );
        }
      }
  ...
  feature_track_set_sptr tracks )
{
  ...
  if( selected_frame == -1 )
  {
    ...
    for( auto cid : reconstructed_cam_ids )
    {
      if( cid_prev >= 0 )
      {
        max_existing_spacing = std::max< int >(
          max_existing_spacing,
    ...
    for( auto rc : frames_to_resection )
    {
      ...
      std::set_intersection(
        currently_reconstructed_landmarks.begin(),
      ...
      long score = static_cast< long >( intersect_lmks.size() *
                                        closest_frame_diff );
      if( score > best_score )
      {
        best_score = score;
  ...
  vital::algo::estimate_pnp_sptr pnp )
{
  ...
  LOG_DEBUG(
    m_logger, "for frame " << frame << " P3P found " << num_inliers <<
  ...
  {
    LOG_DEBUG(
      m_logger, "resectioning image " << frame <<
  ...
  frame_id_t fid_to_resection )
{
  ...
  three_point_pose(
    fid_to_resection, nc, tracks, lms,
  ...
  int& num_constraints_used )
{
      ...
      d_similarity_estimator() )
  {
    ...
    if( cam_positions.size() > 5 )
    {
      ...
      Eigen::JacobiSVD< vital::matrix_3x3d > svd( cov_mat,
        Eigen::ComputeFullV );
      auto sing_vals = svd.singularValues();
      if( sing_vals( 0 ) < 100 * sing_vals( 1 ) )
      {
        sim = d_similarity_estimator()->estimate_transform(
          cam_positions,
          sensor_positions );
        estimated_sim = true;
  ...
  if( !estimated_sim )
  {
    ...
    {
      LOG_DEBUG(
        m_logger, "No canonial transform estimator defined. "
  ...
  auto cams_above = cameras_above_plane(
    cams->map_of_< camera_perspective >(),
    plane );
  ...
  feature_track_set_sptr tracks )
{
  ...
  {
    bool
    operator()( const rel_pose& a, const rel_pose& b ) const
    {
  ...
  std::vector< rel_pose > rel_poses_inlier_ordered( m_rel_poses.begin(),
    m_rel_poses.end() );
  std::sort(
    rel_poses_inlier_ordered.begin(),
  ...
  for( auto& rp_init : rel_poses_inlier_ordered )
  {
    ...
    LOG_DEBUG(
      m_logger,
      "Base focal length: " << m_base_camera.intrinsics()->focal_length() );
    if( c_force_common_intrinsics() )
    {
    ...
    LOG_DEBUG(
      m_logger, "rp_init.well_conditioned_landmark_count "
    ...
    {
      LOG_INFO(
        m_logger, "Running Global Bundle Adjustment on "
      ...
      if( inlier_lm_ids.size() > 5 )
      {
        ...
        if( inlier_lm_ids.size() > 5 )
        {
          d_bundle_adjuster()->optimize(
            *rev_cams, rev_lms, tracks,
            fixed_cams_empty, fixed_lms_empty );
          rev_optimized_rmse = reprojection_rmse(
            rev_cams->cameras(),
            rev_lms, trks );
        }
      }
      ...
      LOG_DEBUG(
        m_logger, "optimized reprojection RMSE: " << optimized_rmse
                                                  << " focal len: "
                                                  << cams->find(
          rp_init.f0 )->get_intrinsics()->focal_length() );
      LOG_DEBUG(
        m_logger, "optimized reversed reprojection RMSE: " << rev_optimized_rmse
                                                           << " focal len: "
                                                           << rev_cams->find(
          rp_init.f0 )->get_intrinsics()->focal_length() );
      ...
      LOG_DEBUG(
        m_logger, "after ba rp_init.well_conditioned_landmark_count "
          << rp_init.well_conditioned_landmark_count
          << " inlier_lm_ids size " << inlier_lm_ids.size() );
      ...
      double thresh_triang_cos_ang = std::cos(
        deg_to_rad *
        c_feature_angle_threshold() );
      clean_cameras_and_landmarks(
        *cams, lms, tracks, thresh_triang_cos_ang,
    ...
    {
      LOG_DEBUG(
        m_logger, "initialization pair is  "
...
public:
  ...
  void
  add_set( const std::set< T >& in_set )
  {
  ...
  bool
  remove_set( const std::set< T >& rem_set )
  {
  ...
  bool
  contains_set( const std::set< T >& test_set )
  {
  ...
  bool
  contains_set( const std::set< T >& test_set, size_t hash )
  {
    auto it_pair = m_map.equal_range( hash );
    for( auto it = it_pair.first; it != it_pair.second; ++it )
    {
...
private:
  size_t
  hash_set( const std::set< T >& to_hash )
  {
    size_t sum = 0;
  ...
  VITAL_UNUSED frame_id_t target_frame )
{
  ...
  if( landmarks_lost_without_target_frame <
      0.1 * landmarks_involving_target_frame )
  {
  ...
  callback_t callback )
{
  ...
  LOG_DEBUG(
    m_logger,
    "beginning_keyframes size " << beginning_keyframes.size() );
  ...
  if( !initialize_reconstruction( cams, lms, tracks ) )
  {
    LOG_DEBUG(
      m_logger,
      "unable to find a good initial pair for reconstruction" );
    return false;
  }

  int constraints_used;
  if( fit_reconstruction_to_constraints(
    cams, lms, tracks,
    constraints, constraints_used ) )
  {
    ...
    LOG_INFO(
      m_logger, "Running Global Bundle Adjustment on "
        ...
        << lms.size() << " landmarks" );
    d_bundle_adjuster()->optimize(
      *cams, lms, tracks, fixed_cams_empty,
      fixed_lms_empty );
  }
  ...
  while( this->continue_processing &&
         ...
         ( c_max_cams_in_keyframe_init() < 0 ||
           cams->size() <
           static_cast< size_t >( c_max_cams_in_keyframe_init() ) ) )
  {
    frame_id_t next_frame_id = select_next_camera(
      frames_to_resection,
      cams, lms, tracks );
    if( next_frame_id == -1 )
    {
...

    {
      ...
      LOG_DEBUG(
        m_logger, "before new camera reprojection RMSE: "
          << before_new_cam_rmse);
      ...
      LOG_DEBUG(
        m_logger, "Optimizing frame " << next_frame_id
                                      << cams->size() - 1 <<
          " fixed cameras and "
                                      << lms.size() << " landmarks" );

      auto ba_config = d_bundle_adjuster()->get_configuration();
      bool opt_focal_was_set =
        ba_config->get_value< bool >( "optimize_focal_length" );
      ...
      LOG_DEBUG(
        m_logger, "after new camera reprojection RMSE: "
          << after_new_cam_rmse);
    }
    ...
    int next_ba_cam_count = std::max< int >(
        ...
        frames_to_resection.empty() || cams->size() < 30 )
    {
      ...
      // bundle adjust result because number of inliers has changed
      // significantly
      if( d_bundle_adjuster() )
      {
        double before_clean_rmse =
          reprojection_rmse( cams->cameras(), lms, trks );
        LOG_DEBUG(
          m_logger, "before clean reprojection RMSE: "
            << before_clean_rmse);
        ...
        // now an overall ba
        LOG_INFO(
          m_logger, "Running optimization on "
        ...
        double optimized_rmse =
          reprojection_rmse( cams->cameras(), lms, trks );
        LOG_DEBUG(
          m_logger, "optimized reprojection RMSE: "
            << optimized_rmse);
            ...
            !m_solution_was_fit_to_constraints )
        {
          ...
          auto nr_cams_perspec =
            std::make_shared< simple_camera_perspective_map >(
              cams->T_cameras() );
          auto nr_cams =
            std::static_pointer_cast< camera_map >( nr_cams_perspec );

          landmark_map_sptr ba_lms2( new simple_landmark_map( lms ) );
          necker_reverse( nr_cams, ba_lms2, false );
          // set from base is required because necker_reverse returns new
          // cameras
          ...
          fit_reconstruction_to_constraints(
            nr_cams_perspec, nr_landmarks,
            tracks, ba_constraints,
            rev_num_constraints_used );

          init_rmse = reprojection_rmse(
            nr_cams_perspec->cameras(),
            nr_landmarks, trks );
          LOG_DEBUG(
            m_logger, "Necker reversed initial reprojection RMSE: "
              << init_rmse);
          if( init_rmse < optimized_rmse * m_reverse_ba_error_ratio )
          {
            landmark_map_sptr nr_landmark_map( new simple_landmark_map(
              nr_landmarks ) );
            ...
            double before_final_ba_rmse2 =
              reprojection_rmse(
                ...
                trks );
            LOG_DEBUG(
              m_logger, "Necker reversed before final reprojection RMSE: "
                << before_final_ba_rmse2);

            d_global_bundle_adjuster()->optimize(
              *nr_cams_perspec, nr_landmarks, tracks,
              fixed_cameras, fixed_landmarks,
              ba_constraints );
            ...
            double final_rmse2 =
              reprojection_rmse(
                ...
                nr_landmarks, trks );
            LOG_DEBUG(
              m_logger, "Necker reversed final reprojection RMSE: "
                << final_rmse2);
  ...
  {
    LOG_INFO(
      m_logger, "Running Final Bundle Adjustment of initial keyframes "
  ...
  callback_t callback )
{
  ...
  do
  {
    ...
    if( cc.size() > 1 )
    {
      ...
      LOG_DEBUG(
        m_logger, "Found " << cc.size() << " connected components "
  ...
  callback_t callback )
{
  ...
  if( metadata_centric_keyframe_initialization(
    cams, false, landmarks, tracks,
  ...
  if( vision_centric_keyframe_initialization(
    cams, landmarks, tracks,
    constraints, keyframes,
    callback ) )
  {
  ...
  double reporj_thresh )
{
  ...
  for( auto& t : cur_tracks )
  {
    ...
    double err = reprojection_error_sqr(
      *vital_cam,
  ...
  sfm_constraints_sptr constraints )
{
  ...
  for( auto& cur_cam_pair : spc )
  {
    ...
    if( prev_cam )
    {
      ...
      nr_cam->insert(
        fid, std::static_pointer_cast< simple_camera_perspective >(
          cur_cam->clone() ) );
    ...
    // make sure we get the camera from cams in case it was changed by a
    // reversal
  ...
  frame_id_t& fid_to_register, frame_id_t& closest_frame ) const
{
  ...
  for( auto f : frames_to_register )
  {
    for( auto& ec : existing_cams )
    {
      ...
      {
        min_diff_cams.clear();
        min_diff_cams.push_back(
          std::pair< frame_id_t, frame_id_t >(
            f,
            ec.first ) );
        min_frame_diff = diff;
      }
  ...
  std::set< frame_id_t >& already_registred_cams )
{
  ...
  for( int lp = 0; lp < 2; ++lp )
  {
    ...
    int loop_count = 0;
    bundled_inlier_count = set_inlier_flags(
      fid_to_register, bundled_cam,
    ...
    while( bundled_inlier_count > prev_bundled_inlier_count )
    {
      // DOES THIS LOOP HELP?
      d_bundle_adjuster()->optimize(
        *cams, cur_landmarks_rand_sub, tracks,
      ...
      if( resectioned_cam )
      {
        ...
        while( resection_inlier_count > prev_resection_inlier_count )
        {
          ...
          resection_inlier_count =
            set_inlier_flags(
              fid_to_register, resectioned_cam,
              cur_landmarks, tracks, 50 );
          ++loop_count;
        }
        if( loop_count > 2 )
  ...
  {
    LOG_DEBUG(
      m_logger, "using resectioned camera for frame "
        << fid_to_register
  ...
  const std::set< frame_id_t >& frames_since_last_local_ba )
{
  ...
  LOG_DEBUG(
    m_logger, "Bundle adjusting " << cams->size() << " cameras ("
                                  << frames_to_fix.size() << " fixed) and "
                                  << variable_landmarks.size() <<
      " landmarks" );
  d_bundle_adjuster()->optimize(
    *cams, variable_landmarks, tracks,
  ...
  feature_track_set_sptr& tracks )
{
  ...
  {
    LOG_WARN(
      m_logger, "beginning num landmarks without tracks "
        << num_landmarks_without_tracks);
  }
  ...
  for( auto& lm : lmks )
  {
    ...
    // should clear m_already_marged_landmarks so that we try all the landmarks
    // again.
    ...
    for( auto nb_lm_id : nearby_landmarks )
    {
      ...
      {
        // only consider points actualy with one volume unit sphere of each
        // other
        continue;
      }
      ...
      if( lm.first < nb_lm_id )
      {
        if( tracks->merge_tracks( nb_tk_sptr, cur_tk ) )
        {
          ...
          {
            auto lm_f_sptr =
              std::dynamic_pointer_cast< landmark_f >( lm.second );
            if( lm_f_sptr )
            {
      ...
      else
      {
        ...
        {
          auto lm_d_sptr =
            std::dynamic_pointer_cast< landmark_d >( nb_lm_it->second );
          if( lm_d_sptr )
          {
            lm_d_sptr->set_loc( lm_merged.loc() );
          }
          ...
          {
            auto lm_f_sptr =
              std::dynamic_pointer_cast< landmark_f >( nb_lm_it->second );
            if( lm_f_sptr )
            {
              lm_f_sptr->set_loc( lm_merged.loc().cast< float >() );
            }
  ...
  {
    LOG_WARN(
      m_logger, "end num landmarks without tracks "
        << num_landmarks_without_tracks);
  }
}
  ...
  callback_t callback )
{
  ...
  while( this->continue_processing )
  {
    if( frames_to_register.empty() )
    {
      ...
      else
      {
        ...
        if( keyframes_to_register.size() < num_frames_to_register )
        {
          ...
          num_frames_to_register = frames_to_register.size();
          LOG_INFO(
            m_logger, "Finished processing key frames, "
    ...
    if( !initialize_next_camera(
      cams, lmks, tracks, nullptr,
    ...
    if( lmks.size() > 1.4 * lmks_last_down_select ||
        frames_to_register.empty() )
    {
      // merging landmarks is very slow with limited benefit, so disable for
      // now.
    ...
    auto reporj_by_cam = reprojection_rmse_by_cam(
      cams->cameras(),
      lmks, tracks->tracks() );
        ...
        frames_since_last_ba > 50 )
    {
      ...
      last_reproj_by_cam = reprojection_rmse_by_cam(
        cams->cameras(),
        lmks, tracks->tracks() );
    }
    ...
    {
      LOG_DEBUG(
        m_logger, "continue processing is false, "
  ...
  sfm_constraints_sptr constraints ) const
{
  ...
  if( !m_priv->initialize_keyframes(
    cams, landmarks, tracks,
  ...
  if( cams->size() == 0 ||
      !m_priv->initialize_remaining_cameras(
        cams, landmarks, tracks,
        constraints, this->m_callback ) )
  {
    return;
