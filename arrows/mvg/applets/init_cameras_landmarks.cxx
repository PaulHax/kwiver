...
public:
  ...
  default_config()
  {
    ...
    auto config =
      kvt::find_configuration( "applets/init_cameras_landmarks.conf" );
...
config->get_value< type >( bc + #name, K_def.name() )
  ...
  run_algorithm()
  {
    ...
    algorithm->initialize(
      camera_map_ptr, landmark_map_ptr,
...
::add_command_options()
{
  ...
  // If we want to remove tracks reading from the config, we should then add
  // this
