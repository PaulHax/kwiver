...
config_valid = false
  ...
  config_valid =
    validate_required_output_file(
      "output_tracks_file", *config,
      main_logger ) &&
    config_valid;

  config_valid =
    validate_optional_output_file(
      "output_homography_file", *config,
      main_logger ) &&
    config_valid;
...
select_frames(
  std::vector< kv::frame_id_t > const& valid_frames,
...
// ------------------------------------------------------------------
  ...
  default_config()
  {
    ...
    config->set_value(
      "output_homography_file", homography_file,
  ...
  run_algorithm()
  {
    ...
    auto const use_video_metadata =
      config->get_value< bool >( "use_video_metadata" );
    ...
    {
      LOG_ERROR(
        main_logger, "Could not open track file for writing: \""
          << track_file << "\"" );
      return false;
    }
    ofs.close();
    ...
    {
      kv::path_t homog_fp =
        config->get_value< kv::path_t >( "output_homography_file" );
      homog_ofs.open( homog_fp.c_str() );
      if( !homog_ofs )
      {
        LOG_ERROR(
          main_logger, "Could not open homography file for writing: "
            << homog_fp);
        return false;
      }
    }
    ...
    for( auto target_frame : selected_frames )
    {
      ...
      if( tracks )
      {
        tracks = kwiver::arrows::core::extract_feature_colors(
          tracks, *image,
          frame );
      }

      // Compute ref homography for current frame with current track set + write
      // to file
      // -> still doesn't take into account a full shotbreak, which would incur
      // a
      // track reset
      if( homog_ofs.is_open() )
      {
        LOG_DEBUG(main_logger, "writing homography" );
        homog_ofs << *( out_homog_generator->estimate(
          frame,
          tracks ) ) << std::endl;
      }
    }
