...
std::vector< double >
reprojection_errors(
  const std::map< frame_id_t, camera_sptr >& cameras,
  ...
  const std::vector< track_sptr >& tracks )
{
  ...
  for( auto& err : cam_errors )
  {
    ...
    {
      ret_errs[ err.first ] = std::sqrt(
        err.second.sum_error_sq /
        static_cast< double >( err.second.num_obs ) );
    }
  }
  return ret_errs;
}
...
reprojection_median_error(
  ...
  const std::vector< track_sptr >& tracks )
{
  std::vector< double > errors = reprojection_errors(
    cameras, landmarks,
    tracks );
  std::nth_element(
    errors.begin(),
