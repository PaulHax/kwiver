...
// ----------------------------------------------------------------------------
void
check_equal( image_memory mem_1, image_memory mem_2 )
{
...
template < typename T >
void
check_equal( image_of< T > const& img_1, image const& img_2 )
{
...
TEST ( image, copy_constructor )
{
  ...
  EXPECT_THROW(
    image_of< float > img_bad_copy{ img_copy },
...
TEST ( image, is_contiguous )
{
  ...
  EXPECT_FALSE(
    ( image{ mem, mem->data(), w, h, d, 1, 2 * w,
             2 * w * h }.is_contiguous() ) )
...
void
image_transform
::check_image_init( image_of< byte > const& img )
{
...
TYPED_TEST ( get_image, crop )
{
  ...
  image_container_sptr img_cont =
    std::make_shared< simple_image_container >( img );
