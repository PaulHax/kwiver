...
TEST ( camera_perspective, clone )
{
  ...
  auto cam_clone =
    std::dynamic_pointer_cast< camera_perspective >( cam.clone() );
...
TEST ( camera_perspective, clone_look_at )
{
  ...
  auto cam_in = simple_camera_perspective{ vector_3d{ 3, -4, 7 }, rotation_d{},
                                           K };
...
TEST ( camera_perspective, projection )
{
  ...
  auto test = [ & ]( vector_3d const& test_pt ){
                vector_4d test_hpt{ test_pt.x(), test_pt.y(), test_pt.z(),
                                    1.0 };
