...
/// \file
/// \brief This file contains the implementation of a local geographical offset
/// coordinate system.
...
///
/// This class is a cleaned up version of the LocalCartesian and Geocentric
/// classes provided in GeoTrans
/// This allows a user to define a local cartesian coordinate system with any
/// origin (expressed in WGS84)
...
public:
  ...
  ///
  /// Set local origin parameters as inputs and sets the corresponding state
  /// variables.
  /// NOTE : If the origin changes, this method needs to be called again to
  /// recompute
  ///        variables needed in the conversion math.
  ///
  /// @param [in] geo_point       : Geograpical origin of the cartesian system
  /// @param [in] orientation     : Orientation angle of the local cartesian
  /// coordinate system,
  ///                               in radians along the Z axis, normal to the
  /// earth surface
  void
  set_origin( geo_point const& origin, double orientation )
  {
    if( origin.is_empty() )
    {
    ...
    N0 = semiMajorAxis /
         sqrt(
      1 - Geocent_e2 * Sin_LocalCart_Origin_Lat *
    ...
    w0 = ( ( N0 * ( 1 - Geocent_e2 ) ) + LocalCart_Origin_Height ) *
         Sin_LocalCart_Origin_Lat;
  }
  ...
  /// (latitude, longitude, and height) to local cartesian coordinates (X, Y,
  /// Z),
  /// according to the WGS84 ellipsoid and local origin parameters.
  ///
  /// @param [in]     geodetic_coordinate : WGS84 longitude/latitude in degrees
  /// and hight in meters
  /// @param [in,out] cartesian_coordinate : The location in the local
  /// coordinate system, in meters
  void
  convert_from_geodetic(
    vector_3d const& geodetic_coordinate,
    vector_3d& cartesian_coordinate ) const
  {
  ...
  ///
  /// @param [in]     cartesian_coordinate : The location in the local
  /// coordinate system, in meters
  /// @param [in,out] geodetic_coordinate  : WGS84 longitude/latitude in degrees
  /// and hight, in meters
  void
  convert_to_geodetic(
    vector_3d const& cartesian_coordinate,
    vector_3d& geodetic_coordinate ) const
  {
    vector_3d geocentric_coordinate;
    ...
    height = rho * c + Z * s - equatorial_radius *
             sqrt( 1.0 - eccentricity_squared * s * s );

    geodetic_coordinate << ( longitude * rad_to_deg ),
      ( latitude * rad_to_deg ), height;
  }
  ...
  /// @param [in,out] cartesian_coordinate  : Calculated local cartesian
  /// coordinate, in meters
  void
  convert_from_geocentric(
    vector_3d const& geocentric_coordinate,
    vector_3d& cartesian_coordinate ) const
  {
    double X, Y, Z;
    ...
    if( LocalCart_Orientation == 0.0 )
    {
      ...
      X = -Sin_LocalCart_Origin_Lon * u_MINUS_u0 + Cos_LocalCart_Origin_Lon *
          v_MINUS_v0;
    ...
    else
    {
      ...
      X = ( -Cos_LocalCart_Orientation * Sin_LocalCart_Origin_Lon +
            Sin_Lat_Sin_Orient * Cos_LocalCart_Origin_Lon ) * u_MINUS_u0 +
          ( Cos_LocalCart_Orientation * Cos_LocalCart_Origin_Lon +
            Sin_Lat_Sin_Orient * Sin_LocalCart_Origin_Lon ) * v_MINUS_v0 +
          ( -Sin_LocalCart_Orientation * cos_lat_w_MINUS_w0 );

      Y = ( -Sin_LocalCart_Orientation * Sin_LocalCart_Origin_Lon -
            Sin_Lat_Cos_Orient * Cos_LocalCart_Origin_Lon ) * u_MINUS_u0 +
          ( Sin_LocalCart_Orientation * Cos_LocalCart_Origin_Lon -
            Sin_Lat_Cos_Orient * Sin_LocalCart_Origin_Lon ) * v_MINUS_v0 +
  ...
  ///
  /// @param [in,out] cartesian_coordinate  : Local cartesian coordinate, in
  /// meters
  /// @param [in]     geocentric_coordinate : The geocentric location, in meters
  void
  convert_to_geocentric(
    vector_3d const& cartesian_coordinate,
    vector_3d& geocentric_coordinate ) const
  {
    double U, V, W;
    ...
    else
    {
      ...
      rotated_y = -Sin_LocalCart_Orientation * X + Cos_LocalCart_Orientation *
                  Y;

      rotated_y_sin_lat = rotated_y * Sin_LocalCart_Origin_Lat;
      z_cos_lat = Z * Cos_LocalCart_Origin_Lat;

      U = -Sin_LocalCart_Origin_Lon * rotated_x - Cos_LocalCart_Origin_Lon *
          rotated_y_sin_lat + Cos_LocalCart_Origin_Lon * z_cos_lat + u0;
      V = Cos_LocalCart_Origin_Lon * rotated_x - Sin_LocalCart_Origin_Lon *
          rotated_y_sin_lat + Sin_LocalCart_Origin_Lon * z_cos_lat + v0;
...
// All are set to a WGS 84 ellipsoid
  ...
  double es2;                       // Eccentricity (0.08181919084262188000)
                                    // squared
...
local_cartesian
::local_cartesian( geo_point const& origin, double orientation )
{
  origin_ = origin;
...
::convert_to_cartesian(
  ...
  vector_3d& cartesian_coordinate ) const
{
  geotrans_->convert_from_geodetic(
    location.location(
      kwiver::vital::SRID::lat_lon_WGS84 ), cartesian_coordinate );
}
