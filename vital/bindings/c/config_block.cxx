  ...
  vital_error_handle_t* eh )
{
  STANDARD_CATCH(
    ...
    return kwiver::vital_c::CONFIG_BLOCK_SPTR_CACHE.get( cb )->is_read_only(
      key );
  );
  return false;
}
  ...
  vital_error_handle_t*  eh )
{
  STANDARD_CATCH(
    ...
    if( ( length == 0 ) || ( keys == 0 ) )
  {
    throw kwiver::vital::invalid_value(
      "One or both provided output parameters "
      "were a NULL pointer." );
  }
  ...
  vital_error_handle_t* eh )
{
  STANDARD_CATCH(
    ...
    return read_config_file_helper(
      eh,
      filepath,
  ...
  vital_error_handle_t*  eh )
{
  STANDARD_CATCH(
    ...
    try
  {
    kwiver::vital::config_block_sptr c =
