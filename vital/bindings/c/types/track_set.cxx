...
vital_trackset_from_sptr( void* sptr )
{
  STANDARD_CATCH(
    ...
    kwiver::vital::track_set_sptr ts_sptr =
      *reinterpret_cast< kwiver::vital::track_set_sptr* >( sptr );
    kwiver::vital_c::TRACK_SET_SPTR_CACHE.store( ts_sptr );
    return reinterpret_cast< vital_trackset_t* >( ts_sptr.get() );
  );
  return 0;
...
size_t
vital_trackset_size(
  vital_trackset_t const* track_set,
  vital_error_handle_t* eh )
{
  STANDARD_CATCH(
  ...
  vital_error_handle_t* eh )
{
    ...
    for( size_t i = 0; i < tracks_v.size(); ++i )
  {
    ...
    track_array[ i ] =
      reinterpret_cast< vital_track_t* >( tracks_v[ i ].get() );
  }
    return track_array;
  );
  return 0;
...
vital_trackset_all_frame_ids(
  vital_trackset_t const* trackset, size_t* length,
  vital_error_handle_t* eh )
{
  STANDARD_CATCH(
...
vital_trackset_first_frame(
  vital_trackset_t const* trackset,
  vital_error_handle_t* eh )
{
  STANDARD_CATCH(
...
vital_trackset_get_track(
  vital_trackset_t const* trackset, int64_t tid,
  vital_error_handle_t* eh )
{
  STANDARD_CATCH(
