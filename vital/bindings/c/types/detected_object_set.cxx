...
SharedPointerCache< kwiver::vital::detected_object_set,
 ...
 */
vital_detected_object_set_t*
vital_detected_object_set_from_sptr(
  kwiver::vital::detected_object_set_sptr sptr )
{
  STANDARD_CATCH(
...
vital_detected_object_set_t*
vital_detected_object_set_from_c_pointer(
  kwiver::vital::detected_object_set* ptr )
{
  STANDARD_CATCH(
...
vital_detected_object_set_to_sptr( vital_detected_object_set_t* handle )
{
  STANDARD_CATCH(
...
// ------------------------------------------------------------------
vital_detected_object_set_t*
vital_detected_object_set_new_from_list(
  ...
  size_t n )
{
    ...
    for( size_t i = 0; i < n; ++i )
  {
    ...
    auto obj_set =
      std::make_shared< kwiver::vital::detected_object_set >( input );
...
// ------------------------------------------------------------------
void
vital_detected_object_set_destroy( vital_detected_object_set_t* obj )
{
  STANDARD_CATCH(
  ...
  vital_detected_object_t* obj )
{
  STANDARD_CATCH(
    ...
    kwiver::vital_c::DOBJ_SET_SPTR_CACHE.get( set )->add(
...
// ------------------------------------------------------------------
size_t
vital_detected_object_set_size( vital_detected_object_set_t* obj )
{
  STANDARD_CATCH(
...
// ------------------------------------------------------------------
void
vital_detected_object_set_select_threshold(
  ...
  size_t* length )
{
  STANDARD_CATCH(
    ...
    auto sel_set =
      kwiver::vital_c::DOBJ_SET_SPTR_CACHE.get( obj )->select( thresh );

    // select to get vector
    *output =
      ( vital_detected_object_t** ) malloc(
        sizeof( vital_detected_object_t* ) *
        sel_set->size() );
    *length = sel_set->size();
    ...
    for( auto ix = sel_set->cbegin(); ix != ie; ++ix )
  {
    ...
    ( *output )[ i ] =
      reinterpret_cast< vital_detected_object_t* >( ( *ix ).get() );
  } );
}

// ------------------------------------------------------------------
void
vital_detected_object_set_select_class_threshold(
  vital_detected_object_set_t* obj,
  ...
  size_t* length )
{
  STANDARD_CATCH(
    ...
    auto sel_set =
      kwiver::vital_c::DOBJ_SET_SPTR_CACHE.get( obj )->select(
        std::string(
          class_name ), thresh );
