...
struct VIBRANT_DESCRIPTORS_EXPORT descriptor_overlap_type
{
  ...
  unsigned src_activity_id;        // activity associated with source track (or
                                   // "NotScored" if none)
  unsigned dst_activity_id;        // activity associated with dst track (or
                                   // "NotScored")
  ...
  bool radial_overlap_flag;        // true: radial overlap (no boxes); false:
                                   // spatial overlap (boxes)
  double mean_percentage_overlap;  // mean of overlap_area / box_union_area
                                   // (only if ! radial_overlap_flag)
  ...
  {}
  bool
  operator==( const descriptor_overlap_type& rhs ) const
  {
    return
...
VIBRANT_DESCRIPTORS_EXPORT std::ostream& operator<<(
  std::ostream& os,
  const descriptor_overlap_type& d );
