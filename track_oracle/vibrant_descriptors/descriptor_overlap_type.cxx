...
operator<<( ostream& os, const descriptor_overlap_type& d )
{
  ...
  string src_activity_name = ( probe ==
                               i2a.end() ) ? "BAD_INDEX" : probe->second;
  probe = i2a.find( d.dst_activity_id );

  string dst_activity_name = ( probe ==
                               i2a.end() ) ? "BAD_INDEX" : probe->second;
  ...
  os << "<descriptor type=\"overlap\">\n"
    ...
    "\"/>\n"
     << "  <radial_overlap_flag value =\"" <<
    static_cast< unsigned >( d.radial_overlap_flag ) << "\"/>\n"
