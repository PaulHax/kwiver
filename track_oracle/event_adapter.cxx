  ...
  const dt::tracking::timestamp_usecs::Type* ts ) const
{
  if( !this->valid )
  {
    LOG_ERROR(
      main_logger,
      "Logic error: timepoint_in_window on an invalid data block" );
  ...
  if( !( this->has_fn || this->has_ts ) )
  {
    LOG_ERROR(
      main_logger,
      "Logic error: event data block marked valid but has no timestamp / frame number?" );
    return make_pair( false, false );
  }
  if( !( fn || ts ) )
  {
    LOG_ERROR(
      main_logger,
      "Logic error: timepoint_in_window called with null time data" );
    return make_pair( false, false );
  }
  ...
  if( this->has_fn && fn )
  {
    ...
    if( ret.first && ( ret.second != fn_check ) )
    {
      LOG_WARN(
        main_logger,
        "event_adapter: timepoint in window: timestamp check for " << *ts
                                                                   <<
  ...
  logging_map_type& msgs )
{
  ...
  if( awk.NF() < 14 )
  {
    LOG_ERROR(
      main_logger,
      "Expected at least 14 tokens; found in " << awk.NF() << " in '" << line <<
        "'" );
    return false;
  }

  stringstream ss( string( awk[ 2 ] ) + " " + awk[ 13 ] + " " + awk[ 4 ] + " " +
    awk[ 6 ] + " " +
  ...
  if( ( b.start_ts > 0 ) &&
      ( log( static_cast< double >( b.start_ts ) ) < 28.0 ) )
  {
    LOG_WARN(
      main_logger,
      "Warning: KWE start timestamp '" << b.start_ts <<
        "' may not be in usecs" );
  }
  if( ( b.end_ts > 0 ) && ( log( static_cast< double >( b.end_ts ) ) < 28.0 ) )
  {
    LOG_WARN(
      main_logger,
      "Warning: KWE end timestamp '" << b.end_ts << "' may not be in usecs" );
  }

  vidtk::event_types::enum_types kwe_event_type =
    static_cast< vidtk::event_types::enum_types >( vidtk_event_type );
  string virat_act_name =
    aries_interface::kwe_index_to_activity( kwe_event_type );
  if( virat_act_name == "" )
  {
    ostringstream oss;
  ...
  logging_map_type& msgs )
{
  ...
  // only set the track style if it's specified
  {
    ...
    if( b.track_style != "" )
    {
      ...
      msgs.add_msg(
        "event_adapter: setting track style to '" + b.track_style +
        "'" );
    }
    else
    {
      pair< bool,
        string > track_style_probe = track_style_field.get( new_track.row );
      if( track_style_probe.first )
      {
        msgs.add_msg(
          "event_adapter: no track style specified; using style '" +
          track_style_probe.second + "' from source" );
      }
      else
      {
        msgs.add_msg(
          "event_adapter: no track style specified; none found in source" );
      }
    }
  }
}
  ...
  const event_data_block& b )
{
  ...
  for( size_t i = 0; i < frames.size(); ++i )
  {
    ...
    pair< bool,
      dt::tracking::frame_number::Type > fn_probe = fn_field.get( f.row );
    pair< bool,
      dt::tracking::timestamp_usecs::Type > ts_probe = ts_field.get( f.row );
  ...
  if( b.debug )
  {
    ...
    LOG_DEBUG(
      main_logger, "Created track of activity " << b.event_type << " prob " <<
