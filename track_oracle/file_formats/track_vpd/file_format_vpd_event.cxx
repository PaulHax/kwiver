...
{
  unsigned event_id, event_type, duration, start_frame, end_frame,
    current_frame;
...
{
  unsigned event_id, event_type, event_duration, start_frame, end_frame,
    n_objects;
  ...
  {
    istringstream iss( s );
    if( !( iss >> event_id >> event_type >> event_duration >> start_frame >>
           end_frame >> n_objects ) )
    {
      LOG_ERROR(
        main_logger,
        "VPD_EVENT: Mapping parser: couldn't parse preamble in '" << s << "'" );
      return false;
    }
    ...
    {
      LOG_WARN(
        main_logger, "VPD_EVENT: Mapping parser: line '" << s << "' specifies "
                                                         << n_objects <<
          " objects but has states for " << object_id_list.size() );
    }
...
mapping_fn_from_event_fn( const string& s )
{
  ...
  if( p == string::npos )
  {
    // don't need to log an error here; open_event_and_mapping_streams handles
    // it
    return "";
  }
...
bool
open_event_and_mapping_streams(
  const string& event_fn,
  ifstream& event_is,
  ifstream& mapping_is,
  string& errors )
{
...
::inspect_file( const string& event_fn ) const
{
  ...
  string errors;
  if( !open_event_and_mapping_streams(
    event_fn, event_is, mapping_is,
    errors ) )
  {
    return false;
  }
  ...
  track_handle_list_type& events ) const
{
  ...
  while( get_next_nonblank_line( mapping_is, tmp ) )
  {
    ...
    if( p.parse( tmp ) )
    {
      if( mappings.find( p.event_id ) != mappings.end() )
      {
        LOG_ERROR(
          main_logger,
          "VPD_EVENT: mapping for '" << event_fn << "': duplicate event ID " <<
            p.event_id );
        return false;
      }
    ...
    else
    {
      LOG_ERROR(
        main_logger,
        "VPD_EVENT: Couldn't parse mapping line '" << tmp << "'" );
      return false;
    }
  }
  ...
  while( get_next_nonblank_line( event_is, tmp ) )
  {
    if( !event_p.parse( tmp ) )
    {
      LOG_ERROR(
        main_logger,
        "VPD_EVENT: file '" << event_fn << "': couldn't parse '" << tmp <<
          "'" );
      return false;
    }
    ...
    if( new_event )
    {
      ...
      map< unsigned,
        vpd_mapping_line_parser >::const_iterator m =
        mappings.find( event_p.event_id );
      if( m == mappings.end() )
      {
        LOG_ERROR(
          main_logger,
          "VPD_EVENT: Event file '" << event_fn << "': event " <<
            event_p.event_id << " not in mapping file" );
        return false;
      }

      // verify event, start_frame, end_frame
      string mismatch_fields = "";
      if( m->second.event_type != event_p.event_type )
      {
        mismatch_fields += " event_type ";
      }
      if( m->second.start_frame != event_p.start_frame )
      {
        mismatch_fields += " start_frame ";
      }
      if( m->second.end_frame != event_p.end_frame )
      {
        mismatch_fields += " end_frame ";
      }
      if( !mismatch_fields.empty() )
      {
        LOG_ERROR(
          main_logger,
          "VPD_EVENT: Event file '" << event_fn <<
            "': event vs. mapping mismatch in fields " << mismatch_fields );
        return false;
      }
    ...
    vpd[ frame ].bounding_box() =
      vgl_box_2d< double >(
        event_p.box_lefttop_x, event_p.box_lefttop_x + event_p.box_width,
