...
/// **** 4) add it to the formats[] map in file_format_manager_impl's
/// constructor
/// ****
...
///   file_format_schema_type::source_id_to_filename( ffs( the_handle
/// ).source_file_id() );
/// [...]
///
///
/// 4) You know you're loading an XGTF file, and want to set the
/// promote-to-PVMoving
...
typedef std::map< file_format_enum,
  file_format_base* >::const_iterator format_map_cit;
...
public:
  ...
  static std::map< file_format_enum,
    std::vector< std::string > > get_all_globs();
  ...
  // use that format regardless of filename; if it is TF_INVALID_TYPE, deduce
  // the
  // format from the filename (failing unless we get exactly one matching
  // format.)
  static bool write(
    const std::string& fn,
  ...
  static std::vector< file_format_enum > format_contains_element(
    const element_descriptor& e );

  // format introspection: what formats match this schema?
  static std::vector< file_format_enum > format_matches_schema(
    const track_base_impl& s );

  // clone (dynamically allocate) a field from an existing format matching the
  // element descriptor
  // (return null if no match.)  Use to construct new schemata by slicing and
  // dicing
  // existing formats.
