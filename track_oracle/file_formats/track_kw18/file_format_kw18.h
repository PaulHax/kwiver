...
{
  bool kw19_hack;  // if true, set per-frame "relevancy" from the 19th column of
                   // a kw18

  kw18_reader_opts&
  set_kw19_hack( bool b )
  {
    this->kw19_hack = b;
    return *this;
  }
...
public:
  ...
  virtual int
  supported_operations() const { return FF_READ | FF_WRITE; }

  // return a dynamically-allocated instance of the schema
  virtual track_base_impl*
  schema_instance() const { return new track_kw18_type(); }
...
private:
  ...
  bool internal_stream_read(
    std::istream& is, size_t file_size,
    track_handle_list_type& tracks ) const;
