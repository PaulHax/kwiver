...
struct kw18_line_parser
{
  ...
  explicit kw18_line_parser(
    const kwiver::track_oracle::kw18_reader_opts& opts )
  {
...
logged_get_field(
  ::kwiver::track_oracle::track_field< T >& tf,
  ::kwiver::track_oracle::oracle_entry_handle_type row,
  map< string, size_t >& warnings )
{
  pair< bool,
    typename ::kwiver::track_oracle::track_field< T >::Type > probe =
    tf.get( row );
  if( !probe.first )
  {
...
string
logged_output(
  const ::kwiver::track_oracle::track_field< T >& tf,
  ::kwiver::track_oracle::oracle_entry_handle_type row,
  map< string, size_t >& warnings )
{
...
{
  const kw18_reader_opts* rhs =
    dynamic_cast< const kw18_reader_opts* >( &rhs_base );
  if( rhs )
  {
  ...
  else
  {
    LOG_ERROR(
      main_logger,
      "Assigned non-kw18 options structure to kw18 opts; slicing" );
  }
  return *this;
}
  ...
  track_handle_list_type& tracks ) const
{
  ...
  while( get_next_nonblank_line( is, tmp ) )
  {
    ...
    kw18[ current_frame ].frame_number() =
      static_cast< unsigned >( p.dbl_frame_num );
    ...
    kw18[ current_frame ].track_location() = vgl_point_2d< double >(
      p.loc_x,
      p.loc_y );
    ...
    kw18[ current_frame ].obj_location() = vgl_point_2d< double >(
      p.obj_loc_x,
      p.obj_loc_y );
    ...
    kw18[ current_frame ].world_location() = vgl_point_3d< double >(
      p.world_x,
      p.world_y, p.world_z );
  ...
  const track_handle_list_type& tracks ) const
{
  ...
  for( size_t i = 0; i < tracks.size(); ++i )
  {
    ...
    for( size_t j = 0; j < n_frames; ++j )
    {
      ...
      unsigned long long kw18_ts_usecs = logged_get_field(
        kw18.timestamp_usecs,
        f.row, warnings );
  ...
  if( !warnings.empty() )
  {
    ...
    {
      sum += i->second;
      os << "# default values for '" << i->first << "': " << i->second <<
        " times\n";
    }
    LOG_WARN(
      main_logger, "Writing " << this->current_filename
                              << " : used " << sum <<
        " total default values for "
