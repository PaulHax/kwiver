  ...
  const csv_handler_map_type& handlers )
{
  for( csv_handler_map_cit i = handlers.begin(); i != handlers.end(); ++i )
  {
    ...
    element_store_base* b =
      track_oracle_core::get_mutable_element_store_base( i->first );
...
struct redundant_object_helper_type
{
  ...
  {
    field_handle_type world_location_fh =
      track_oracle_core::lookup_by_name( dt::tracking::world_location::c.name );
    this->world_location_store =
      track_oracle_core::get_mutable_element_store_base( world_location_fh );
    field_handle_type obj_location_fh =
      track_oracle_core::lookup_by_name( dt::tracking::obj_location::c.name );
    this->obj_location_store =
      track_oracle_core::get_mutable_element_store_base( obj_location_fh );
    field_handle_type bounding_box_fh =
      track_oracle_core::lookup_by_name( dt::tracking::bounding_box::c.name );
    this->bounding_box_store =
      track_oracle_core::get_mutable_element_store_base( bounding_box_fh );
    field_handle_type track_uuid_fh =
      track_oracle_core::lookup_by_name( dt::tracking::track_uuid::c.name );
    this->track_uuid_store =
      track_oracle_core::get_mutable_element_store_base( track_uuid_fh );
  }
    ...
    const map< string, string >& header_value_map )
  {
    ...
    if( have_world_xy )
    {
      ...
      local_hv_map[ "world_location_z" ] =
        ( ( world_xyz_probes[ 2 ] == header_value_map.end() ) ||
          ( world_xyz_probes[ 2 ]->second.empty() ) )
        ? "0.0"
    ...
    map< string,
      string >::const_iterator unique_id_probe =
      header_value_map.find( "unique_id" );
...
::inspect_file( string const& fn ) const
{
  ...
  if( !csv_tokenizer::get_record( ifs, values ) && !ifs.eof() )
  {
    return false;
  }
  ...
  track_handle_list_type& tracks ) const
{
  ...
  vector< string >::iterator track_sequence_probe = std::find(
    headers.begin(),
    headers.end(), "_track_sequence" );
  vector< string >::iterator external_id_probe = std::find(
    headers.begin(),
    headers.end(), "external_id" );
      ...
      ( external_id_probe == headers.end() ) )
  {
    LOG_ERROR(
      main_logger,
      "CSV headers do not contain either external_id or _track_sequence; cannot parse" );
    return false;
  }
  ...
  string frame_tag =
    ...
    ? "_parent_track"
    : "frame_number";
  vector< string >::iterator frame_probe = find(
  ...
  if( frame_probe == headers.end() )
  {
    LOG_ERROR(
      main_logger,
      "CSV headers do not contain expected frame tag '" << frame_tag <<
        "'; cannot parse" );
    return false;
  }
  ...
  for( unsigned long long record = 1; is; ++record )
  {
    ...
    if( values.size() != headers.size() )
    {
      LOG_ERROR(
        main_logger,
        "Line " << record << " has " << values.size() << " entries; expected "
    ...
    if( row_is_track )
    {
      ...
      if( sequence_to_handle_map.find( this_id ) ==
          sequence_to_handle_map.end() )
      {
    ...
    else
    {
      ...
      {
        // If we get here, it's because we're a 'frame' row referring to a
        // non-existent
        ...
        //
        // we can recover IF we've embedded the external_id on every frame.  Try
        // that
        // before bailing out.
        ...
        else
        {
          LOG_ERROR(
            main_logger,
            "FATAL: record " << record << " belongs to " <<
              this_frame_belongs_to
                             << ", which has no handle" );
