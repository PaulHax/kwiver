...
void
extract_frame(
  STANAG_4676::TrackPoint const* f,
  ...
  vidtk::track_handle_list_type& tracks )
{
  ...
  {
    LOG_ERROR(
      "Failed to convert track number '" << t->getNumber()
                                         << "'; ignoring track\n" );
    return false;
  }
...
bool
extract_tracks(
  STANAG_4676::TrackMessage::sptr message,
  vidtk::track_handle_list_type& tracks )
{
...
::inspect_file( string const& fn ) const
{
  ...
  {
    if( tokens[ i ].find( "<trackProduct" ) != string::npos )
    {
      has_product = true;
    }
    if( tokens[ i ].find( "stanag4676" ) != string::npos )
    {
      has_stanag4676 = true;
    }
  }
