...
get_optional_fields()
{
  ...
  // Note that these regexs are tied to the synthesized field names in the
  // reader.
  kwiversys::RegularExpression field_dbl( "^([a-zA-Z0-9]+)_([0-9]+)$" );
  kwiversys::RegularExpression field_kv( "^" + kpf_key_str +
    "_([a-zA-Z0-9]+)$" );
    ...
    if( e.typeid_str == dbltype )
    {
      ...
      if( field_dbl.find( e.name ) )
      {
        ...
        if( style != KPF::packet_style::INVALID )
        {
          ...
          optional_fields[ i ] = KPF::packet_t(
            KPF::packet_header_t(
              style,
              stoi( field_dbl.match( 2 ) ) ) );
  ...
  const KPF::packet_t& p )
{
  ...
  switch( p.header.style )
  {
    ...
    case KPF::packet_style::TS:
    {
      switch( p.header.domain )
      {
        case KPFC::timestamp_t::FRAME_NUMBER:
        {
          ...
          x( row ) =
            static_cast< dt::tracking::frame_number::Type >( p.timestamp.d );
          break;
        }
        default:
        {
    ...
    case KPF::packet_style::CSET:
    {
      ...
      auto s2i_map =
        track_oracle_core::get_field< kpf_cset_s2i_type >(
          SYSTEM_ROW_HANDLE,
          s2i_f );
...
::optional_fields_to_packets(
  kpf_utils::optional_field_state& ofs,
  const oracle_entry_handle_type& row )
{
  ...
  for( auto fh : track_oracle_core::fields_at_row( row ) )
  {
    ...
    switch( p.header.style )
    {
      ...
      case KPF::packet_style::CSET:
      {
        ...
        {
          const auto& s2i_map =
            track_oracle_core::get< kpf_cset_s2i_type >(
              SYSTEM_ROW_HANDLE,
              s2i_fh );
          const auto& payload =
            track_oracle_core::get< kpf_cset_sys_type >( row, fh );
          if( !( s2i_map.first && payload.first ) )
          {
            LOG_ERROR(
              main_logger, "KPF converting " << p.header
                                             << ": lost s2i map (" <<
                s2i_map.first << ") and/or "
                                             << "payload map (" <<
                payload.first << ")" );
            all_okay = false;
          }
        ...
        else
        {
          LOG_ERROR(
            main_logger,
            "lost string-to-index map for " << p.header  << "?" );
        }
        if( all_okay )
        {
          lost_value_flag = false;
      ...
      default:
      {
        ...
        ostringstream oss;
        oss << "No track-oracle-to-kpf handler for optional packet " <<
          p.header;
...
::write_optional_packets(
  const vector< KPF::packet_t >& packets,
  kwiver::logging_map_type& log_map,
...
kpf_utils
::read_unstructured_yaml( const string& fn )
{
