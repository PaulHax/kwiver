...
template < typename T >
bool
read_vector_from_xml(
  TiXmlElement* descriptor,
  const string& tag,
  vector< T >& d )
{
...
{
  const kwxml_reader_opts* rhs =
    dynamic_cast< const kwxml_reader_opts* >( &rhs_base );
  ...
  else
  {
    LOG_WARN(
      main_logger,
      "Assigned a non-kwxml options structure to a kwxml options structure: Slicing the class" );
  }
...
::inspect_file( const string& fn ) const
{
  ...
  {
    if( tokens[ i ].find( "<vibrantDescriptors>" ) != string::npos )
    {
      return true;
    }
  }
  ...
  track_handle_list_type& tracks ) const
{
             ...
             xml_root->IterateChildren( xml_track_objects ) ) )
  {
    ...
    //
    // treat track style a little differently, since we may want to skip
    // everything else
    ...
    {
      TiXmlText* track_style_text =
        track_handle.FirstChildElement( "trackStyle" ).FirstChild().ToText();
      if( track_style_text )
      {
    ...
    {
      TiXmlText* txt =
        track_handle.FirstChildElement( "clipFilename" ).FirstChild().ToText();
      if( txt ) { kwxml.clip_filename() = txt->ValueStr(); }
    }
    {
      TiXmlText* txt =
        track_handle.FirstChildElement(
          "basicAnnotation" ).FirstChild().ToText();
      if( txt ) { kwxml.basic_annotation() = txt->ValueStr(); }
    }
    {
      TiXmlText* txt =
        track_handle.FirstChildElement(
          "augmentedAnnotation" ).FirstChild().ToText();
      if( txt ) { kwxml.augmented_annotation() = txt->ValueStr(); }
    }
    {
      TiXmlText* txt =
        track_handle.FirstChildElement(
          "start_time_secs" ).FirstChild().ToText();
      if( txt )
      {
    ...
    {
      TiXmlText* txt =
        track_handle.FirstChildElement( "end_time_secs" ).FirstChild().ToText();
      if( txt )
      {
        istringstream iss( txt->ValueStr() );
        double tmp;
    ...
    {
      TiXmlText* txt =
        track_handle.FirstChildElement( "latitude" ).FirstChild().ToText();
      if( txt )
      {
        istringstream iss( txt->ValueStr() );
        double tmp;
    ...
    {
      TiXmlText* txt =
        track_handle.FirstChildElement( "longitude" ).FirstChild().ToText();
      if( txt )
      {
        istringstream iss( txt->ValueStr() );
        double tmp;
    ...
    {
      unsigned video_id;
      TiXmlText* video_id_text =
        track_handle.FirstChildElement( "videoID" ).FirstChild().ToText();
      if( video_id_text )
      {
    ...
    {
      TiXmlText* time_stamp_text =
        track_handle.FirstChildElement( "timeStamp" ).FirstChild().ToText();
      if( time_stamp_text )
      {
...

    {
      ...
      TiXmlText* source_tracks_text =
        track_handle.FirstChildElement(
          "sourceTrackIDs" ).FirstChild().ToText();
    ...
    // Object assessment element not considered currently. If necessary, add
    // here.

    // Frame information
    for( TiXmlElement* bbox_child =
           xml_track_objects->FirstChildElement( "bbox" );
         bbox_child != 0;
         bbox_child = bbox_child->NextSiblingElement( "bbox" ) )
    {
      ...
      if( bbox_child->QueryStringAttribute(
        "type",
        &box_type ) != TIXML_SUCCESS )
      {
      ...
      if( timestamp_str )
      {
        ...
        if( !( iss >> timestamp ) )
        {
          LOG_WARN(
            main_logger,
            "XML frame " << frame << ": couldn't parse timestamp string '" <<
              timestamp_str << "' as unsigned long long\n" );
        }
      }
    ...
    {
      TiXmlElement* labels_node =
        xml_track_objects->FirstChildElement( "labels" );
      if( labels_node )
      {
        ...
        if( label_domain_ptr )
        {
          ...
          if( label_domain == "virat" )
          {
                 ...
                 d = d->NextSiblingElement( "event" ) )
            {
              ...
              bool good = true;
              if( d->QueryStringAttribute(
                "type",
                &s.activity_name ) != TIXML_SUCCESS )
              {
                LOG_WARN(
                  main_logger,
                  "labels node event sub-node without event-type attribute at "
                    << d->Row() );
                good = false;
              }
              else
              {
                ...
                catch( aries_interface_exception& )
                {
                  LOG_WARN(
                    main_logger,
                    "labels node event '" << s.activity_name <<
                      "' is not a virat event at " << d->Row() );
                  good = false;
                }
              }
              if( d->QueryDoubleAttribute(
                "spatialOverlap",
                &s.spatial_overlap ) != TIXML_SUCCESS )
              {
                LOG_WARN(
                  main_logger,
                  "labels node event sub-node without spatialOverlap attribute at "
                    << d->Row() );
                good = false;
              }
              if( d->QueryDoubleAttribute(
                "temporalOverlap",
                &s.temporal_overlap ) != TIXML_SUCCESS )
              {
                LOG_WARN(
                  main_logger,
                  "labels node event sub-node without temporalOverlap attribute at "
                    << d->Row() );
                good = false;
              }
          ...
          else
          {
            LOG_WARN(
              main_logger,
              "labels node with unsupported domain '" << label_domain
                                                      <<
                "' (only virat currently support); skipped at " <<
                labels_node->Row() );
          }
        }
        else
        {
          LOG_WARN(
            main_logger,
            "labels node without domain attribute; skipped at " <<
              labels_node->Row() );
        }
      } // ...if d
    } // ... event labels

    for( TiXmlElement* descriptor =
           xml_track_objects->FirstChildElement( "descriptor" );
         descriptor != 0;
         descriptor = descriptor->NextSiblingElement( "descriptor" ) )
    {
      ...
      if( !descr_type )
      {
        LOG_WARN(
          main_logger,
          "Descriptor element without type attribute at " <<
            descriptor->Row() << "\n" );
      }

      // Classifier descriptor not currently implemented, but should be added to
      // this loop when done.
      if( descr_type && ( ( string( descr_type ) == "classifier" )
                          ||
                          ( string( descr_type ) ==
                            "PersonOrVehicleMovement" ) ) )
      {
        vector< double > activity_probabilities;
        activity_probabilities.resize(
          aries_interface::index_to_activity_map().size(), 0.0 );

        for( TiXmlElement* probNode =
               descriptor->FirstChildElement( "probability" );
             probNode != 0;
             probNode = probNode->NextSiblingElement( "probability" ) )
        {
          ...
          catch( aries_interface_exception& /*e*/ )
          {
            LOG_ERROR(
              main_logger,
              "Couldn't recognize " << probNode->Attribute( "activity" )
                                    << " as a valid activity?" );
            return false;
          }

          double prob;
          if( probNode->QueryDoubleAttribute(
            "value",
            &prob )  != TIXML_SUCCESS )
          {
            LOG_ERROR(
              main_logger,
              "Couldn't find a probability at " << probNode->Row() << "?" );
            prob = 0;
          }
      ...
      else if( string( descr_type ) == "texasHOF" )
      {
        ...
        TiXmlElement* count_e =
          descriptor->FirstChildElement( "vectorHOFCount" );
        if( ( !count_e ) || ( count_e->QueryIntAttribute(
          "value",
          &n_rows ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No vectorHOFCount at " << descriptor->Row() << "?\n" );
        }
        ...
        while( ( xml_row_node = descriptor->IterateChildren( xml_row_node ) ) )
        {
          ...
          if( !e )
          {
            LOG_WARN(
              main_logger,
              "Couldn't convert to element at " << xml_row_node->Row() <<
                "?\n" );
          }
          ...
          if( e->QueryIntAttribute( "id", &index ) != TIXML_SUCCESS )
          {
            LOG_WARN(
              main_logger,
              "Couldn't parse id near " << xml_row_node->Row() << "?\n" );
          }
      ...
      else if( string( descr_type ) == "rpiDBN1" )
      {
        ...
        if( !read_vector_from_xml< double >(
          descriptor, "vectorRPIDBN1",
          dbn1_data ) )
        {
          LOG_WARN(
            main_logger,
            "No rpiDBN1 data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "rpiDBN2" )
      {
        ...
        if( !read_vector_from_xml< double >(
          descriptor, "vectorRPIDBN2",
          dbn2_data ) )
        {
          LOG_WARN(
            main_logger,
            "No rpiDBN2 data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "vectorKWSOTI3" )
      {
        ...
        if( !read_vector_from_xml< double >(
          descriptor, "vectorKWSOTI3",
          kwsoti3_data ) )
        {
          LOG_WARN(
            main_logger,
            "No KWSOTI3 data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "icsiHOG" )
      {
        ...
        if( !read_vector_from_xml< double >(
          descriptor, "vectorICSIHOG",
          icsihog_data ) )
        {
          LOG_WARN(
            main_logger,
            "No icsiHOG data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "texasHOG" )
      {
        ...
        if( !read_vector_from_xml< double >(
          descriptor, "vectorTexasHOG",
          texashog_data ) )
        {
          LOG_WARN(
            main_logger,
            "No texasHOG data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "cornell" )
      {
          ...
          cornell_data ) )
        {
          LOG_WARN(
            main_logger,
            "No cornell data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "pvoHOG" )
      {
        ...
        if( !read_vector_from_xml< double >(
          descriptor, "pvoRawScores",
          pvo_hog_data ) )
        {
          LOG_WARN(
            main_logger,
            "No pvoHOG data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "UMDLDS" )
      {
        ...
        if( !read_vector_from_xml< double >( descriptor, "data", lds_data ) )
        {
          LOG_WARN(
            main_logger,
            "No UMDLDS data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "UMDPeriodicity" )
      {
        ...
        if( !read_vector_from_xml< double >( descriptor, "data", per_data ) )
        {
          LOG_WARN(
            main_logger,
            "No UMDPeriodicity data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "UMDSSM" )
      {
        ...
        if( !read_vector_from_xml< double >( descriptor, "data", ssm_data ) )
        {
          LOG_WARN(
            main_logger,
            "No UMDSSM data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      {
        wmap.add_msg(
          "Ignoring raw descriptor until disambiguated between TRED and VIRAT" );
      ...
      else if( string( descr_type ) == "CUTIC" )
      {
        ...
        if( !read_vector_from_xml< double >(
          descriptor, "scoreClass",
          cutic.score_class ) )
        {
          LOG_WARN(
            main_logger,
            "No scoreClass in CUTIC descriptor at " << descriptor->Row() <<
              "\n" );
        }
        else if( !read_vector_from_xml< int >(
          descriptor, "scoreType",
          cutic.score_type ) )
        {
          LOG_WARN(
            main_logger,
            "No scoreType in CUTIC descriptor at " << descriptor->Row() <<
              "\n" );
        }
        else if( !read_vector_from_xml< double >(
          descriptor, "simTemporal",
          cutic.sim_temporal ) )
        {
          LOG_WARN(
            main_logger,
            "No simTemporal in CUTIC descriptor at " << descriptor->Row() <<
              "\n" );
        }
        else if( !read_vector_from_xml< short >(
          descriptor, "descIndex",
          cutic.desc_index ) )
        {
          LOG_WARN(
            main_logger,
            "No descIndex in CUTIC descriptor at " << descriptor->Row() <<
              "\n" );
        }
        else if( !read_vector_from_xml< double >(
          descriptor, "descRaw",
          cutic.desc_raw ) )
        {
          LOG_WARN(
            main_logger,
            "No descRaw in CUTIC descriptor at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "CUBoF" )
      {
        ...
        if( !read_vector_from_xml< double >( descriptor, "data", bof_data ) )
        {
          LOG_WARN(
            main_logger,
            "No CUBoF data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "CUTexture" )
      {
        ...
        if( !read_vector_from_xml< double >( descriptor, "data", tex_data ) )
        {
          LOG_WARN(
            main_logger,
            "No CUTexture at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "CUColMoment" )
      {
        ...
        if( !read_vector_from_xml< double >( descriptor, "data", colmom_data ) )
        {
          LOG_WARN(
            main_logger,
            "No CUColMoment data at " << descriptor->Row() << "\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "overlap" )
      {
        ...
        if( ( !e ) || ( e->QueryUnsignedAttribute(
          "value",
          &dot.src_trk_id ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No src_trk_id at " << descriptor->Row() << "\n" );
          good = false;
        }
        e = descriptor->FirstChildElement( "dst_trk_id" );
        if( ( !e ) || ( e->QueryUnsignedAttribute(
          "value",
          &dot.dst_trk_id ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No dst_trk_id at " << descriptor->Row() << "\n" );
          good = false;
        }
        e = descriptor->FirstChildElement( "src_activity_id" );
        if( ( !e ) || ( e->QueryUnsignedAttribute(
          "value",
          &dot.src_activity_id ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No src_activity_id at " << descriptor->Row() << "\n" );
          good = false;
        }
        e = descriptor->FirstChildElement( "dst_activity_id" );
        if( ( !e ) || ( e->QueryUnsignedAttribute(
          "value",
          &dot.dst_activity_id ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No dst_activity_id at " << descriptor->Row() << "\n" );
          good = false;
        }
        e = descriptor->FirstChildElement( "n_frames_src" );
        if( ( !e ) || ( e->QueryUnsignedAttribute(
          "value",
          &dot.n_frames_src ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No n_frames_src at " << descriptor->Row() << "\n" );
          good = false;
        }
        e = descriptor->FirstChildElement( "n_frames_dst" );
        if( ( !e ) || ( e->QueryUnsignedAttribute(
          "value",
          &dot.n_frames_dst ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No n_frames_dst at " << descriptor->Row() << "\n" );
          good = false;
        }
        e = descriptor->FirstChildElement( "n_frames_overlap" );
        if( ( !e ) || ( e->QueryUnsignedAttribute(
          "value",
          &dot.n_frames_overlap ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No n_frames_overlap at " << descriptor->Row() << "\n" );
          good = false;
        }
        e = descriptor->FirstChildElement( "mean_centroid_distance" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &dot.mean_centroid_distance ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No mean_centroid_distance at " << descriptor->Row() << "\n" );
          good = false;
        }
        ...
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &dot.mean_percentage_overlap ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No mean_percentage_overlap at " << descriptor->Row() << "\n" );
          good = false;
        }

        // You know that sometime, somewhere, this will come to pass...
        // only check if good is still true (otherwise can't be sure
        // dot.{src,dst}_activity_id is valid)
        if( good )
        {
          const map< size_t,
            string >& i2a = aries_interface::index_to_activity_map();
          ...
          if( ( !e ) || ( e->QueryStringAttribute(
            "value",
            &s_descriptor ) != TIXML_SUCCESS ) )
          {
            LOG_WARN(
              main_logger,
              "No src_activity_name at " << descriptor->Row() << "\n" );
            good = false;
          }
          ...
          if( s_descriptor != s_system )
          {
            LOG_WARN(
              main_logger,
              "Src activity index is " << dot.src_activity_id << ", named '" <<
                s_descriptor
                                       <<
                "' in the file but is now '" << s_system << "' at " <<
                descriptor->Row() );
            good = false;
          }
            ...
            &s_descriptor ) != TIXML_SUCCESS ) )
          {
            LOG_WARN(
              main_logger,
              "No dst_activity_name at " << descriptor->Row() << "\n" );
            good = false;
          }
          ...
          if( s_descriptor != s_system )
          {
            LOG_WARN(
              main_logger,
              "Dst activity index is " << dot.dst_activity_id << ", named '" <<
                s_descriptor
                                       <<
                "' in the file but is now '" << s_system << "' at " <<
                descriptor->Row() );
      ...
      else if( string( descr_type ) == "queryResultScore" )
      {
          ...
          &d ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No queryResultScore value at " << descriptor->Row() << "?\n" );
        }
        else
        {
      ...
      else if( string( descr_type ) == "metadataDescriptor" )
      {
        ...
        else if( ( e->QueryDoubleAttribute(
          "value",
          &meta.gsd ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "Could not read GSD at " << descriptor->Row() << "?\n" );
        }
        e = descriptor->FirstChildElement( "SensorLatitude" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.sensor_latitude ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No SensorLatitude value at " << descriptor->Row() << "?\n" );
        }
        e = descriptor->FirstChildElement( "SensorLongitude" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.sensor_longitude ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No SensorLongitude value at " << descriptor->Row() << "?\n" );
        }
        e = descriptor->FirstChildElement( "UpperLeftCornerLatitude" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.upper_left_corner_latitude ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No UpperLeftCornerLatitude value at " << descriptor->Row() <<
              "?\n" );
        }
        e = descriptor->FirstChildElement( "UpperLeftCornerLongitude" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.upper_left_corner_longitude ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No UpperLeftCornerLongitude value at " << descriptor->Row() <<
              "?\n" );
        }
        e = descriptor->FirstChildElement( "UpperRightCornerLatitude" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.upper_right_corner_latitude ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No UpperRightCornerLatitude value at " << descriptor->Row() <<
              "?\n" );
        }
        e = descriptor->FirstChildElement( "UpperRightCornerLongitude" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.upper_right_corner_longitude ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No UpperRightCornerLongitude value at " << descriptor->Row() <<
              "?\n" );
        }
        e = descriptor->FirstChildElement( "LowerLeftCornerLatitude" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.lower_left_corner_latitude ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No LowerLeftCornerLatitude value at " << descriptor->Row() <<
              "?\n" );
        }
        e = descriptor->FirstChildElement( "LowerLeftCornerLongitude" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.lower_left_corner_longitude ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No LowerLeftCornerLongitude value at " << descriptor->Row() <<
              "?\n" );
        }
        e = descriptor->FirstChildElement( "LowerRightCornerLatitude" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.lower_right_corner_latitude ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No LowerRightCornerLatitude value at " << descriptor->Row() <<
              "?\n" );
        }
        e = descriptor->FirstChildElement( "LowerRightCornerLongitude" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.lower_right_corner_longitude ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No LowerRightCornerLongitude value at " << descriptor->Row() <<
              "?\n" );
        }
        e = descriptor->FirstChildElement( "HorizontalFieldOfView" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.horizontal_field_of_view ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No HorizontalFieldOfView value at " << descriptor->Row() <<
              "?\n" );
        }
        e = descriptor->FirstChildElement( "VerticalFieldOfView" );
        if( ( !e ) || ( e->QueryDoubleAttribute(
          "value",
          &meta.vertical_field_of_view ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No VerticalFieldOfView value at " << descriptor->Row() << "?\n" );
        }
        ...
        if( ( !e ) || ( str_time_stamp = e->Attribute( "value" ) ) == NULL )
        {
          LOG_WARN(
            main_logger,
            "No TimeStampMicrosecondsSince1970 value at " <<
              descriptor->Row() << "?\n" );
        }
        try
        {
        ...
        if( ( !e ) || ( e->QueryFloatAttribute(
          "value",
          &meta.slant_range ) != TIXML_SUCCESS ) )
        {
          LOG_WARN(
            main_logger,
            "No SlantRange value at " << descriptor->Row() << "?\n" );
        }
      ...
      else if( string( descr_type ) == "motionDescriptor" )
      {
        ...
        for( unsigned d_index = 0; ptrs[ d_index ] != 0; ++d_index )
        {
          ...
          if( ( !e ) || ( e->QueryDoubleAttribute(
            "value",
            ptrs[ d_index ] ) != TIXML_SUCCESS ) )
          {
            LOG_WARN(
              main_logger,
              "No " << tags[ d_index ] << " at " << descriptor->Row() <<
                "in motionDescriptor\n" );
          }
        }
      ...
      else
      {
        ...
        {
          wmap.add_msg(
            string( "Unknown descriptor type '" ) + descr_type +
            string( "'" ) );
        }
        else
        {
          LOG_WARN(
            main_logger, "At " << descriptor->Row()
