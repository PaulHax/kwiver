...
struct TRACK_KWXML_EXPORT kwxml_reader_opts : public
                                              file_format_reader_opts_base
{
  std::string track_style_filter; // if set, only load tracks whose <trackStyle>
                                  // matches this

  kwxml_reader_opts&
  set_track_style_filter( const std::string& s )
  {
  ...
  virtual kwxml_reader_opts&
  reset()
  {
    set_track_style_filter( "" );
    return *this;
  }
...
public:
  ...
  schema_instance() const { return new track_kwxml_type(); }
