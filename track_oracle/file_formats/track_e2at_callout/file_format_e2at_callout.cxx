...
parse_time( const string& s )
{
  ...
  if( re.find( s ) )
  {
    ...
    {
      return make_pair(
        "e2at-callout: Logic error: couldn't parse timestamp from '" +
        iss.str() + "'", 0 );
    }
  }
  else
  {
    return make_pair(
      "e2at-callout: timestamp regexp fails on '" + s + "'",
      0 );
  }
}
...
parse_latlon(
  ...
  double& lon )
{
  kwiversys::RegularExpression re(
    "([0-9\\-\\.]+)([NnSs])([0-9\\-\\.]+)([WwEe])" );
  ...
  if( !( latss >> lat ) )
  {
    LOG_ERROR(
      main_logger,
      "Logic error: expected to parse a latitude from '" << re.match( 1 ) <<
        "'" );
    return false;
  }
  if( !( lonss >> lon ) )
  {
    LOG_ERROR(
      main_logger,
      "Logic error: expected to parse a longitude from '" << re.match( 3 ) <<
        "'" );
    return false;
  }
  ...
  for( size_t i = 0; i < 2; ++i )
  {
    ...
    if( !getline( is, line ) )
    {
      if( i == 0 )
      {
        LOG_ERROR(
          main_logger,
          "Couldn't read first line from '" << fn << "' during inspection" );
      }
      return false;
    }
    trim( line );
  ...
  track_handle_list_type& tracks ) const
{
  ...
  while( getline( is, line ) )
  {
    ...
    if( tokens.size() < 5 )
    {
      LOG_WARN(
        main_logger,
        "E2AT callout: line " << c << ": skipping too-short line '" << line <<
          "'" );
      continue;
    }
    ...
    if( !ts_probe.first.empty() )
    {
      LOG_WARN(
        main_logger,
        "E2AT callout: line " << c <<
          ": skipping due to timestamp conversion failure '" <<
          ts_probe.first << "'; line is '" << line << "'" );
      continue;
    }
