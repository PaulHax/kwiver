...
::from_string( const string& s )
{
  ...
  else if( s == file_format_type::to_string( TF_MITRE_BOX_XML ) )
  {
    return TF_MITRE_BOX_XML;
  }
  else if( s == file_format_type::to_string( TF_COMMS_XML ) )
  {
    return TF_COMMS_XML;
  }
  else if( s == file_format_type::to_string( TF_KST ) ) { return TF_KST; }
  else if( s == file_format_type::to_string( TF_VATIC ) ) { return TF_VATIC; }
  else if( s == file_format_type::to_string( TF_VPD_TRACK ) )
  {
    return TF_VPD_TRACK;
  }
  else if( s == file_format_type::to_string( TF_VPD_EVENT ) )
  {
    return TF_VPD_EVENT;
  }
  else if( s == file_format_type::to_string( TF_E2AT_CALLOUT ) )
  {
    return TF_E2AT_CALLOUT;
  }
  ...
  else if( s == file_format_type::to_string( TF_KPF_GEOM ) )
  {
    return TF_KPF_GEOM;
  }
  else if( s == file_format_type::to_string( TF_KPF_ACT ) )
  {
    return TF_KPF_ACT;
  }
