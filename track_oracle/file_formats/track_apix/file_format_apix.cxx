...
// a helper class so that DBFClose is always called even if we return on an
// error
...
{
  const apix_reader_opts* rhs =
    dynamic_cast< const apix_reader_opts* >( &rhs_base );
  ...
  else
  {
    LOG_WARN(
      main_logger,
      "Assigned a non-apix options structure to a apix options structure: Slicing the class" );
  }
  ...
  track_handle_list_type& tracks ) const
{
  ...
  if( h.hDBF == NULL )
  {
    LOG_ERROR(
      main_logger,
      "Couldn't open '" << fn << "' to read an APIX track" );
    return false;
  }
  ...
  const char* reqFields[] = { "Latitude", "Longitude", "DataUTCTim",
                              "DataTimeMS", "MGRS", "FrameNum", 0 };
  bool allOkay = true;
  for( unsigned i = 0; reqFields[ i ] != 0; ++i )
  {
    if( field_map.find( reqFields[ i ] ) == field_map.end() )
    {
      LOG_ERROR(
        main_logger,
        "APIX track file '" << fn << "' does not contain a '" <<
          reqFields[ i ] << "' field" );
      allOkay = false;
    }
  }
  ...
  for( int i = 0; i < DBFGetRecordCount( h.hDBF ); ++i )
  {
    ...
    apix_track[ f ].lat() = DBFReadDoubleAttribute(
      h.hDBF, i,
      field_map[ "Latitude" ] );
    apix_track[ f ].lon() = DBFReadDoubleAttribute(
      h.hDBF, i,
      field_map[ "Longitude" ] );

    int tsecs = DBFReadIntegerAttribute( h.hDBF, i, field_map[ "DataUTCTim" ] );
    int tmsecs = DBFReadIntegerAttribute(
      h.hDBF, i,
      field_map[ "DataTimeMS" ] );
    int frame_num = DBFReadIntegerAttribute(
      h.hDBF, i,
      field_map[ "FrameNum" ] );
    apix_track[ f ].utc_timestamp() = vital::timestamp(
      ( tsecs * 1.0e6 ) +
      ( tmsecs * 1.0e3 ), frame_num );
    ...
    if( this->opts.verbose )
    {
      ...
      kwiver::geographic::geo_coords latlon_coords( apix_track[ f ].lat(),
        apix_track[ f ].lon() );
      ...
      if( !( mgrs_coords.is_valid() && latlon_coords.is_valid() ) )
      {
        LOG_INFO(
          main_logger,
          "APIX track file '" << fn << "' frame " << frame_num <<
            ": geocoord error: "
                              << "mgrs '" << mgrs <<
            "'; valid: " << mgrs_coords.is_valid() << "; lat/lon "
                              << apix_track[ f ].lat() << "," <<
            apix_track[ f ].lon() << ": valid: " << latlon_coords.is_valid() );
        continue;
      }
      ...
      LOG_INFO(
        main_logger,
        "Info: APIX track file '" << fn << "' frame " << frame_num <<
          ": lat/long vs MGRS "
                                  << dEasting << " , " <<
          dNorthing << " " << tsecs << "." << tmsecs );
    }
  }
