...
::inspect_file( const string& fn ) const
{
  ...
  {
    if( tokens[ i ].find( "<queryRegionElements>" ) != string::npos )
    {
      return true;
    }
  }
  ...
  track_handle_list_type& tracks ) const
{
  ...
  if( !doc.LoadFile() )
  {
    LOG_ERROR(
      main_logger,
      "MITRE XML file '" << fn << "': TinyXML failed to load document" );
    return false;
  }
  ...
  if( !xmlRoot )
  {
    LOG_INFO(
      main_logger,
      "MITRE XML file '" << fn << "': TinyXML found no root node?" );
    return false;
  }
  ...
  while( ( xmlTrackObjects = xmlRoot->IterateChildren( xmlTrackObjects ) ) )
  {
    ...
    if( !e )
    {
      LOG_ERROR(
        main_logger,
        "MITRE XML file '" << fn << "': TinyXML couldn't cast to element?" );
      return false;
    }
    ...
    if( e->QueryDoubleAttribute( "height", &height ) != TIXML_SUCCESS )
    {
      LOG_INFO(
        main_logger,
        "MITRE XML file '" << fn << "': row " << e->Row() << ": no height?" );
      return false;
    }
    if( e->QueryDoubleAttribute( "width", &width ) != TIXML_SUCCESS )
    {
      LOG_INFO(
        main_logger,
        "MITRE XML file '" << fn << "': row " << e->Row() << ": no width?" );
      return false;
    }
    if( e->QueryDoubleAttribute( "x", &x ) != TIXML_SUCCESS )
    {
      LOG_INFO(
        main_logger,
        "MITRE XML file '" << fn << "': row " << e->Row() << ": no x?" );
      return false;
    }
    if( e->QueryDoubleAttribute( "y", &y ) != TIXML_SUCCESS )
    {
      LOG_INFO(
        main_logger,
        "MITRE XML file '" << fn << "': row " << e->Row() << ": no y?" );
      return false;
    }
