...
struct TRACK_COMMS_XML_EXPORT comms_xml_reader_opts : public
                                                      file_format_reader_opts_base
{
  ...
  set_comms_qid( const std::string& s )
  {
    this->comms_qid = s;
    return *this;
  }

  comms_xml_reader_opts& operator=(
    const file_format_reader_opts_base& rhs_base );
...
{
public:
  file_format_comms_xml() : file_format_base( TF_COMMS_XML,
                              "MITRE VIRAT test harness comms XML" )
  {
  ...
  schema_instance() const { return new track_comms_xml_type(); }
