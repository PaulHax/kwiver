  ...
  ::kwiver::track_oracle::track_handle_list_type& track_ids )
{
  ...
  while( ( xmlTrackObjects = activitySet->IterateChildren( xmlTrackObjects ) ) )
  {
    ...
    if( !e )
    {
      LOG_ERROR(
        main_logger,
        "COMMS XML file '" << filename <<
          "': TinyXML couldn't cast to element?" );
      return false;
    }
    ...
    if( e->QueryStringAttribute( "strmid", &track_source ) != TIXML_SUCCESS )
    {
      LOG_ERROR(
        main_logger,
        "COMMS XML file '" << filename << "': row " << e->Row() <<
          ": no strmid?" );
      return false;
    }
    ...
    string::reverse_iterator sep = find(
      track_source.rbegin(),
      track_source.rend(), '/' );
    ...
    while( ( xmlFrameObjects = e->IterateChildren( xmlFrameObjects ) ) )
    {
      ...
      if( f->QueryValueAttribute( "frame", &frameTime ) != TIXML_SUCCESS )
      {
        LOG_ERROR(
          main_logger,
          "COMMS XML file '" << filename << "': row " << f->Row() <<
            ": no frame?" );
        return false;
      }
...
{
  const comms_xml_reader_opts* rhs =
    dynamic_cast< const comms_xml_reader_opts* >( &rhs_base );
  ...
  else
  {
    LOG_WARN(
      main_logger,
      "Assigned a non-comms_xml options structure to a comms_xml options structure: Slicing the class" );
  }
  ...
  track_handle_list_type& tracks ) const
{
  ...
  if( !doc.LoadFile() )
  {
    LOG_ERROR(
      main_logger,
      "COMMS XML file '" << fn << "': TinyXML failed to load document" );
    return false;
  }
  ...
  while( ( thRecv = doc.IterateChildren( "thRecv", thRecv ) ) )
  {
    ...
    if( !e )
    {
      LOG_ERROR(
        main_logger,
        "COMMS XML file '" << fn << "': TinyXML couldn't cast to element?" );
      return false;
    }

    string mtype;
    if( e->QueryValueAttribute( "mtype", &mtype ) != TIXML_SUCCESS )
    {
      continue;
    }
    ...
    string resp_qid;
    if( e->QueryValueAttribute( "qid", &resp_qid ) != TIXML_SUCCESS )
    {
      continue;
    }
    if( !this->opts.comms_qid.empty() && ( resp_qid != this->opts.comms_qid ) )
    {
      continue;
    }
