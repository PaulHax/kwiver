...
public:
  ...
  kst_etype
  get_etype() const { return KST_STRING; }
  string
  get_str() const { return s; }
};
...
public:
  ...
  kst_etype
  get_etype() const { return KST_NODE_PTR; }
  kst_node*
  get_node_ptr() const { return n; }
};
...
::get_element( unsigned index )
{
  if( index >= this->elist.size() )
  {
    LOG_ERROR(
      main_logger,
      "Attempt to access node " << index << " in element list of size " <<
        this->elist.size() );
    return 0;
  }
...
::get_string( unsigned index, string& s )
{
  ...
  if( e->get_etype() != KST_STRING )
  {
    LOG_ERROR(
      main_logger,
      "Tried to access node as " << KST_STRING << "; found " <<
        e->get_etype() );
    return false;
  }
...
::e_is_long_long( unsigned index, unsigned long long& ts )
{
  ...
  if( !( iss >> ts ) )
  {
    LOG_ERROR(
      main_logger,
      "Couldn't parse unsigned long long from '" << es << "'" );
    return false;
  }
  return true;
}
...
::e_is_node( unsigned index, kst_node*& ptr )
{
  ...
  if( e->get_etype() != KST_NODE_PTR )
  {
    LOG_ERROR(
      main_logger,
      "Tried to access node as " << KST_NODE_PTR << "; found " <<
        e->get_etype() );
    return false;
  }
...
::e_is_box( unsigned index, vgl_box_2d< double >& box )
{
  ...
  if( n->elist.size() != 2 )
  {
    LOG_ERROR(
      main_logger,
      "Parsing box; expected two elements; found " << n->elist.size() );
    return false;
  }
  ...
  if( ul.size() != 2 )
  {
    LOG_ERROR(
      main_logger,
      "Parsing box; upper left contains " << ul.size() <<
        " elements; expected 2" );
    return false;
  }
  if( lr.size() != 2 )
  {
    LOG_ERROR(
      main_logger,
      "Parsing box; lower right contains " << lr.size() <<
        " elements; expected 2" );
    return false;
  }
...
get_token( istream& is )
{
  ...
  while( is.good() )
  {
    if( token_index == token_buf_size )
    {
      ...
      LOG_ERROR(
        main_logger,
        "KST token more than " << token_buf_size <<
          " characters?  Partial token:\n'"
                               << token_buf << "'\n" );
    ...
    // if it's a # (outside of quotes), read until either is.not-good or
    // end-of-line
    if( c == '#' )
    {
      char comment_c;
  ...
  kst_node* this_node )
{
  ...
  while( is.good() )
  {
    ...
    // otherwise, it's a string; add it to the element list (only if it's
    // nonempty)
    else if( s.size() > 0 )
    {
  ...
  track_handle_list_type& tracks ) const
{
  ...
  LOG_INFO(
    main_logger, "KST: Parsed " << n_toplevel_elements << " elements in "
  ...
  {
    LOG_ERROR(
      main_logger, "KST tree only has " << n_toplevel_elements
  ...
  if( !( kst_tree->e_is_str( index++, p1 ) && kst_tree->e_is_str(
    index++,
    p2 ) &&
         // (p1 == "RESULTS") && (p2 == "1" )))
         ( p1 == "RESULTS" ) && ( p2 == "2" ) ) )
  ...
  while( index + ( node_element_count - 1 )  < n_toplevel_elements )
  {
...
#if 0
    {
      ...
      LOG_DEBUG(
        main_logger,
        "instance: index " << index << " +3 " << 3 << " : " << index + 3 <<
          " : " << rc << " '" << s << "'" );
    }
#endif
    ...
    //
    // Further update: it seems PVMoving is reported as un-merged segments
    // rather
    ...
    if( !n->elist.empty() )
    {
      ...
      for( size_t node_index = 0; node_index < n->elist.size(); ++node_index )
      {
        ...
        // that's great.
        bool is_classifier = ( ( s == "\"Classifier\"" ) ||
                               ( s == "\"classifier\"" ) );
  ...
  LOG_INFO(
    main_logger, "KST: Parsed " << tracks.size() << " tracks in "
                                << timer.real()  / 1000.0 << " seconds" );
  timer.mark();
  // clean up
  delete kst_tree;
  LOG_INFO(
    main_logger,
    "KST: Cleaned up tree in " << timer.real() / 1000.0 << " seconds" );
