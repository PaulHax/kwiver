...
::read( istream& is, track_handle_list_type& tracks ) const
{
  ...
  while( reader.next() )
  {
    ...
    try
    {
      ...
      {
        ostringstream oss;
        oss << "Missing packet ID1 on non-meta record line " << rc <<
          "; skipping";
      ...
      kpf_loader fields[] = {
        { KPF::packet_header_t(
          KPF::packet_style::GEOM,
          KPFC::bbox_t::IMAGE_COORDS ), true },
        { KPF::packet_header_t(
          KPF::packet_style::TS,
          KPFC::timestamp_t::FRAME_NUMBER ), true },
        { KPF::packet_header_t(
          KPF::packet_style::ID,
          KPFC::id_t::DETECTION_ID ), true } };
    ...
    catch( const kpf_geom_exception& e )
    {
      LOG_ERROR(
        main_logger,
        "Error parsing KPF geom: " << e.what << "; skipping" );
  ...
  for( const auto& t : tracks )
  {
    for( const auto& f : track_oracle_core::get_frames( t ) )
    {
      ...
      w.set_schema( KPF::schema_style::GEOM )
        ...
        box_adapter(
          entry[ f ].bounding_box() ), KPFC::bbox_t::IMAGE_COORDS )
        << KPF::writer< KPFC::timestamp_t >(
        entry[ f ].frame_number(),
