...
{
  bool promote_pvmoving; // if set, some activities will be converted to
                         // {p,v}moving

  xgtf_reader_opts&
  set_promote_pvmoving( bool p )
  {
    this->promote_pvmoving = p;
    return *this;
  }

  xgtf_reader_opts& operator=( const file_format_reader_opts_base& rhs_base );

  virtual xgtf_reader_opts&
  reset()
  {
    set_promote_pvmoving( false );
    return *this;
  }
...
{
public:
  file_format_xgtf() : file_format_base( TF_XGTF,
                         "ViPER ground truth (using VIRAT schemas)" )
  {
    this->opts.reset();
  ...
  schema_instance() const { return new track_xgtf_type(); }
