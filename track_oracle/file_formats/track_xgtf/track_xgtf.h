...
{
  // track level data
  track_field< dt::tracking::external_id > external_id; // attribute id in phase
                                                        // one
  track_field< std::string >& type; // 'name' in phase one. could be pvo as
                                    // well.
  ...
  track_xgtf_type() :
    ...
    frame_span( Track.add_field< std::pair< unsigned int,
