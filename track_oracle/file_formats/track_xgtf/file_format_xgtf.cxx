...
typedef multimap< pair< unsigned int, unsigned int >,
  activity_type > viper_activity_list_t;
typedef multimap< pair< unsigned int, unsigned int >,
  activity_type >::iterator viper_activity_list_it;
typedef multimap< pair< unsigned int, unsigned int >,
  activity_type >::const_iterator viper_activity_list_c_it;
  ...
  named_nodes_t& named_nodes )
{
  ...
  while( ( xml_child_node = node->IterateChildren( xml_child_node ) ) )
  {
    ...
    {
      LOG_ERROR(
        main_logger, "Error reading line " << xml_child_node->Row()
                                           <<
  ...
  pair< unsigned int, unsigned int >& span )
{
  ...
  if( !okay )
  {
    LOG_ERROR(
      main_logger,
      "Couldn't parse framespan '" << framespan_str << "'" );
  }
  ...
  vgl_box_2d< double >& box )
{
  ...
  if( !e )
  {
    LOG_ERROR(
      main_logger,
      "extract_bounding_box: Couldn't convert xmlnode to element?" );
    return false;
  }
  if( !parse_framespan( e->Attribute( "framespan" ), span ) )
  {
    LOG_ERROR(
      main_logger,
      "extract_bounding_box: Couldn't extract framespan?" );
    return false;
  }
  ...
  double& occlusion_value )
{
  ...
  if( !e )
  {
    LOG_ERROR(
      main_logger,
      "extract_occlusion: Couldn't convert xmlnode to element?" );
    return false;
  }
  if( !parse_framespan( e->Attribute( "framespan" ), span ) )
  {
...
bool
extract_viper_frame_data(
  const named_nodes_t& named_nodes,
  ::kwiver::track_oracle::track_handle_type& this_track,
  map< unsigned int,
    ::kwiver::track_oracle::frame_handle_type >& xgtf_frame_map )
{
  ...
  while( ( xml_bbox_node =
             xml_location_node->IterateChildren( xml_bbox_node ) ) )
  {
    ...
    {
      ::kwiver::track_oracle::frame_handle_type frame_handle =
        xgtf_schema( this_track ).create_frame();
  ...
  if( probe != named_nodes.end() )
  {
    ...
    while( ( xml_fval_node =
               xml_occlusion_node->IterateChildren( xml_fval_node ) ) )
    {
      ...
      {
        map< unsigned,
          ::kwiver::track_oracle::frame_handle_type >::const_iterator frame_it =
          xgtf_frame_map.find( frame );
  ...
  ::kwiver::logging_map_type& warnings )
{
  ...
  size_t PERSON_MOVING_INDEX =
    ::kwiver::track_oracle::aries_interface::activity_to_index(
      "PersonMoving" );
  size_t VEHICLE_MOVING_INDEX =
    ::kwiver::track_oracle::aries_interface::activity_to_index(
      "VehicleMoving" );
  ...
  for( named_nodes_c_it n = named_nodes.begin(); n != named_nodes.end(); ++n )
  {
    ...
    while( ( xml_bvalue_node = n->second->IterateChildren( xml_bvalue_node ) ) )
    {
      ...
      if( !e )
      {
        LOG_ERROR(
          main_logger,
          "extract_viper_activities: couldn't convert " << activity_name
                                                        <<
            " bvalues to element?" );
        return false;
      }
      ...
      else
      {
        ...
        if( activity_name == "Entering a Vehicle" )
        {
          activity_name = "Getting Into a Vehicle";
        }
        else if( activity_name == "Exiting a Vehicle" )
        {
          activity_name = "Getting Out of a Vehicle";
        }
        else if( activity_name == "Person Entering a Facility" )
        {
          activity_name = "Entering a Facility";
        }
        else if( activity_name == "Person Exiting a Facility" )
        {
          activity_name = "Exiting a Facility";
        }
        else if( activity_name == "Environment-Related Occlusion" )
        {
          activity_name = "Not Scored";
        }

        // adjust for ARIES aliasing (sigh)
        if( activity_name == "Pulling" ) { activity_name = "PERSON_PULLING"; }
        else if( activity_name == "Carrying Together" )
        {
          activity_name = "PERSON_CARRYING_TOGETHER";
        }
        else if( activity_name == "Climbing Atop" )
        {
          activity_name = "PERSON_CLIMBING_ATOP";
        }
        else if( activity_name == "Driving into a Facility" )
        {
          activity_name = "VEHICLE_DRIVING_INTO_A_FACILITY";
        }
        else if( activity_name == "Driving out of a Facility" )
        {
          activity_name = "VEHICLE_DRIVING_OUT_OF_A_FACILITY";
        }
        else if( activity_name == "Kicking" )
        {
          activity_name = "PERSON_KICKING";
        }
        else if( activity_name == "Laying Wire" )
        {
          activity_name = "PERSON_LAYING_WIRE";
        }
        else if( activity_name == "Looping" )
        {
          activity_name = "VEHICLE_LOOPING";
        }
        else if( activity_name == "Maintaining Distance" )
        {
          activity_name = "VEHICLE_MAINTAINING_DISTANCE";
        }
        else if( activity_name == "Passing" )
        {
          activity_name = "VEHICLE_PASSING";
        }
        else if( activity_name == "Pushing" )
        {
          activity_name = "PERSON_PUSHING";
        }
        else if( activity_name == "Sitting" )
        {
          activity_name = "PERSON_SITTING";
        }
        else if( activity_name == "Throwing" )
        {
          activity_name = "PERSON_THROWING";
        }
      }
      ...
      if( !skip_flag )
      {
        ...
        try
        {
          ...
          if( ::kwiver::track_oracle::aries_interface::promote_to_PERSON_MOVING(
            act_type.second ) )
          {
            promote_type.first = true;
            promote_type.second = PERSON_MOVING_INDEX;
          }
          else if( ::kwiver::track_oracle::aries_interface::
                   promote_to_VEHICLE_MOVING( act_type.second ) )
          {
            promote_type.first = true;
        ...
        catch( ::kwiver::track_oracle::aries_interface_exception&
               aries_type_exception )
        {
          warnings.add_msg(
            string( "unrecognized activity: " ) +
            activity_name + ": " + aries_type_exception.what() );
        ...
        if( e->Attribute( "value" ) == true_str )
        {
          ...
          if( !parse_framespan( e->Attribute( "framespan" ), span ) )
          {
            LOG_ERROR(
              main_logger,
              "Couldn't parse framespan for " << activity_name << "?" );
            return false;
          }
...
doc_to_source_node( const string& filename, xml_document_t& doc )
{
  ...
  if( !xml_root )
  {
    LOG_ERROR(
      main_logger,
      "Couldn't load root element from '" << filename << "'; skippig" );
    return 0;
  }
  ...
  if( !xml_data )
  {
    LOG_ERROR(
      main_logger,
      "Couldn't find the 'data' child in '" << filename << "'; skipping" );
    return 0;
  }
  ...
  if( !xml_source )
  {
    LOG_ERROR(
      main_logger,
      "Couldn't find the 'sourcefile' child in '" << filename <<
        "'; skipping" );
    return 0;
  }
...
{
  const xgtf_reader_opts* rhs =
    dynamic_cast< const xgtf_reader_opts* >( &rhs_base );
  ...
  else
  {
    LOG_WARN(
      main_logger,
      "Assigned a non-xgtf options structure to a xgtf options structure: Slicing the class" );
  }
  ...
  track_handle_list_type& tracks ) const
{
  ...
  if( !doc.LoadFile() )
  {
    ...
    LOG_ERROR(
      main_logger,
      "Error location: row " << doc.ErrorRow() << "; col " << doc.ErrorCol() );
    return false;
  }
  ...
  while( ( xml_viper_object =
             xml_source->IterateChildren( xml_viper_object ) ) )
  {
    ...
    // --- one named 'Location'
    // ------ whose children are 'data:bbox' with framespan and bounding box as
    // the attributes
    // --- one named 'Occlusion'
    // ------ whose children are 'data:fvalue' with framespan and occluded value
    // as the attributes
    // --- one for each of the 20 activities
    // ------ whose children are 'data:bvalue' with framespan [actM:actN] and
    // true/false as the attributes
    //
    // At any given frame, exactly one of the bvalues should be true.
    //
    // We'll split out tracks based on the framespan of the bvalues,
    // keeping in mind that each bvalue should be clipped by the object
    // framespan.
    //
    ...
    int viperID = -1;
    if( xmle->QueryIntAttribute( "id", &viperID ) != TIXML_SUCCESS )
    {
      return false;
    }
    ...
    // determine if gt is in appearance form divided into static and moving
    // categories
    ...
    {
      LOG_ERROR(
        main_logger, "XGTF reader: file '" << fn << "' row " << xmle->Row()
                                           << " track " << viperID <<
          ":unknown class '" << viper_class << "'" );
      return false;
    }
    ...
    if( !xmle->Attribute( "framespan" ) )
    {
      LOG_WARN(
        main_logger,
        "xgtf_reader: viper_class " << viper_class <<
          " has no framespan near row "
                                    << xmle->Row() <<
          "?  Skipping" );
      continue;
    }
    ...
    // =====
    // 4) From the children nodes of this (track) node, generate the list of
    // frames
    // =====
    ...
    for( viper_activity_list_c_it act_it = activities.begin();
         act_it != activities.end(); ++act_it )
    {
      //
      ...
      for( unsigned int frame = act_it->first.first;
           frame <= act_it->first.second; ++frame )
      {
        // look up the frame in the overall xgtf track
        map< unsigned,
          frame_handle_type >::const_iterator probe =
          xgtf_frame_map.find( frame );
        ...
        dst_schema[ this_frame ].bounding_box() =
          xgtf_schema[ lookup_frame ].bounding_box();
        dst_schema[ this_frame ].frame_number() =
          xgtf_schema[ lookup_frame ].frame_number();
        if( dst_schema[ lookup_frame ].occlusion.exists() )
        {
          dst_schema[ this_frame ].occlusion() =
            xgtf_schema[ lookup_frame ].occlusion();
        }
      }
  ...
  if( !warnings.empty() )
  {
    LOG_INFO(
      main_logger,
      "xgtf_reader: Warnings from loading '" << fn << "':" );
    warnings.dump_msgs();
