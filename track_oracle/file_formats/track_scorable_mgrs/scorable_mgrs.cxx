...
::align_zones( const scorable_mgrs& other ) const
{
  ...
  vector< int > ret( static_cast< int >( N_ZONES ),
    static_cast< int >( N_ZONES ) );
...
scorable_mgrs
::diff(
  const scorable_mgrs& pos1, const scorable_mgrs& pos2,
  const vector< int >& zone_map )
{
  ...
  scorable_mgrs& m )
{
  ...
  if( !( is >> m.entry_valid[ scorable_mgrs::DEFAULT_ZONE ] ) )
  {
    return set_bad( is );
  }
  if( !( is >> m.zone[ scorable_mgrs::DEFAULT_ZONE ] ) )
  {
    return set_bad( is );
  }
  if( !( is >> m.northing[ scorable_mgrs::DEFAULT_ZONE ] ) )
  {
    return set_bad( is );
  }
  if( !( is >> m.easting[ scorable_mgrs::DEFAULT_ZONE ] ) )
  {
    return set_bad( is );
  }
  if( ( !( is >> tmp ) ) && ( tmp != "alt" ) ) { return set_bad( is ); }
  if( ( !( is >> tmp ) ) && ( tmp != "v/z/n/e:" ) ) { return set_bad( is ); }
  if( !( is >> m.entry_valid[ scorable_mgrs::ALT_ZONE ] ) )
  {
    return set_bad( is );
  }
  if( !( is >> m.zone[ scorable_mgrs::ALT_ZONE ] ) ) { return set_bad( is ); }
  if( !( is >> m.northing[ scorable_mgrs::ALT_ZONE ] ) )
  {
    return set_bad( is );
  }
  if( !( is >> m.easting[ scorable_mgrs::ALT_ZONE ] ) )
  {
    return set_bad( is );
  }
  m.valid = true;
...
::find_zone( int z ) const
{
  ...
  if( this->entry_valid[ DEFAULT_ZONE ] && this->zone[ DEFAULT_ZONE ] == z )
  {
    return DEFAULT_ZONE;
  }
  if( this->entry_valid[ ALT_ZONE ] && this->zone[ ALT_ZONE ] == z )
  {
    return ALT_ZONE;
  }
  return N_ZONES;
