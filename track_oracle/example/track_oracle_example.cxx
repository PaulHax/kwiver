...
struct derived_track_throws_exception : public ::kwiver::track_oracle::
                                        track_base< derived_track_throws_exception, my_particular_track >
{
  track_field< double >& score;
...
struct derived_track : public ::kwiver::track_oracle::track_base< derived_track,
                         my_particular_track >
{
...
main( int argc, char* argv[] )
{
...

  {
    ...
    ::kwiver::track_oracle::frame_handle_list_type frames =
      track_oracle_core::get_frames( track_handle );
    ...
    if( argc > 1 )
    {
      ...
      if( probe.exists( f3.row ) )
      {
        LOG_INFO(
          main_logger,
          "Frame 3's '" << argv[ 1 ] << "' value is " << probe( f3.row ) <<
            "" );
      }
      else
      {
  ...
  // It's still there:
  //
  // THE TRACK'S DATA IS NOT ASSOCIATED WITH INSTANCES OF OUR TRACK DATA
  // STRUCTURE.
  //
  ...
  //
  {
    ...
    LOG_INFO(
      main_logger,
      "Before frame appending: " << t( track_handle ) << "" );
    t( track_handle ).add_frames( track_oracle_core::get_frames( temp ) );
    LOG_INFO(
      main_logger,
      "After frame appending: " << t( track_handle ) << "" );
  }

  // we can examine what fields a track or frame has
  {
    vector< field_handle_type > columns =
      track_oracle_core::fields_at_row( 1001 );
    LOG_INFO( main_logger, "Row 1001 has " << columns.size() << " fields:" );
    for( size_t i = 0; i < columns.size(); ++i )
    {
      element_descriptor e =
        track_oracle_core::get_element_descriptor( columns[ i ] );
      LOG_INFO(
        main_logger, i << ": " << e.name << " ( " << e.description << " ) type "
                       << e.typeid_str << " role: " <<
          element_descriptor::role2str( e.role ) << "" );
    }
    LOG_INFO( main_logger, "" );
  }
  ...
  {
    my_particular_track t;
    // this is an example of when it would be nice for track_fields to know if
    // they're associated with Track or Frame
    {
      LOG_INFO( main_logger, "Lookup main_logger, #1:" );
      LOG_INFO( main_logger, "" );

      frame_handle_type lookup_handle =
        frame_handle_type(
          t.timestamp.lookup(
            103,
            ::kwiver::track_oracle::DOMAIN_ALL ) );
...

    {
      ...
      LOG_INFO( main_logger, "" );
      // If the track_field is associated with the Frame, we can look it up
      // relative
      // to a track
      LOG_INFO( main_logger, "Lookup relative to track handle" );

      frame_handle_type lookup_handle =
        frame_handle_type( t.timestamp.lookup( 103, track_handle ) );
      if( lookup_handle.is_valid() )
      {
        track_oracle_frame_view f = t[ lookup_handle ].frame();
        LOG_INFO( main_logger, f << "" );
      ...
      else
      {
        LOG_ERROR(
          main_logger,
          "Couldn't find timestamp 103 relative to track handle?" );
      }
    }

    {
      ...
      track_field< ::kwiver::track_oracle::dt::tracking::timestamp_usecs >
      frame_term;
      frame_handle_type lookup( frame_term.lookup(
        103,
        ::kwiver::track_oracle::DOMAIN_ALL ) );
      LOG_INFO(
        main_logger,
        "Track field lookup on data_term: timestamp 103 found? " <<
          lookup.is_valid() );
...

  {
    ...
    LOG_INFO(
      main_logger,
      "A different track structure sees:\n" << t2( track_handle ) << "" );

    // note that t2 also has five frames, but since they have no fields
    // associated with them, they're empty from perspective of the t2
    // definition.
    // Hmm.
  }
  ...
  LOG_INFO(
    main_logger,
    "About to define a track field which re-types 'label'..." );
  try
  {
...

  {
    ...
    vector< element_descriptor > missing_fields =
      ::kwiver::track_oracle::schema_algorithm::name_missing_fields(
        mpt,
        adt_tracks );
    LOG_INFO(
      main_logger,
      "a_different_track schema is missing " << missing_fields.size()
                                             <<
...

  {
    ...
    LOG_INFO(
      main_logger,
      "About to create an instance of a schema with a duplicated field; should throw..." );
