...
main( int argc, char* argv[] )
{
  ...
  vul_arg< string > output_prefix_arg( "-out",
    "Output prefix (will write .geom.yml, .activity.yml" );
  vul_arg< int > activity_domain_arg( "-d", "Activity KPF domain", 2 );

  vul_arg_parse( argc, argv );
  file_format_manager::initialize();

  if( !( geom_in_fn_arg.set() && act_in_fn_arg.set() &&
         output_prefix_arg.set() ) )
  {
  ...
  if( !file_format_manager::read( geom_in_fn_arg(), input_geom_tracks ) )
  {
    LOG_ERROR(
      main_logger,
      "Couldn't read input geometry from '" << geom_in_fn_arg() << "'" );
    return EXIT_FAILURE;
  }
  LOG_INFO(
    main_logger,
    "Read " << input_geom_tracks.size() << " input geometry tracks" );

  track_handle_list_type activity_tracks;
  if( !track_filter_kpf_activity::read(
    act_in_fn_arg(),
    input_geom_tracks,
    activity_domain_arg(),
    activity_tracks ) )
  {
    LOG_ERROR(
      main_logger,
      "Couldn't read activity tracks from '" << act_in_fn_arg() << "'" );
    return EXIT_FAILURE;
  }

  LOG_INFO(
    main_logger,
    "Generated " << activity_tracks.size() << " activity tracks" );

  {
    ...
    LOG_INFO(
      main_logger,
      "Wrote KPF geometry to " << geom_out_fn << " success: " << okay );
  }

  {
    ...
    LOG_INFO(
      main_logger,
      "Wrote KPF activity to " << act_out_fn << " success: " << okay );
  }
}
