...
main( int argc, char* argv[] )
{
  if( argc == 2 )
  {
    ...
    if( ( opt == "-h" ) || ( opt == "-?" ) )
    {
      LOG_INFO(
        main_logger,
        "Usage: " << argv[ 0 ] << " [field-name] [field-name ...]\n"
                  <<
...
dump_all_formats()
{
  for( size_t tfi = TF_BEGIN; tfi != TF_INVALID_TYPE; ++tfi )
  {
    ...
    if( !ff )
    {
      LOG_INFO(
        main_logger,
        "No format manager for enumeration " << tfi << "?" );
      continue;
    }

    map< field_handle_type,
      track_base_impl::schema_position_type > schema_elements =
      ff->enumerate_schema();
    LOG_INFO(
      main_logger,
      "Format " << file_format_type::to_string( ff->get_format() ) << " (" <<
        ff->format_description() << ") contains "
                << schema_elements.size() << " elements:" );
    for( map< field_handle_type,
      track_base_impl::schema_position_type >::const_iterator i =
           schema_elements.begin();
         ...
         ++i )
    {
      ...
      switch( i->second )
      {
        case track_base_impl::IN_TRACK: oss << "(track)";
          break;
        case track_base_impl::IN_FRAME: oss << "(frame)";
          break;
        default: oss << "(unknown?)";
          break;
      }
...
match_against_schema( int argc, char* argv[] )
{
  ...
  // what matches it?
  vector< file_format_enum > matches =
    file_format_manager::format_matches_schema( schema );
