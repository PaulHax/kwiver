...
int load_tracks(
  const string& fn, track_handle_list_type& tracks,
  const string& unique_key, bool kpf_any );
int probe_formats( const string& fn );

int
main( int argc, char* argv[] )
{
  vul_arg< bool > probe_arg( "-probe",
    "probe the file with each format's inspection routine" );
...
#if KWIVER_ENABLE_KPF
  vul_arg< string > kpf_g_arg( "-kpf-g",
    "write out as KPF geometry to this file" );
  vul_arg< bool > kpf_any_arg( "-kpf-any", "read file as unstructured yaml" );
#else
  auto kpf_any_arg = [](){ return false; };
#endif
  vul_arg< string > csv_v1_arg( "-csv-v1",
    "write out as old-style csv to this file" );
  vul_arg< string > kwxml_ts_arg( "-kwxml_ts",
    "if writing kwxml, set default track style to this", "trackObjectKitware" );
  vul_arg< string > tag_arg( "-tag",
    "if writing kwiver, test tags by setting track-level 'test' flag to this" );
  vul_arg< string > unique_arg( "-u",
    "enumerate unique XML values of this field", "" );
  vul_arg< string > test_tracks_arg( "-t",
    "write test tracks to this file and exit", "" );
  vul_arg< string > test_headers_arg( "-th",
    "test tracks will only contain these headers (plus a few others)", "" );
  ...
  if( test_tracks_arg.set() )
  {
    ...
    if( test_headers_arg.set() )
    {
      ...
      if( !is )
      {
        LOG_ERROR(
          main_logger,
          "Couldn't open test headers file '" << test_headers_arg() << "'" );
        return EXIT_FAILURE;
      }
      ...
      LOG_INFO(
        main_logger,
        "Found " << values.size() << " headers naming " <<
          keep_headers.size() << " elements in "  << test_headers_arg() );
    }

    bool rc = file_format_manager::write_test_tracks(
      test_tracks_arg(),
      keep_headers );
  ...
  if( !fn_arg.set() )
  {
    LOG_INFO(
      main_logger,
      "Usage: " << argv[ 0 ] <<
        " [-probe] [-k kwxml-output-file] -f trackfile\n"
                <<
  ...
  else
  {
    ...
    if( kw18_arg.set() )
    {
      bool okay = file_format_manager::get_format( TF_KW18 )->write(
        kw18_arg(),
        tracks );
      LOG_INFO(
        main_logger,
        "Wrote kw18 to " << kw18_arg() << " success: " << okay );
    }
    if( csv_arg.set() || csv_v1_arg.set() )
    {
      ...
      if( os )
      {
        bool okay = track_oracle_core::write_csv(
          os, tracks,
          csv_v1_semantics );
        LOG_INFO(
          main_logger,
          "Wrote CSV to " << csv_arg() << " success: " << okay <<
            " : v1 semantics " << csv_v1_semantics );
      }
      else
      {
        LOG_ERROR(
          main_logger,
          "Could not write CSV to '" << csv_arg() << "'" );
      }
    }
#if KWIVER_ENABLE_KPF
    if( kpf_g_arg.set() )
    {
      bool okay =
        file_format_manager::get_format( TF_KPF_GEOM )->write(
          kpf_g_arg(),
          tracks );
      LOG_INFO(
        main_logger,
        "Wrote KPF geometry to " << kpf_g_arg() << " success: " << okay );
    }
#endif
  }
  return rc;
...
int
probe_formats( const string& track_fn )
{
  ...
  {
    LOG_INFO(
      main_logger, file_format_type::to_string( i->second->get_format() )
...
int
load_tracks(
  const string& track_fn, track_handle_list_type& tracks,
  const string& unique_key, bool kpf_any )
{
#if KWIVER_ENABLE_KPF
  if( kpf_any )
...
#endif
  {
    if( !file_format_manager::read( track_fn, tracks ) )
    {
      LOG_ERROR(
        main_logger,
        "Error: couldn't read tracks from '" << track_fn << "'; exiting" );
      return EXIT_FAILURE;
    }
  }
  if( tracks.empty() )
  {
    LOG_INFO(
      main_logger,
      "Info: reader succeeded but no tracks were loaded?  Weird!" );
    return EXIT_FAILURE;
  }
  ...
  LOG_INFO(
    main_logger, "Loaded " << tracks.size() << " tracks, "
                           << "format: " <<
      file_format_type::to_string( ffs( tracks[ 0 ] ).format() )
                           << "" );
  ...
  for( size_t i = 0; i < tracks.size(); ++i )
  {
    vector< field_handle_type > ts =
      track_oracle_core::fields_at_row( tracks[ i ].row );
    for( size_t t = 0; t < ts.size(); ++t ) { ++track_stats[ ts[ t ] ]; }

    frame_handle_list_type frames =
      track_oracle_core::get_frames( tracks[ i ] );
     ...
     *  for (size_t j=0; j<frames.size(); ++j)
     *  {
     *  vector< field_handle_type > fs = track_oracle_core::fields_at_row(
     * frames[j].row );
     *  for (size_t t=0; t<fs.size(); ++t) ++frame_stats[ fs[t] ];
     *  }
     */
    for( auto fields_at_frame :
         track_oracle_core::fields_at_rows( frame_handles ) )
    {
    ...
    if( timer.real() > 5 * 1000 )
    {
      LOG_INFO(
        main_logger,
        "Computed stats on " << i << " of " << tracks.size() << "; " <<
          frame_count << " frames" );
      timer.mark();
    }
  }
       ...
       ++i )
  {
    element_descriptor e =
      track_oracle_core::get_element_descriptor( i->first );
    LOG_INFO(
      main_logger,
      i->second << " / " << tracks.size() << ":\t" << e.name << "" );
  }
       ...
       ++i )
  {
    ...
    LOG_INFO(
      main_logger,
      i->second << " / " << frame_count << ":\t" << e.name << "" );
  }

  // if requested, display unique values and counts of requested field
  if( unique_key != "" )
  {
    const element_store_base* const esb =
      track_oracle_core::get_element_store_base(
        track_oracle_core::lookup_by_name( unique_key ) );
    if( !esb )
    {
      LOG_ERROR(
        main_logger,
        "No such field '" << unique_key << "' in the tracks" );
      return EXIT_FAILURE;
    }
    ...
    LOG_INFO(
      main_logger,
      "Counts of '" << unique_key << "' values ( N = " << counts.size() <<
        " ):" );
