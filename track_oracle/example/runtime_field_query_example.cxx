...
///
/// An example program demonstrating querying for a dynamically selected
/// attribute.
...
main( int argc, char* argv[] )
{
  ...
  {
    LOG_INFO(
      main_logger, "Usage: " << argv[ 0 ] << " track-file field_name\n"
                             <<
  ...
  for( size_t t = 0; t < tracks.size(); ++t )
  {
    ...
    frame_handle_list_type frames =
      track_oracle_core::get_frames( tracks[ t ] );
  ...
  LOG_INFO(
    main_logger,
    "Info: found " << count << " instances of '" << argv[ 2 ] <<
      "' in the file" );
  if( count > 0 )
  {
    LOG_INFO(
      main_logger,
      "Info: ... the sum was " << sum << " and the average was " <<
        sum / count << "" );
  }
}
