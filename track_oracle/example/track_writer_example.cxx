...
main( int argc, char* argv[] )
{
  if( argc < 3 )
  {
    LOG_INFO(
      main_logger,
      "Usage: " << argv[ 0 ] << " input_file format [output_file]\n\n" <<
  ...
  if( !file_format_manager::read( argv[ 1 ], tracks ) )
  {
    LOG_ERROR(
      main_logger,
      "Error: could not read tracks from '" << argv[ 1 ] << '\'' );
    return EXIT_FAILURE;
  }
  ...
  if( ff_type == TF_INVALID_TYPE )
  {
    LOG_ERROR(
      main_logger,
      "Error: could not find format '" << argv[ 2 ] << '\'' );
    return EXIT_FAILURE;
  }
  ...
  if( !ff )
  {
    LOG_ERROR(
      main_logger,
      "Error: failed to load file format manager for '" << argv[ 2 ] << '\'' );
    return EXIT_FAILURE;
  }
  ...
  if( argc > 3 )
  {
    if( !ff->write( argv[ 3 ], tracks ) )
    {
      LOG_ERROR(
        main_logger,
        "Error: failed to write tracks to '" << argv[ 3 ] << '\'' );
      return EXIT_FAILURE;
    }
  }
  else
