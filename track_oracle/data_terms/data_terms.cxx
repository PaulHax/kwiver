  ...
  T& d )
{
  ...
  {
    map< string,
      string >::const_iterator p = header_value_map.find( headers[ i ] );
...
ostream&
track_location
::to_stream( ostream& os, const vgl_point_2d< double >& d ) const
{
...
ostream&
track_location
::to_csv( ostream&  os, const vgl_point_2d< double >& d ) const
{
  return kwiver_write( os, d, "," );
}
...
bool
track_location
::from_str( const string& s, vgl_point_2d< double >& d ) const
{
...
ostream&
bounding_box
::to_stream( ostream& os, const vgl_box_2d< double >& d ) const
{
  return kwiver_write( os, d, " " );
}
...
ostream&
bounding_box
::to_csv( ostream& os, const vgl_box_2d< double >& d ) const
{
  return kwiver_write( os, d, "," );
}
...
bool
bounding_box
::from_str( const string& s, vgl_box_2d< double >& d ) const
{
  return kwiver_read( s, d );
}
...
bool
bounding_box
::read_xml( const TiXmlElement* e, vgl_box_2d< double >& d ) const
{
...
ostream&
world_x
::to_stream( ostream& os, const double& d ) const
{
...
ostream&
world_location
::to_stream( ostream& os, const vgl_point_3d< double >& d ) const
{
...
ostream&
world_location
::to_csv( ostream& os, const vgl_point_3d< double >& d ) const
{
  streamsize old_prec = os.precision( 10 );
  kwiver_write( os, d, "," );
  os.precision( old_prec );
...
bool
world_location
::from_str( const string& s, vgl_point_3d< double >& d ) const
{
  return kwiver_read( s, d );
}
...
ostream&
time_stamp
::to_stream( ostream& os, const vital::timestamp& ts ) const
{
...
bool
time_stamp
::from_str( const string& s, vital::timestamp& ts ) const
{
...
bool
time_stamp
::read_xml( const TiXmlElement* e, vital::timestamp& ts ) const
{
...
::write_xml(
  ...
  const vital::timestamp& ts ) const
{
...
{
  map< string,
    string >::const_iterator ts_f = header_value_map.find( "ts_frame" );
  map< string,
    string >::const_iterator ts_t = header_value_map.find( "ts_time" );
  bool okay = true;
...
ostream&
time_stamp
::to_csv( ostream& os, const vital::timestamp& ts ) const
{
  pair< string, string > ts_strings = kwiver_ts_to_strings( ts );
...
ostream&
track_uuid
::to_stream( ostream& os, const vital::uid& uid ) const
{
...
bool
track_uuid
::from_str( const string& s, vital::uid& uid ) const
{
...
ostream&
event_type
::to_stream( ostream& os, const int& d ) const
{
...
bool
event_type
::from_str( const string& s, int& d ) const
{
  bool okay = false;
  try
  {
...
ostream&
event_type
::to_csv( ostream& os, const int& d ) const
{
  os << "virat,";
...
bool
event_type
::from_csv( const map< string, string >& header_value_map, int& d ) const
{
  map< string,
    string >::const_iterator e_d = header_value_map.find( "event_domain" );
  map< string,
    string >::const_iterator e_t = header_value_map.find( "event_type" );
  bool okay = true;
...
void
event_type
::write_xml( ostream& os, const string& indent, const int& d ) const
{
  os << indent << "<" << event_type::c.name << " domain=\"virat\"  index=\"" <<
    d << "\" > ";
...
bool
event_type
::read_xml( const TiXmlElement* const_e, int& d ) const
{
...
ostream&
source_track_ids
::to_stream( ostream& os, const vector< unsigned >& d ) const
{
...
bool
source_track_ids
::from_str( const string& s, vector< unsigned >& d ) const
{
...
ostream&
actor_track_rows
::to_stream( ostream& os, const track_handle_list_type& d ) const
{
  vector< unsigned > tmp;
  for( auto i : d )
  {
...
bool
actor_track_rows
::from_str( const string& s, track_handle_list_type& d ) const
{
  vector< unsigned > tmp;
...
::to_stream( ostream& os, const vector< double >& d ) const
{
  for( size_t i = 0, n = d.size(); i < n; ++i )
  {
    os << d[ i ];
...
bool
descriptor_classifier
::from_str( const string& s, vector< double >& d ) const
{
  string src( s );
  size_t found = src.find_first_of( ":" );
  while( found != string::npos )
  ...
  const vector< double >& d ) const
{
  ...
  os << indent << "<" << descriptor_classifier::c.name << " size=\"" << n <<
    "\" n_nonzero=\"" << n_nonzero << "\" >\n";
  for( size_t i = 0; i < n; ++i )
  {
    if( d[ i ] != 0 )
    {
      ...
      if( probe == i2a.end() )
      {
        os << "<!-- invalid activity index " << i << " (probability " <<
          d[ i ] << ") -->\n";
      }
      else
      {
        os << indent << "  <probability activity=\"" << probe->second <<
          "\" value=\"" << d[ i ] << "\" />\n";
      }
    }
  }
...
bool
descriptor_classifier
::read_xml( const TiXmlElement* const_e, vector< double >& d ) const
{
  TiXmlElement* e = const_cast< TiXmlElement* >( const_e );
  const map< size_t, string >& i2a = aries_interface::index_to_activity_map();
    ...
    try
    {
      activity_idx =
        kwiver::track_oracle::aries_interface::activity_to_index(
          probNode->Attribute( "activity" ) );
    }
    catch( kwiver::track_oracle::aries_interface_exception& /*e*/ )
    {
      LOG_ERROR(
        main_logger, "Couldn't recognize " << probNode->Attribute( "activity" )
                                           << " as a valid activity?" );
      return false;
    }
