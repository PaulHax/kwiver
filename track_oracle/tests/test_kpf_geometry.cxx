...
static std::string kw18_tracks_file = "track_oracle_data/generic_tracks.kw18";
static std::string kpf_tracks_file =
  "track_oracle_data/generic_tracks.geom.yml";
...
TEST ( track_oracle, file_globs )
{
  ...
  for( const auto& g : test_globs )
  {
    ...
    EXPECT_EQ(
      matching_formats.size(),
      g.first ) << "Glob '" << g.second << "' matches " << g.first <<
      " formats";
  }
}
...
TEST ( track_oracle, kpf_geometry )
{
  ...
  {
    string fn = g_data_dir + "/" + kw18_tracks_file;
  ...
  {
    string fn = g_data_dir + "/" + kpf_tracks_file;
  ...
  EXPECT_EQ(
    kpf_tracks.size(),
    kpf_tracks.size() ) << " same number of tracks in kw18 and kpf";
  ...
  EXPECT_EQ(
    kpf_tracks.size(),
    kpf_map.size() ) << " same number of KPF tracks in map";
  EXPECT_EQ(
    kw18_tracks.size(),
    kw18_map.size() ) << " same number of kw18 tracks in map";
  ...
  for( const auto& kw18 : kw18_map )
  {
    ...
    EXPECT_TRUE(
      kpf_probe !=
      kpf_map.end() ) << " kw18 track " << kw18_id << " is in KPF";
    ...
    EXPECT_EQ(
      n_kw18_frames,
      kw18_frame_map.size() ) << " track " << kw18_id <<
      " same number of kw18 frames in map";
    EXPECT_EQ(
      n_kpf_frames,
      kpf_frame_map.size() ) << " track " << kw18_id <<
      " same number of kpf frames in map";
    ...
    for( const auto& kw18_f : kw18_frame_map )
    {
      ...
      EXPECT_TRUE(
        kpf_frame_probe !=
        kpf_frame_map.end() ) << " kw18 track " << kw18_id << " frame " <<
        kw18_frame_num << " exists in kpf";
      ...
      EXPECT_NEAR(
        kw18_box.min_x(), kpf_box.min_x(),
        1.0e-8 ) << " track " << kw18_id << " frame " << kw18_frame_num;
      EXPECT_NEAR(
        kw18_box.min_y(), kpf_box.min_y(),
        1.0e-8 ) << " track " << kw18_id << " frame " << kw18_frame_num;
      EXPECT_NEAR(
        kw18_box.max_x(), kpf_box.max_x(),
        1.0e-8 ) << " track " << kw18_id << " frame " << kw18_frame_num;
      EXPECT_NEAR(
        kw18_box.max_y(), kpf_box.max_y(),
        1.0e-8 ) << " track " << kw18_id << " frame " << kw18_frame_num;
    }
  } // ...for each track
}
