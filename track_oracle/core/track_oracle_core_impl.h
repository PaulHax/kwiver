...
struct csv_element
{
  // the field handle being emitted, or INVALID_FIELD_HANDLE for bookkeeping
  // field
  // such as _track_sequence or _parent_track
  ::kwiver::track_oracle::field_handle_type fh;

  // true if the field defines a default value which should be emitted if not
  // explicitly
  ...
  csv_element( ::kwiver::track_oracle::field_handle_type f, bool e, bool t )
    : fh( f ),
      emit_default_if_absent( e ),
      is_track( t )
  {}

  csv_element()
    : fh( ::kwiver::track_oracle::INVALID_FIELD_HANDLE ),
      emit_default_if_absent( false ),
      is_track( false )
  {}
};
...
private:
  ...
  template < typename T > std::pair< std::map< oracle_entry_handle_type, T >*,
    T > lookup_table( field_handle_type field );
  ...
  void get_csv_columns(
    const oracle_entry_handle_type& row,
    std::map< field_handle_type, std::vector< std::string > >& m ) const;
...
public:
  ...
  template < typename T > field_handle_type create_element(
    const element_descriptor& e );
  ...
  element_store_base* get_mutable_element_store_base(
    field_handle_type f ) const;
  ...
  // Given a {track,frame}/field (row/column) location, return a ref to the
  // value
  ...
  template < typename T > std::pair< bool,
    T > get( oracle_entry_handle_type row, field_handle_type field );
  ...
  template < typename T > void remove_field(
    oracle_entry_handle_type row,
    field_handle_type field );

  // return fields which have entries at this row
  std::vector< field_handle_type > fields_at_row(
    oracle_entry_handle_type row ) const;

  // return fields which have entries for these rows (faster than one-at-time
  // calls to above)
  std::vector< std::vector< field_handle_type > > fields_at_rows(
    const std::vector< oracle_entry_handle_type >& rows ) const;

  // return the row containing the value-- return first if multiple; probably
  // should alert somehow
  template < typename T > oracle_entry_handle_type lookup(
    field_handle_type field, const T& val, domain_handle_type domain );

  // return the frames associated with the handle without requiring a specific
  // schema
  frame_handle_list_type get_frames( const track_handle_type& t );

  // hammer the list of frames as the associated frames for the track (may leak
  // until a GC is in t_o)
  void set_frames(
  ...
  domain_handle_type lookup_domain(
