  ...
  const track_handle_list_type tracks )
{
  ...
  for( map< field_handle_type, bool >::const_iterator i =
         missing_fields_all_tracks.begin();
...
// what elements in the first (reference) schema are missing in the second
// (candidate) schema?
vector< element_descriptor >
schema_compare(
  const track_base_impl& ref,
  const track_base_impl& candidate )
{
  vector< element_descriptor > ret;
  typedef map< field_handle_type,
    track_base_impl::schema_position_type > schema_map_type;
  typedef map< field_handle_type,
    track_base_impl::schema_position_type >::const_iterator schema_map_cit;
