...
::kwiver::track_oracle::track_oracle_core_impl* ::kwiver::track_oracle::
track_oracle_core::impl = 0;
...
public:
  ...
  csv_v1_mapping_type( const string& c, const string& v, bool th, bool ed )
    : valid( true ),
      current_name( c ),
      v1_name( v ),
      fh( kwiver::track_oracle::INVALID_FIELD_HANDLE ),
      is_track_header( th ),
      emit_default( ed )
  {}
  csv_v1_mapping_type()
  ...
  {
    return ( this->valid &&
             ( this->fh != kwiver::track_oracle::INVALID_FIELD_HANDLE ) );
  }

  static map< kwiver::track_oracle::field_handle_type,
    vector< string > > remap_from_existing_headers(
    vector< csv_v1_mapping_type >& v1_types,
    bool is_track_header,
    const map< kwiver::track_oracle::field_handle_type,
      kwiver::track_oracle::element_store_base* >& element_pool );
};
    ...
    kwiver::track_oracle::element_store_base* >& element_pool )
{
  ...
  for( size_t i = 0; i < v1_types.size(); ++i )
  {
    ...
    //
    // Plow through the element pool, looking for v1_col.current_name in the
    // csv_headers
    //
    kwiver::track_oracle::field_handle_type found_fh =
      kwiver::track_oracle::INVALID_FIELD_HANDLE;
    for( map< kwiver::track_oracle::field_handle_type,
      kwiver::track_oracle::element_store_base* >::const_iterator p =
           element_pool.begin();
         ( p != element_pool.end() ) &&
         ( found_fh == kwiver::track_oracle::INVALID_FIELD_HANDLE );
         ++p )
    {
      const vector< string >& headers = p->second->csv_headers();
      if( find(
        headers.begin(), headers.end(),
        v1_col.current_name ) != headers.end() )
      {
        found_fh = p->first;
      }
    }
    if( found_fh == kwiver::track_oracle::INVALID_FIELD_HANDLE )
    {
      throw runtime_error(
        "v1-mapping couldn't find '" + v1_col.current_name +
        "'" );
    }
    v1_col.fh = found_fh;
...
::track_oracle_core_impl()
  : field_count( 0 ),
    row_count( 1000 ),
    last_domain_allocated( DOMAIN_ALL )
{
...
{
  map< string,
    field_handle_type >::const_iterator probe = this->name_pool.find( name );
  field_handle_type h =
...
{
  map< string,
    field_handle_type >::const_iterator probe = this->name_pool.find( fn );
  if( probe == this->name_pool.end() )
  {
    throw runtime_error( "Lost required field '" + fn + "'" );
  }
  return probe->second;
}
...
::get_element_descriptor( field_handle_type f ) const
{
  ...
  map< field_handle_type,
    element_store_base* >::const_iterator probe = this->element_pool.find( f );
  return
...
bool
track_oracle_core_impl
::unlocked_field_has_row(
  oracle_entry_handle_type row,
  field_handle_type field ) const
{
  if( field == INVALID_FIELD_HANDLE ) { return false; }

  map< field_handle_type,
    element_store_base* >::const_iterator probe =
    this->element_pool.find( field );
...
::fields_at_row( oracle_entry_handle_type row ) const
{
  std::lock_guard< std::mutex > lock( this->api_lock );
  vector< field_handle_type > ret;
  for( map< field_handle_type, element_store_base* >::const_iterator i =
         this->element_pool.begin();
...
::get_domain( domain_handle_type domain )
{
  ...
  if( domain != DOMAIN_ALL )
  {
    map< domain_handle_type,
      handle_list_type >::const_iterator i = this->domain_pool.find( domain );
...
::lookup_domain( const string& domain_name, bool create_if_not_found )
{
  std::lock_guard< std::mutex > lock( this->api_lock );
  map< string,
    domain_handle_type >::iterator probe =
    this->domain_names.find( domain_name );
...
{
  for( map< field_handle_type, element_store_base* >::iterator i =
         this->element_pool.begin();
       i != this->element_pool.end();
       ++i )
  {
...
::release_domain( const domain_handle_type& domain )
{
  ...
  map< domain_handle_type,
    handle_list_type >::iterator probe = this->domain_pool.find( domain );
  if( probe == this->domain_pool.end() )
  {
    LOG_ERROR(
      main_logger,
      "release_domain called on un-allocated domain " << domain );
    return false;
  }

  field_handle_type flh = this->unlocked_lookup_by_name( "__frame_list" );
  if( flh == INVALID_FIELD_HANDLE )
  {
    throw runtime_error( "Lost __frame_list" );
  }

  const pair< map< oracle_entry_handle_type, frame_handle_list_type >*,
    frame_handle_list_type >& frame_list_lookup =
    this->lookup_table< frame_handle_list_type >( flh );
  const map< oracle_entry_handle_type,
    frame_handle_list_type >& frame_lists = *frame_list_lookup.first;
  ...
  for( size_t i = 0; i < handles.size(); ++i )
  {
    ...
    map< oracle_entry_handle_type,
      frame_handle_list_type >::const_iterator frame_probe =
      frame_lists.find( handles[ i ] );
  ...
  // erase domain name
  for( map< string, domain_handle_type >::iterator i =
         this->domain_names.begin();
...
{
  return this->unlocked_get_field< frame_handle_list_type >(
    t.row,
    this->lookup_required_field( "__frame_list" ) );
}
...
{
  std::lock_guard< std::mutex > lock( this->api_lock );
  this->unlocked_get_field< frame_handle_list_type >(
    t.row,
    this->lookup_required_field( "__frame_list" ) ) = frames;
}
...
::get_n_frames( const track_handle_type& t )
{
  ...
  return this->unlocked_get_field< frame_handle_list_type >(
    t.row,
    this->lookup_required_field( "__frame_list" ) ).size();
}
  ...
  const oracle_entry_handle_type& dst )
{
  ...
  if( dst == INVALID_ROW_HANDLE )
  {
    LOG_ERROR(
      main_logger,
      "clone_nonsystem_fields: uninitialized destination" );
    return false;
  }
...
track_oracle_core_impl
::emit_pool_as_kwiver(
  ostream& os, const string& indent,
  oracle_entry_handle_type row ) const
{
  for( map< field_handle_type, element_store_base* >::const_iterator j =
         this->element_pool.begin();
...
{
  field_handle_type external_id_fh =
    this->unlocked_lookup_by_name( "external_id" );
  if( external_id_fh == INVALID_FIELD_HANDLE )
  {
    throw runtime_error( "emit-csv lost external ID column?" );
  }
  ...
  for( size_t i = 0; i < n; ++i )
  {
    ...
    {
      map< field_handle_type,
        element_store_base* >::const_iterator p =
        this->element_pool.find( target_fh );
      if( p->first == INVALID_FIELD_HANDLE )
      {
        if( p == this->element_pool.end() )
        {
          throw runtime_error( "Lost a data column writing CSV" );
        }
      }
          ...
          ( target_fh == external_id_fh ) )
      {
        ...
        field_handle_type pt_fh =
          this->unlocked_lookup_by_name( "__parent_track" );
        map< field_handle_type,
          element_store_base* >::const_iterator pt_probe =
          this->element_pool.find( pt_fh );
        element_store< oracle_entry_handle_type >* es_ptr =
          dynamic_cast< element_store< oracle_entry_handle_type >* >( pt_probe->
                                                                      second );
        if( !es_ptr )
        {
          throw runtime_error( "Bad column cast looking for external id" );
        }

        map< oracle_entry_handle_type,
          oracle_entry_handle_type >::const_iterator pt_row_probe =
          es_ptr->storage.find( row );
        if( pt_row_probe == es_ptr->storage.end() )
        {
          throw runtime_error( "Frame has no parent track?" );
        }
        emitted_row = pt_row_probe->second;
      }

      p->second->emit_as_csv(
        os, emitted_row,
        output_order[ i ].emit_default_if_absent );
    }
...
::write_kwiver( ostream& os, const track_handle_list_type& tracks )
{
  ...
  os << "<kwiver>\n"
     << "<!--\n"
     << "  File of " << tracks.size() <<
    " tracks emitted by track_oracle's kwiver writer\n"
     << "-->\n";
  ...
  for( size_t i = 0; i < tracks.size(); ++i )
  {
    ...
    os << indents[ 0 ] << "<track index=\"" << i << "\" len=\"" <<
      frames.size() << "\" >\n";
  ...
  bool csv_v1_semantics )
{
  ...
  if( ( frame_headers.find( this->unlocked_lookup_by_name( "world_x" ) ) !=
        frame_headers.end() )
      &&
      ( frame_headers.find( this->unlocked_lookup_by_name( "world_y" ) ) !=
        frame_headers.end() ) )
  {
    bool gcs_in_frame = frame_headers.find( world_gcs_fh ) !=
                        frame_headers.end();
    bool gcs_in_track = track_headers.find( world_gcs_fh ) !=
                        track_headers.end();
    if( !( gcs_in_frame || gcs_in_track ) )
    {
      frame_headers[ world_gcs_fh ] =
        this->element_pool[ world_gcs_fh ]->csv_headers();
    }
  }

  // another special case for world_gcs: if it's defined on the track, delete
  // that,
  ...
  if( csv_v1_semantics )
  {
    ...
    v1_order.push_back(
      csv_v1_mapping_type(
        ...
        is_track_header, no_emit_default ) );
    v1_order.push_back(
      csv_v1_mapping_type(
        "track_uuid", "unique_id",
        is_track_header, YES_EMIT_DEFAULT ) );
    v1_order.push_back(
      csv_v1_mapping_type(
        "frame_number", "frame_number",
        is_frame_header, no_emit_default ) );
    v1_order.push_back(
      csv_v1_mapping_type(
        "timestamp_usecs",
        "timestamp_usecs", is_frame_header, no_emit_default ) );
    v1_order.push_back(
      csv_v1_mapping_type(
        "obj_location_x", "obj_x",
        is_frame_header, no_emit_default ) );
    v1_order.push_back(
      csv_v1_mapping_type(
        "obj_location_y", "obj_y",
        is_frame_header, no_emit_default ) );
    v1_order.push_back(
      csv_v1_mapping_type(
        "bounding_box_ul_x",
        "obj_bbox_ul_x", is_frame_header, no_emit_default ) );
    v1_order.push_back(
      csv_v1_mapping_type(
        "bounding_box_ul_y",
        "obj_bbox_ul_y", is_frame_header, no_emit_default ) );
    v1_order.push_back(
      csv_v1_mapping_type(
        "bounding_box_lr_x",
        "obj_bbox_lr_x", is_frame_header, no_emit_default ) );
    v1_order.push_back(
      csv_v1_mapping_type(
        "bounding_box_lr_y",
        "obj_bbox_lr_y", is_frame_header, no_emit_default ) );
    v1_order.push_back(
      csv_v1_mapping_type(
        "world_x", "world_x",
        is_frame_header, no_emit_default ) );
    ...
    v1_order.push_back(
      csv_v1_mapping_type(
        "world_z", "world_z",
        is_frame_header, YES_EMIT_DEFAULT ) );
    v1_order.push_back(
      csv_v1_mapping_type(
        "world_gcs", "world_gcs",
        is_frame_header, YES_EMIT_DEFAULT ) );
    ...
    for( size_t i = 0; i < v1_order.size(); ++i )
    {
      ...
      if( !this_col.is_valid() )
      {
        LOG_WARN(
          main_logger,
          "CSV version 1: Column '" << this_col.v1_name <<
            "' invalid; will not be emitted" );
        continue;
      }

      bool emit_default = ( ( this_col.v1_name == "world_gcs" ) ||
                            ( this_col.v1_name == "unique_id" ) );
  ...
  LOG_DEBUG(
    main_logger, "Writing " << track_headers.size() << " track headers, "
                            << frame_headers.size() <<
      " frame headers; output order length "
                            << output_order.size() );
  ...
  for( size_t i = 0; i < tracks.size(); ++i )
  {
    ...
    this->emit_pool_as_csv(
      os, output_order, tracks[ i ].row, i,
      /* is_track = */ true, csv_v1_semantics );

    frame_handle_list_type frames = this->unlocked_get_frames( tracks[ i ] );
    for( size_t j = 0; j < frames.size(); ++j )
    {
      this->emit_pool_as_csv(
        os, output_order, frames[ j ].row, i,
        /* is_track = */ false, csv_v1_semantics );
    }
  }
...
::get_csv_handler_map( const vector< string >& headers )
{
       ...
       ++i )
  {
    ...
    if( local_debug )
    {
      LOG_DEBUG(
        main_logger,
        "gchm: " << local_debug_name << " has " << element_headers.size() <<
          " headers" );
    }
    ...
    for( size_t j = 0; j < element_headers.size(); ++j )
    {
      ...
      vector< string >::const_iterator p = find(
        headers.begin(), headers.end(),
        this_header );
      if( local_debug )
      {
        LOG_DEBUG(
          main_logger,
          "gchm: " << local_debug_name << " ('" << this_header <<
            "') in caller's header? " << ( p != headers.end() ) );
      }
      ...
      map< string,
        field_handle_type >::iterator check_p = header_map.find( this_header );
      if( check_p->second != INVALID_FIELD_HANDLE )
      {
        // whoops: name the offenders
        const element_descriptor& that_ed =
          this->element_pool[ check_p->second ]->get_descriptor();
        throw runtime_error(
          "CSV header collision: header '" + element_headers[ j ] +
          "' is claimed by both " +
      ...
      if( local_debug )
      {
        LOG_DEBUG(
          main_logger,
          "gchm: " << local_debug_name << " is at header index " <<
            this_header_index << " and has handle " << i->first );
      }
    ...
    if( element_headers.size() != header_indices.size() )
    {
      LOG_ERROR(
        main_logger,
        "CSV header: " << this_ed.name << " requires " <<
          element_headers.size() <<
          " headers but only " << header_indices.size() << " supplied" );
      {
        ostringstream oss;
        for( size_t j = 0; j < element_headers.size(); ++j )
        {
          oss << element_headers[ j ] << " ";
        }
        LOG_ERROR(
          main_logger,
          "CSV header: requires headers '" << oss.str() << "'" );
      }
      {
        ostringstream oss;
        for( size_t j = 0; j < header_indices.size(); ++j )
        {
          oss << headers[ header_indices[ j ] ] << " ";
        }
        LOG_ERROR(
          main_logger,
          "CSV header: supplied headers '" << oss.str() << "'" );
      }
    ...
    else
    {
      ...
      if( local_debug )
      {
        LOG_DEBUG(
          main_logger,
          "gchm: " << local_debug_name << " mapping " << i->first << " to " <<
            header_indices.size() << " headers" );
      }
    }
       ...
       ++i )
  {
    ...
    throw runtime_error(
      "CSV parser: header " + headers[ i->first ] +
      " used multiple times?" );
  }
