...
::remove_me()
{
  ...
  vector< frame_handle_type > frames =
    track_oracle_core::get_frames(
      track_handle_type(
        this->Track.get_cursor() ) );
...
{
  frame_handle_list_type these_frames =
    track_oracle_core::get_frames(
      track_handle_type(
        this->Track.get_cursor() ) );
  these_frames.insert( these_frames.end(), frames.begin(), frames.end() );
  track_oracle_core::set_frames(
    track_handle_type( this->Track.get_cursor() ),
    these_frames );
}
...
{
  os << "Track is: " <<
    *( dynamic_cast< track_oracle_row_view const* >( &track.Track ) );

  frame_handle_list_type frame_list =
    track_oracle_core::get_frames(
      track_handle_type(
        track.Track.get_cursor() ) );
  ...
  for( unsigned i = 0; i < frame_list.size(); ++i )
  {
    os << "...Frame " << i << " of " << frame_list.size()
       << " ";
    os <<
      *( dynamic_cast< track_oracle_row_view const* >( &track.Frame[
                                                         frame_list[ i ] ] ) )
       << "\n";
  }
  return os;
...
::list_missing_elements( const track_handle_type& _track ) const
{
  ...
  map< field_handle_type, bool > field_is_missing;
  {
    vector< field_handle_type > missing_fields =
      this->Track.list_missing_elements( _track.row );
    for( size_t i = 0; i < missing_fields.size(); ++i )
    {
      field_is_missing[ missing_fields[ i ] ] = true;
    }
  }
  ...
  {
    vector< field_handle_type > missing_fields =
      this->Frame.list_missing_elements( frame_list[ i ].row );
    for( size_t j = 0; j < missing_fields.size(); ++j )
    {
      field_is_missing[ missing_fields[ j ] ] = true;
    }
  }

  vector< field_handle_type > fields;
  for( map< field_handle_type, bool >::const_iterator i =
         field_is_missing.begin();
...
{
  return ( this->Track.contains_element( e ) ||
           this->Frame.contains_element( e ) );
}
...
::add_field_at_position(
  const pair< track_field_base*,
    schema_position_type >& f )
{
