...
::get_map( const vector< size_t >& data )
{
       ...
       ++component )
  {
    ...
    map< size_t,
      string_index_map_t >::const_iterator s =
      this->status_maps.find( component_index );
    if( s == this->status_maps.end() )
    {
      LOG_ERROR(
        main_logger,
        "Trace-flags for " << component->second <<
          " has no status map at index "
                           << component_index );
      return ret;
    }
    ...
    for( sim_cit i = s_map.begin();
         ( probe == s_map.end() && ( i != s_map.end() ) ); ++i )
    {
    ...
    if( probe == s_map.end() )
    {
      LOG_ERROR(
        main_logger,
        "Trace-flags for " << component->second <<
          " has no status string for index "
                           << status_index );
      return ret;
    }
...
::set_flag( const string& component, const string& status )
{
  if( ( component.find( ' ' ) != string::npos ) ||
      ( status.find( ' ' ) != string::npos ) )
  {
    throw runtime_error(
      "Cannot have spaces in state flag key/value pairs: '" +
      component + "' ; '" + status + "'" );
  }

  // both members of ci are indices; if ci = (5,7) that means data[5] == 7.
  pair< size_t,
    size_t > ci = state_flags_backend::get_instance().set_flag(
    component,
    status );
...
{
  pair< size_t,
    size_t > ci = state_flags_backend::get_instance().set_flag( component, "" );
...
ostream&
operator<<( ostream& os, const state_flag_type& t )
{
  map< string, string > m = t.get_flags();
  size_t n_bars = m.size() - 1;
  for( map< string, string >::const_iterator i = m.begin(); i != m.end();
       ++i, n_bars-- )
  {
    os << i->first;
    if( !i->second.empty() )
...
istream&
operator>>( istream& is, state_flag_type& t )
{
  string text;
  if( is >> text )
  {
...
dt::context dt::utility::state_flags::c(
  dt::utility::state_flags::get_context_name(),
