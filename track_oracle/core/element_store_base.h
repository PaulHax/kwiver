...
public:
  ...
  virtual std::vector< bool > exists(
    const std::vector< oracle_entry_handle_type >& sorted_hlist ) const = 0;

  virtual bool copy_value(
    const oracle_entry_handle_type& src,
    const oracle_entry_handle_type& dst ) = 0;
  virtual std::ostream& emit_as_kwiver(
    std::ostream& os,
    const oracle_entry_handle_type& h, const std::string& indent ) const = 0;
  virtual std::ostream& emit_as_csv(
    std::ostream& os,
    const oracle_entry_handle_type& h,
    bool emit_default_if_missing = false ) const = 0;
  virtual bool read_kwiver_xml_to_row(
    const oracle_entry_handle_type& h,
    const TiXmlElement* e ) = 0;
  virtual bool read_csv_to_row(
    const oracle_entry_handle_type& h,
    const std::map< std::string, std::string >& header_value_map ) = 0;
