...
{
  friend TRACK_ORACLE_EXPORT std::ostream& operator<<(
    std::ostream& os,
    track_base_impl& track );
...
// for e.g. printing
  ...
  std::vector< field_handle_type > list_missing_elements(
    const track_handle_type& track ) const;

  bool schema_contains_element( const element_descriptor& e ) const;
  bool schema_contains_element( field_handle_type f ) const;

  std::pair< track_field_base*,
    schema_position_type > clone_field_from_element(
    const element_descriptor& e ) const;
  bool add_field_at_position(
    const std::pair< track_field_base*,
      schema_position_type >& f );

  std::map< field_handle_type,
    schema_position_type > list_schema_elements() const;
};
