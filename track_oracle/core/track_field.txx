...
get_element_store( const field_handle_type& fh )
{
  ...
  if( typed_b == 0 )
  {
    ...
    throw runtime_error(
      "Failed to downcast element store for data term '" +
      ed.name + "'" );
  }
  return typed_b;
}
...
{
  static void
  set_handler( const dt::context& c )
  {
...
{
  static const bool value = impl_type_has_default< is_that_a_class< T >::value,
    T >::value;
};
...
{
  static void
  set_handler( element_store< Type >* es_ptr )
  {
...
{
  static void
  set_handler( element_store< Type >* )
  {
    // do nothing
  }
};
...
::lookup_or_create_element_store( const string& field_name )
{
  ...
  if( f == INVALID_FIELD_HANDLE )
  {
    ...
    set_default_value_handler< type_has_default_value_member< T >::value, T,
      Type >::set_handler( es_ptr );
    return f;
  }
...
::operator()( void ) const
{
  ...
  if( !track_oracle_core::field_has_row(
    this->host->get_cursor(),
    field_handle ) )
  {
    ostringstream oss;
  ...
  return track_oracle_core::get_field< Type >(
    this->host->get_cursor(),
    field_handle );
}
...
{
  track_oracle_core::remove_field< typename track_field< T >::Type >(
    row,
    this->field_handle );
}
...
track_field< T >
::lookup(
  const typename track_field< T >::Type& val,
  ...
  const track_handle_type& src_track )
{
  ...
  for( unsigned i = 0; i < frames.size(); ++i )
  {
    if( ( track_oracle_core::field_has_row(
      frames[ i ].row,
      this->field_handle ) ) &&
        ( track_oracle_core::get_field< Type >(
          frames[ i ].row,
          this->field_handle ) == val ) )
    {
      return frames[ i ].row;
    }
  }
...
::exists( void ) const
{
  ...
  return track_oracle_core::field_has_row(
    this->host->get_cursor(),
    this->field_handle );
}
...
{
  pair< bool,
    typename track_field< T >::Type > probe =
    track_oracle_core::get< Type >( src, field_handle );
  if( probe.first )
  {
...
::io() const
{
  element_store< Type >* typed_b =
    get_element_store< Type >( this->field_handle );
  return track_field_io_proxy< Type >(
    typed_b->get_io_handler(),
    this->operator()() );
}
...
template < typename T >
ostream&
operator<<( ostream& os, const track_field< T >& f )
{
  os << " (" << f.field_handle << ") "
     << f.name;
  try
