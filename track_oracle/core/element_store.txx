...
static kwiver::vital::logger_handle_t local_logger( kwiver::vital::get_logger(
  __FILE__ ) );
...
::element_store( const element_descriptor& e )
  : element_store_base( e ),
    default_value_ptr( 0 ),
    io_base_ptr( new kwiver_io_base< T >( e.name ) )
{}
...
::exists( const oracle_entry_handle_type& h ) const
{
  typename map< oracle_entry_handle_type,
    T >::const_iterator p = this->storage.find( h );
...
::exists( const vector< oracle_entry_handle_type >& sorted_hlist ) const
{
  ...
  typename map< oracle_entry_handle_type,
    T >::const_iterator p = this->storage.lower_bound( sorted_hlist[ 0 ] );
  typename map< oracle_entry_handle_type,
    T >::const_iterator e = this->storage.end();
  ...
  while( keep_going )
  {
    ...
    // or
    //
    // 2) if p->first == sorted_hlist[ret_index], set return flag and increment
    // both
    //
    // or
    //
    ...
    oracle_entry_handle_type left( p->first ),
    right( sorted_hlist[ ret_index ] );
    if( left < right )
    {
      keep_going = ( ++p != e );
    }
  ...
  const oracle_entry_handle_type& dst )
{
  if( src == INVALID_ROW_HANDLE )
  {
    LOG_ERROR(
      local_logger,
      "copy_value for " << this->get_descriptor().name <<
  ...
  typename map< oracle_entry_handle_type,
    T >::iterator p = this->storage.find( src );
...
element_store< T >
::emit_as_kwiver(
  ostream& os, const oracle_entry_handle_type& h,
...
::emit_as_csv(
  ostream& os, const oracle_entry_handle_type& h,
  bool emit_default_if_missing ) const
{
  typename map< oracle_entry_handle_type,
    T >::const_iterator p = this->storage.find( h );
  if( p != this->storage.end() )
...
bool
element_store< T >
::read_kwiver_xml_to_row(
  const oracle_entry_handle_type& h,
  const TiXmlElement* e )
{
  ...
  if( !this->io_base_ptr->read_xml( e, val ) )
  {
    LOG_ERROR(
      local_logger,
      "Couldn't parse instance of " << this->get_descriptor().name <<
        " at row " << e->Row() );
    return false;
  }
  this->storage[ h ] = val;
...
::get_default_value() const
{
  ...
  {
    throw runtime_error(
      "No default value set for " +
      this->get_descriptor().name );
  }
...
//
// Implementation of emit_as_XML() deferred to xml_output_common_classes.cxx and
// the
