...
::explore( const kwiver::vital::plugin_factory_handle_t fact )
{
  ...
  {
    out_stream() << "    Instrumentation type: " << instr_name << "   " <<
      descrip << std::endl;
    return;
  }
  ...
  for( kwiver::vital::config_block_key_t const& key : all_keys )
  {
    ...
    m_context->output_stream() << indent << "Description: " <<
      m_context->wrap_text( descr ) << std::endl;
  }
...
register_explorer_plugin( kwiver::vital::plugin_loader& vpm )
{
  ...
  auto fact = vpm.ADD_FACTORY(
    ...
    kwiver::vital::process_instrumentation_explorer );
  fact->add_attribute(
    kwiver::vital::plugin_factory::PLUGIN_NAME,
    "process-instrumentation" )
    .add_attribute(
    kwiver::vital::plugin_factory::PLUGIN_DESCRIPTION,
    "Plugin explorer for process category." )
