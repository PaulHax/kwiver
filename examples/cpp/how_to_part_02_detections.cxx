...
how_to_part_02_detections()
{
  ...
  // kwiver::vital::algo::image_object_detector interface
  ...
  kwiver::vital::image_container_sptr hough_img =
    drawer->draw( hough_detections, ocv_img );
  ...
  cv::namedWindow( "Hough Detections", cv::WINDOW_AUTOSIZE ); // Create a window
                                                              // for display.
  cv::imshow( "Hough Detections", hough_mat );                     // Show our
                                                                   // image
                                                                   // inside it.
  ...
  kwiver::vital::bounding_box_d bbox1( ocv_img->width() * 0.25,
    ocv_img->height() * 0.25,
  ...
  // A Classification
  // The detected_object_type is created by a classifier which is sometimes part
  // of the detector.
  ...
  kwiver::vital::detected_object_type_sptr type1(
  ...
  kwiver::vital::detected_object_sptr detection1(
    new kwiver::vital::detected_object( bbox1, confidence1, type1 ) );
  detection1->set_detector_name( "center" );

  // Let's add a few more detections to our detection set and write it out in
  // various formats
  kwiver::vital::bounding_box_d bbox2( ocv_img->width() * 0.05,
    ocv_img->height() * 0.05,
    ocv_img->width() * 0.55, ocv_img->height() * 0.55 );
  double confidence2 = 0.50;
  kwiver::vital::detected_object_type_sptr type2(
    new kwiver::vital::detected_object_type() );
  type2->set_score( "car", 0.04 );
  type2->set_score( "fish", 0.12 );
  type2->set_score( "flag pole", 0.67 );

  kwiver::vital::detected_object_sptr detection2(
    new kwiver::vital::detected_object( bbox2, confidence2, type2 ) );
  detection2->set_detector_name( "upper left" );

  kwiver::vital::bounding_box_d bbox3( ocv_img->width() * 0.45,
    ocv_img->height() * 0.45,
    ocv_img->width() * 0.95, ocv_img->height() * 0.95 );
  double confidence3 = 0.75;
  kwiver::vital::detected_object_type_sptr type3(
    new kwiver::vital::detected_object_type() );
  type3->set_score( "car", 0.22 );
  type3->set_score( "fish", 0.08 );
  type3->set_score( "flag pole", 0.07 );

  kwiver::vital::detected_object_sptr detection3(
    new kwiver::vital::detected_object( bbox3, confidence3, type3 ) );
  ...
  kwiver::vital::image_container_sptr img_detections = drawer->draw(
    detections,
    ocv_img );
  // Let's see what it looks like
  cv::Mat mat =
    kwiver::arrows::ocv::image_container::vital_to_ocv(
      img_detections->get_image(),
      kwiver::arrows::ocv::image_container::RGB_COLOR );
  cv::namedWindow( "Detections", cv::WINDOW_AUTOSIZE ); // Create a window for
                                                        // display.
  cv::imshow( "Detections", mat );                     // Show our image inside
                                                       // it.
  cv::waitKey( 2000 );                                       // Wait for 2s
  ...
  else
  {
    ...
    for( auto det = kpf_detections->cbegin(); det != ie; ++det )
    {
      ...
      ss << "detector_name " << ( *det )->detector_name() << "\n"
         << "bounding box :" << "x1(" << bbox.min_x() << ") y1(" <<
        bbox.min_y() << ") x2(" << bbox.max_x() << ") y2(" << bbox.max_y() <<
        ") \n"
